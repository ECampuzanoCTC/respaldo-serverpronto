	    ...
	    $result_pid = mysql_query($query_pid) or die('Query failed: ' . mysql_error());
            ...
            if ($msg != "" && $user_id && mysql_num_rows($result)) {
                ...
                if ($valid_comp) {
                    ...
                    $query2 = "INSERT INTO easycases (uniq_id,case_no,project_id,user_id,type_id,priority,title,message,assign_to,due_date,istype,format,status,legend,previous_legend,isactive,dt_created,actual_dt_created,from_email) VALUES
('" . $unq . "','" . $row['case_no'] . "','" . $row['project_id'] . "','" . $user_id . "','" . $row['type_id'] . "','" . $row['priority'] . "','','" . mysql_real_escape_string($msg) . "','" . $row['assign_to'] . "','" . $row['due_date'] . "','2','2','" . $row['status'] . "','".$row['legend']."','".$row['legend']."','" . $row['isactive'] . "','" . $actual_dt_created . "','" . $actual_dt_created . "','1')";
                 #   echo "<pre>";print_r($row);print $query2;exit;
