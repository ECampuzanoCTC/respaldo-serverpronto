$(document).ready(function () {
    ...
    $(document).on('click', 'body', function (e) {
        $(e.target).closest('.estb').size() > 0 || $(e.target).hasClass('fl') ? '' : $('.estb').show();
        $('.est_hr').hide();
    });
    $('[rel=tooltip]').tipsy({
        gravity: 's',
...
function editTimelog(obj) {
    $obj = $(obj);
    if (typeof $obj.closest('td').attr('data-logid') == 'undefined' && typeof $obj.closest('td').attr('data-prjctid') == 'undefined')
        return false;
    createlog('', '', $obj);
}
/*delete time log*/
function deletetimelog(obj) {
    $obj = $(obj);
    if (typeof $(obj).closest('td').attr('data-logid') == 'undefined')
        return false;
...
function changeEstHour(caseId, caseUniqId, cno, value) {
    ...
    var proj_uniq_id = $('#CS_project_id' + caseId).val();
    var prev_est_hr = LogTime.convertToMin($("#est_hr" + caseId).attr('data-default-val'));
    $(".estb").hide();
    ...
    $.post(HTTP_ROOT + "easycases/ajax_change_estHour", {"caseId": caseId, "estHour": estHour},
        ...
        $.post(HTTP_ROOT + "easycases/getTotEsthr", {proj_uniq_id: proj_uniq_id}, function (res) {
            if (res) {
                if (res.total_tsk_etimate_hr > res.prjct_estmt_hr) {
                    // openPopup();
                    var dfiffrnce_hr = res.total_tsk_etimate_hr - res.prjct_estmt_hr;
                    var project_dffrnc = res.prjct_estmt_hr + dfiffrnce_hr;
                    var tsk_dffrnce = estHour - dfiffrnce_hr > 0 ? estHour - dfiffrnce_hr : 0;
                    ...
                    $('#est_prjctid').val(res.projId);
                    $('#est_tskid').val(caseId);
                    $('#prjct_dfrnce').val(project_dffrnc);
                    $('#tsk_dffrnce').val(tsk_dffrnce);
                    $('#chng_type').val('task_reply');
                    $('#tsk_id').val(caseId);
                    $('#tsk_unq_id').val(caseUniqId);
                    $('#tsk_cs_no').val(cno);
                    $('#prev_est_hr').val(prev_est_hr);
                } else {
...
function setEditTimeLog(response) {
    ...
    //console.log(response.start_datetime_v1);
...
function stvariables() {
    ...
    $(".autogeneratecls").each(function (index) {
        if ($(this).prop("checked") == true) {
            autoindx += index + ",";
        }
    });
    $('#chked_autos').val(autoindx);
    $stay = false;
...
function ajax_timelog_export_csv() {
    //var url_params = 'projuniqid='+$('#projFil').val()+'&usrid='+$('#rsrclog').val()+'&strddt='+$('#logstrtdt').val()+'&enddt='+$('#logenddt').val();
