<?php if(isset($allProjArr) && count($allProjArr)){ ?>
    <?php if($page != 'reports' && $page!='import' && $page != 'chart' && $page != 'hours_report' && $page != 'glide_chart' && $page != 'timer' && $page != 'ganttv2') {?>
...
<a href="javascript:jsVoid();" class="proj_lnks ttc" onClick="updateAllProj('proj_all', 'all', '<?php echo $page; ?>', '0', '<?php echo  rawurlencode('All');?>')"><?php echo $this->Format->shortLength('All',30); ?> (<?php echo $allPjCount['0']['0']['count']; ?>)</a>
      <?php }
?>
	<?php
	$i = 0;
	...
	?>
		...
		<?php } else if($page == 'timer'){ ?>
                 <option value="<?php echo $proj['p']['uniq_id'];?>"><?php echo $proj['p']['name']; ?></option>
		<?php } else { ?>
