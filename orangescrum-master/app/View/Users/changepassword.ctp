...
<table cellspacing="0" cellpadding="0" class="col-lg-5" style="text-align:left;">
    <tbody>
        <tr style="visibility:<?php if(NO_PASSWORD){echo 'hidden; margin-top: -40px;position: absolute;';} else { echo 'visible;';} ?>" >
            <th><?php echo __("Old Password"); ?>:</th>
            <td style="padding-bottom:25px">
		<?php echo $this->Form->password('old_pass',array('value'=>'','class'=>'form-control','id'=>'old_pass','onKeyPress'=>'return noSpace(event)','autocomplete'=>'off','readonly' => true)); ?>
	    </td>
        </tr>
        <tr>
		...
		<button type="submit" value="<?php if(NO_PASSWORD){echo 'Set';}else{ echo 'Change';}?>" name="submit_Pass"  id="submit_Pass" class="btn btn_blue" onclick="document.getElementById('changepass').value='1'"><i class="icon-big-tick"></i><?php echo __("Change"); ?></button>
...
<?php echo $this->Form->end();
    ...
    echo "<center>".__('Sorry, this is not available in this version.')."</center>";
} ?>
