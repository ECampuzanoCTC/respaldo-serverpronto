...
<div class="proj_grids">
    ...
    <div class="col-lg-12 user_div m-left-20">
        ...
        foreach ($userArr as $user) {
            ...
            $ori_user = array('Owner', 'Admin', 'User');
            ?>
            <div class="usr_mcnt fl cmn_bdr_shadow">	

                <div class="usr_top_cnt">
                    <div class="usr_cat <?php echo $colors;
            if (!in_array($usr_typ_name, $ori_user)) {
                echo " usr_other_role";
            } ?>"><?php echo $usr_typ_name; ?></div>
                        ...
                        <ul class="dropdown-menu mng_dropdown-caret usr_mng">
                            ...
                            <?php } else { ?>
                                ...
                                <?php } else if ($role == 'client' && defined('INV') && INV == 1) { ?>
                                    ...
                                    <?php } ?>
                                    <?php
                                    if (defined('ROLE') && ROLE == 1 && array_key_exists('Disable Users', $roleAccess) && $roleAccess['Disable Users'] == 0) {
            ...
            if (defined('ROLE') && ROLE == 1 && array_key_exists('Remove Project', $roleAccess) && $roleAccess['Remove Project'] == 0) {
                ...
                ?>
                    ...
                    <div class="user_img holder <?php echo $random_bgclr; ?>">
                        ...
                        <?php } else { ?>
                            <?php if (isset($user['User']['name']) && trim($user['User']['name'])) { ?>
                                ...
                                <?php
                            } else if (isset($user['User']['short_name']) && trim($user['User']['short_name'])) {
                    ...
                    <h3 class="invite_user_cls ellipsis-view"  rel="tooltip" title="<?php echo ucfirst($user['User']['name']); ?>" data-usr-id="<?php echo $user['User']['id']; ?>" data-usr-name="<?php echo trim($user['User']['name']); ?>"><?php
                    if (isset($user['User']['name']) && trim($user['User']['name'])) {
                        echo trim($user['User']['name']);
                    } else {
                        echo "&nbsp;";
                    }
                    ?></h3>
    ...
    ?></span>
                        ...
                        <li>
                            <span class="usr_email cnt_ttl_usr"><?php echo __("Role"); ?></span>
                            <span class="cnt_usr"><?php echo $usr_typ_name; ?></span>
                        </li>
                        <li>
    ...
    <?php
    if (isset($user['User']['all_project']) && trim($user['User']['all_project'])) {
        echo $user['User']['all_project'];
    } else {
        echo 'N/A';
    }
    ?>
...
<?php } ?>
<style>
    .usr_top_cnt .usr_cat.usr_other_role {
        font-size: 12px;
        left: 0;
        right: 0px;
        text-align: center;
