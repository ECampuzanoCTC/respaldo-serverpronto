...
<?php if (PAGE_NAME != 'confirm_import') { ?>
    <div class="exp_innerdiv" id="imploade_file"  <?php if (isset($fileds)) {
...
//			   }else{
            ...
            <form action="<?php echo HTTP_ROOT; ?>projects/csv_dataimport/<?php echo $proj_uid; ?>" enctype="multipart/form-data" method="post" name="data_import_form" id="data_import_form">
                ...
                <div class="fl customfile-button">
                    <input type="file" class="fl" name="import_csv" id="import_csv" onchange="check_csvfile();check_multiple_project();"/>
        ...
        <div class="fl import-info-dif import_proj" style="border:1px solid #DDD;">
            <div class="chk_content">
                ...
                <ul class="chk_desc">
                    ...
                    <li style="color:#666666"><b><?php echo __("Estimated Hour"); ?>  - </b> (<span style="color:#FF0000"><?php echo __("hh:mm format only"); ?></span>)</li>
                    <li style="color:#666666"><b><?php echo __("Start Time & End Time"); ?>  - </b> (<span style="color:#FF0000"n><?php echo __("hh:mmam/pm format only, Ex. 10:45am/10:45pm - no space between time and meridian"); ?></span>)</li>
                    <li style="color:#666666"><b><?php echo __("Break Time"); ?>  - </b> (<span style="color:#FF0000"><?php echo __("hh:mm format only"); ?></span>)</li>
                    <li style="color:#666666"><b><?php echo __("Is Billable"); ?>  - </b> (<span>0,1</span>)</li>
                </ul>

                <h4 class="chk_head"><?php echo __("Help"); ?></h4>
                <ul class="chk_desc">
                    <li><?php echo __("Choose a project to Import Task"); ?><br /><font style="color:red;"><?php echo 'NOTE :-'; ?></font> <br /><?php echo __('We can upload more one project if we select "All" project from project dropdown'); ?></li>
                    ...
                    <li><span style="color:#FF0000"><?php echo __("Left blank the columns having no entry."); ?></span></li>
                </ul>
    ...
    <?php if (isset($fileds)) { ?>
            ...
            echo htmlentities($mserialize); ?>" name="milestone_arr"/>
                <input type="hidden" value="<?php echo $new_file_name; ?>" name="new_file_name"/>
                <input type="hidden" value="<?php $tserialize = serialize($task);
            echo htmlentities($tserialize); ?>" name="task_arr"/>
        ...
        <?php if (isset($task_err) && $task_err) { ?>
            ...
            <?php if (in_array('Start Time', $fileds) || in_array('End Time', $fileds)) { ?>
                                    ...
                                    <li><?php echo __("Invalid start time or end time or break time or spent hour"); ?></li>
                                <?php } ?>					
                                <li><?php echo __('"Assigned To"  must be an email'); ?></li>
                            </ul>
        ...
        <!--			<tr>
                    ...
                    <?php //foreach ($milestone_arr as $key => $val){?>
                                    <table width="100%">
                                            <tbody>
                                            <tr>
                                                    <td colspan="6"><?php echo '<b><i>' . $val['title'] . '</i></b>'; ?> &nbsp;&nbsp; <span class="fr" style="margin-right: 2px;font-style:italic"><?php if ($val['start_date']) { ?> <b><?php echo __("Start Date"); ?>: </b><?php echo date('m/d/Y', strtotime($val['start_date']));
            } ?>&nbsp;&nbsp; <?php if ($val['end_date']) { ?> <b><?php echo __("End Date"); ?>: </b><?php echo date('m/d/Y', strtotime($val['end_date']));
            } ?></span></td>
                                            </tr>-->
        <?php
        if (isset($task) && $task) {
            ...
            foreach ($task AS $k => $v) {
                ...
                ?>
                    ...
                    <?php if (isset($v[strtolower($hv)])) { ?>
                                        ...
                                        $err = 1;
                                        ?>class="error-imp-data"<?php } ?> valign="top"><?php echo htmlentities($v[strtolower($hv)]); ?></td>
                    <?php }
                    }
                ?>
                            </tr>
            ...
            <tr>
                <td colspan="2">
                    <table>
                        ...
                        <tr>
                            <td colspan="2"><?php echo __("Total data"); ?>:&nbsp;<b><?php echo ($newtotal_task); ?></b> <?php echo __("rows"); ?></td>
                        </tr>
                        <tr>
                            <td colspan="2" ><?php echo __("Valid data"); ?>:&nbsp;<b><?php echo $total_valid_rows; ?></b> <?php echo __("rows"); ?></td>
                        </tr>
                        <tr>
                            <td colspan="2" ><b><?php echo $newtotal_task; ?></b> <?php echo __("Task(s) Imported into project"); ?>:&nbsp;<b><?php echo $proj_name; ?></b></td>
                        </tr>
    <!--					<tr>
        ...
        if (pro_id == 'all') {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.trim() == 2) {
                        alert('There is no project column in the CSV');
                        window.location = HTTP_ROOT + "<?php echo $this->request->url; ?>";
                        $("#import_csv").val('');
                    }
                }
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.trim() == 'more_pro') {
                    delete_file(filename);
                    var msg = 'You have choosen "' + pname + '" project to import task, but you are trying to import task(s) for project(s) other than selected project. Please choose "all" option from the project drop down menu to import multiple project(s) task(s) at a time.';
                    alert(msg);
                    ...
                    delete_file(filename);
                    var msg = "Invalid CSV file, <a href='" + HTTP_ROOT + "projects/download_sample_csvfile' style='text-decoration:underline;color:#0000FF'>Download</a> and check with our sample file";
                    showTopErrSucc('error', msg);
                    window.location = HTTP_ROOT + 'import-export';
                }
            }
    ...
    $(function () {
        ...
        if (cont == 'projects' && action == 'confirm_import') {
            ...
            if (valid_val != 0) {
                ...
                showTopErrSucc('success', "Succesfully imported")
            } else {
                var msg = "Invalid CSV file, <a href='" + HTTP_ROOT + "projects/download_sample_csvfile' style='text-decoration:underline;color:#0000FF'>Download</a> and check with our sample file";
                showTopErrSucc('error', msg);
            }
