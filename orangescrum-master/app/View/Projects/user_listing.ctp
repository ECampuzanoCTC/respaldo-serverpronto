...
<div class="scrl-ovr">
	...
	<tr id="listing<?php echo $cscount; ?>" class="rw-cls <?php echo $class; ?>">
	    <td>
			...
			?>
	    	    <input type="checkbox" class="chkbx_cur" id="usCheckBox<?php echo $cscount; ?>" value="<?php echo $user_id; ?>" data-usr-name="<?php echo trim($user_name);?>" checked="checked" disabled="disabled"/>
			<?php
		    } else {
			?>
	...
	<?php if ($v1['ProjectUser']['default_email'] == 0) { ?>
	    	    <ul class="onoff">
	    		<li class="off" ><a href="javascript:void(0)" onclick="setemail(this, 'off', '<?php echo $v1['ProjectUser']['id'] ?>', 'on');"><?php echo __("OFF"); ?></a></li>
	    		<li><a href="javascript:void(0)" onclick="setemail(this, 'on', '<?php echo $v1['ProjectUser']['id'] ?>', 'off');"><?php echo __("ON"); ?></a></li>
	    	    </ul>
	<?php } else { ?>
	    	    <ul class="onoff">
	    		<li ><a href="javascript:void(0)" onclick="setemail(this, 'off', '<?php echo $v1['ProjectUser']['id'] ?>', 'on');"><?php echo __("OFF"); ?></a></li>
	    		<li class="on"><a href="javascript:void(0)" onclick="setemail(this, 'on', '<?php echo $v1['ProjectUser']['id'] ?>', 'off');"><?php echo __("ON");?></a></li>
	    	    </ul>
	<?php } ?>   
		</td>
...
<?php } else { ?>
    <center class="fnt_clr_rd"><?php echo __("No users assigned"); ?></center>
