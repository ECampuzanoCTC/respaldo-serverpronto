...
<?php
if (defined('LANG') && LANG == 1 && Configure::read('Config.language') != 'eng') {
...
<?php } ?>
            ...
            eventRender: function (event, element) {
                ...
                if (event.photo != undefined && event.photo != '') {
                    element.find(".fc-title").before("<img rel='tooltip' src='" + HTTP_ROOT + "users/image_thumb/?type=photos&file=" + event.photo + "&sizex=26&sizey=26&quality=100' class='round_profile_img' height='26' width='26' title='Assigned to: " + event.name + "'/>" + addition);
                    //element.prev(".fc-day-number").css( "background-color", "red" );
                } else {
                    element.find(".fc-title").before("<span class='round_profile_img " + event.usr_color + " cal_prof_styl1' title='Assigned to: " + event.name + "' rel ='tooltip'>" + event.usr_shtnm + "</span>" + addition);
                ...
                element.find('.fc-title').attr({title: event.original_title, rel: 'tooltip'}).tipsy({html: true});
                element.find('.round_profile_img').attr({rel: 'tooltip'}).tipsy({html: true});
                element.find('.fc-content').css("background-color", event.legendcolor);
                //console.log(event);
                var clrCod = '';
