						...
						<a href="javascript:void(0);">
							<div class="fr" style="width:64px;">
								<?php if($totalRecentCase >3 && $limit1 >=3  ) {?>
									<span class="menu_arrow_prev" onclick="previousRecentCase(<?php echo $limit1, __('less'); ?>);" title="Previous"> << </span>
								<?php } if($totalRecentCase >3 && ($limit1+3)< $totalRecentCase){ ?>
									<span class="menu_arrow_next" onclick="showmoreRecentCase(<?php echo $limit1, __('more'); ?>);" title="Next"> >> </span>
								<?php } ?>	
							</div>
							<div class="cb"></div>
						</a>
    ...
    ?>
		<li style="color:#FFFFFF;text-align:center;font-size:14px;"><?php echo __("No recently viewed"); ?></li>
    <?php } ?>
