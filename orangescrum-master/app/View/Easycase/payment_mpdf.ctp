        ...
        <div class="invoice-content">
            <div class="invo-cont1">
                <table class="width100">
                    ...
                    <?php if(isset($i['Payment']['logo']) && trim($i['Payment']['logo']) !='' && $this->Format->pub_file_exists(DIR_INVOICE_PHOTOS_S3_FOLDER. $company_id . '/', $i['Payment']['logo'])) { ?>
                        <img src="<?php echo $this->Format->generateTemporaryURL(DIR_INVOICE_PHOTOS_S3 . $company_id . '/'.  $i['Payment']['logo']);?>" style="max-height:100px;" />
                        <?php sleep(2);?>
                        <?php ?>
                        <?php }else if ($i['Payment']['logo'] != '' && $this->Format->imageExists(DIR_USER_PHOTOS, $i['Payment']['logo'])) { ?>
                        <img src="<?php echo HTTP_ROOT.'files'.DS.'photos'.DS.trim($i['Payment']['logo']);?>" style="max-height:100px;" /><?php ?>
                    <?php } else { ?>
                        <?php ?><img src="<?php echo HTTP_IMAGES; ?>default-invoice-logo.png" style="max-height:100px;" /><?php  ?>&nbsp;
                    <?php } ?>
                        
                    <?php if (!empty($i['Payment']['payment_from'])) { ?>
                        <br /><br />
                        <p>  <?php echo nl2br($i['Payment']['payment_from']); ?> </p>
                        <br />
                    <?php } ?>

                    <div class="bill">
                        <h2><span style="color:#1f88c1">Bill To</span></h2>
                        <?php if (!empty($i['Payment']['payment_to'])) { ?>
                        <br />
                            <p><?php echo nl2br($i['Payment']['payment_to']); ?></p>               
                        <?php } ?>
                    </div>
                </td>
                    ...
                    <div class="invo-hd text-right">                       
                        <div class="invo-dt text-right">
                            <table class="width100 margintp" >
                                <tr>
                                    <td class="invo-rht-dt margintp" align="right" colspan="2"><h1 class="margin5"><a><span style="color:#D8783C; font-size:32px;">Payment<span></a></h1></td>
                                </tr>
                                <tr>
                                    <td colspan="2">&nbsp;</td>
                                </tr>
                                <tr>
                                    <td class="invo-rht-dt" align="right">Payment#</td>
                                    <td class="invo-lft-dt"  align="left">:&nbsp;&nbsp;<?php print $i['Payment']['payment_no']; ?></td>
                                </tr>
                                <?php /*
                                 <tr>
                                    <td class="invo-rht-dt" align="right" >Terms</td>
                                    <td class="invo-lft-dt"  align="left">:&nbsp;&nbsp;<?php
                                 if (isset($i['Payment']['payment_term'])) {
                                    $invoice_term = $i['Payment']['payment_term'];
                                    echo intval($invoice_term) == 0 ? 'Due on receipt' : 'Net ' . $invoice_term;
                                }
                                ?></td>
                                </tr> */ ?>
                                <tr>
                                    <td class="invo-rht-dt" align="right">Generation Date</td>
                                    <td class="invo-lft-dt"  align="left">:&nbsp;&nbsp;<?php echo date('M d,Y', strtotime($i['Payment']['issue_date'])); ?></td>
                                </tr>
                                <tr>
                                    <td class="invo-rht-dt" align="right">Received Date</td>
                                    <td class="invo-lft-dt"  align="left">:&nbsp;&nbsp;<?php if($i['Payment']['receive_date']){ echo date('M d,Y', strtotime($i['Payment']['receive_date'])); } ?></td>
                                </tr>
                            </table>
                            </div>
                        </div>
            ...
            <div class="tab-cont">
                <table border="0" cellpadding="0" cellspacing="0">
                    ...
                    <?php if (!empty($i['PaymentLog'])) { ?>
                        <?php foreach ($i['PaymentLog'] as $log) { ?>
                ...
                <?php if (!empty($i['PaymentLog'])) { ?>
                    <table class="tol-amount width100 margintp">
                        ...
                        <table class="inv-summary text-right width100 margintp" border="0" cellpadding="0" cellspacing="0">
                            ...
                            <?php if (floatval($i['Payment']['discount']) > 0) { ?>
                                <tr>
                                    <td class="width85">Discount<span class="percent"><?php print ($i['Payment']['discount_type'] != 'Flat') ? '('.$i['Payment']['discount'] . '%)' : ''; ?></span></td>
                                     <td class="width220">
                                        <?php
                                        $discount = ($i['Payment']['discount_type'] != 'Flat') ? number_format(($grandTotal * $i['Payment']['discount']) / 100, 2, '.', '') : number_format($i['Payment']['discount'], 2, '.', '');
                                        print number_format($discount, 2, '.', ',');
                                        ?>
                                    </td>
                                </tr>
                            <?php } ?>
                            <?php if (floatval($i['Payment']['tax']) > 0) { ?>
                                <tr>
                                    <td>Tax<span class="percent">(<?php echo floatVal($i['Payment']['tax']); ?>%)</span></td>
                                    <td>
                                        <?php
                                        $tax = ((floatVal($grandTotal) - floatVal($discount)) * floatVal($i['Payment']['tax'])) > 0 ? number_format(((floatVal($grandTotal) - floatVal($discount)) * floatVal($i['Payment']['tax'])) / 100, 2, '.', '') : 0;
                            ...
                            <tr class="tot-tr">
                                ...
                                <td class="tot-amt width220" style="border-left:0px;">
                                    <?php echo trim($i['Payment']['currency']) != '' ? trim($i['Payment']['currency']) : 'USD'; ?>
            ...
            <?php if (trim($i['Payment']['note']) != '') { ?>
                <div class="inf">
                    <b>Remittance Details:</b>
                    <p><?php echo nl2br($i['Payment']['note']); ?></p>
                </div>
            <?php } ?>
            <?php if (trim($i['Payment']['terms']) != '') { ?>
                <div class="inf">
                    <b>Note:</b>
                    <p> <?php echo nl2br($i['Payment']['terms']); ?> </p>
                </div>
            <?php } ?>
        </div>
    </body>
