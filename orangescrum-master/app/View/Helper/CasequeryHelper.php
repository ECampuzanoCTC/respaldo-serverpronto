...
class CasequeryHelper extends AppHelper {
	...
	function getProjectName($pid)
	{
		...
		$Project = ClassRegistry::init('Project');
		$Project->recursive = -1;
		$fild_prj = array('Project.name','Project.uniq_id');
		if (defined('TSG') && TSG == 1) {
			$fild_prj = array('Project.name','Project.uniq_id','Project.workflow_id');
		}
		$pjArr = $Project->find('first', array('conditions'=>array('Project.id' => $pid,'Project.isactive'=>1,'Project.company_id' => SES_COMP), 'fields'=>$fild_prj));
		return $pjArr;
	}
