...
#show_milestonelist .kanban-main .kanban-child{width:24.2%;margin:0 1% 20px 0;}
#show_milestonelist .kanban-main .kanban-child:nth-child(4n+4){margin-right:0}
.kanban-main .dropdown-menu {left:inherit;right:0;}
...
<div class="kanban-main">
	...
	if(caseCount && caseCount != 0){
		...
		for(var caseKey in caseAll){
			...
			console.log(getdata.Easycase.Mid+"---"+getdata.Easycase.Emid);
			var getTotRep = 0;
            var max_lgndall = 0;
			...
			%>
				...
				<div class="kbhead kbhead_<%= getdata.Easycase.Mid %>">
				  ...
				  <div class="fr" title="<% if(getdata.Easycase.Mid == null){ %><%= milestone_hrs.closed_task +' closed' %><% }else{ %><%= getdata.Easycase.totalClosedCs + ' closed' %><% } %>" rel="tooltip">
					...
					<% if(!getdata.Easycase.Mid){ %>
                    <?php if(defined('ROLE') && ROLE == 1 && array_key_exists('Create Milestone', $roleAccess) && $roleAccess['Create Milestone'] == 0){}else{ ?>
...
<% } %>
<% if(typeof milestones !="undefined" && countJS(milestones)>0){
