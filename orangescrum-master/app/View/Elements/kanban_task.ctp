...
<!--		<div class="fr"><%= mlsttotalCs %></div>-->
		...
		<div class="cb"></div>
		<div class=" fl imprv_bar col-lg-6" title="<%= totalClosedCs + ' <?php echo __("out of");?> ' + mlsttotalCs + ' <?php echo __("tasks are closed");?>' %>" rel="tooltip">
...
<div class="kanban-main">
	...
	for(var taskallkey in caseAll){
		...
		var tasktype='';
		if(taskallkey=='newTask'){tasktype='New';}else if(taskallkey=='inprogressTask'){tasktype='In Progress';}else if(taskallkey=='resolvedTask'){tasktype='Resolved'}else if(taskallkey=='closedTask'){tasktype='Closed';}else{tasktype =taskallkey;}
	 if(!morecontent){
	%>
		...
		<div class="kbhead kbhead_<%= taskallkey.replace(/\s/g, '') %>">
			...
			<div class="fr" title="<?php echo __("Tasks"); ?>" rel="tooltip" id="cnter_<%= lgnd %>" style="margin-left:7px;color:#aaaaaa"></div>
                       <?php if(defined('CR') && CR == 1 && SES_CLIENT ==1 && $this->Format->get_client_permission('task')==1){ 
                            /**Not Show create task */
                        }else{?> 
			...
			<div class="fl mlst_prj_nm estimated_spent_dv">	
				...
				%>
				<span style="margin-right:10px;" title="<?php echo __("Estimated");?>: <%= getHourMinut(arr_kanban_hours[taskallkey].estd_hrs) %>" rel="tooltip"><%= getHourMinut(arr_kanban_hours[taskallkey].estd_hrs) %></span>
				<span class="<%= res_progreess[1] %>" title="<?php echo __("Spent");?>: <%= getHourMinut(arr_kanban_hours[taskallkey].sphours) %>" rel="tooltip"><%= getHourMinut(arr_kanban_hours[taskallkey].sphours) %></span>
			</div>
			<div class="cb"></div>
			<% if(prjworkflow_id == 0 && tasktype == 'New' || tasktype == 'In Progress' || tasktype == 'Resolved' || tasktype == 'Closed'){ %>
				<input type="hidden" id="<%= taskallkey.replace(/\s/g,'') %>_limit" value="<%= taskallkey.replace(/\s/g,'')+'_limit' %>" />
			<% }else{ %>
				<input type="hidden" id="<%= taskallkey.replace(/\s/g,'') %>_limit" value="<%= taskallkey.replace(/\s/g,'')+'_limit' %>" />
			<% } %>
		</div>
	...
	<%	}
		...
		<div class="pr_<%= easycase.getPriority(casePriority) %> kbtask_div" title="<% if(getTotRep && getTotRep!=0) { %><?php echo __('Updated'); ?><% } else { %><?php echo __('Created'); ?><% } %> <?php echo __("by");?> <%= getdata.Easycase.usrShortName %> <% if(getdata.Easycase.updtedCapDt.indexOf('Today')==-1 && getdata.Easycase.updtedCapDt.indexOf('Y\'day')==-1) { %><?php echo __('on'); ?><% } %> <%= getdata.Easycase.updtedCapDt %> ">
			<!--<div class="fl" rel="tooltip" title="<% if(getTotRep && getTotRep!=0) { %><?php echo __('Updated'); ?><% } else { %><?php echo __('Created'); ?><% } %> <?php echo __("by");?> <%= getdata.Easycase.usrShortName %> <% if(getdata.Easycase.updtedCapDt.indexOf('Today')==-1 && getdata.Easycase.updtedCapDt.indexOf('Y\'day')==-1) { %>on<% } %> <%= getdata.Easycase.updtedCapDt %> "> <%= getdata.Easycase.proImage %></div> -->	
			...
			<div class="fr" title="<?php echo __('Assigned to'); ?><%= getdata.Easycase.asgnShortName %>" style="float:right">
				...
				<div class="fl" >
					<% if((projUniq != 'all') && (caseLegend == 1 || caseLegend == 2 || caseLegend == 4)){ %>
                        <span id="showUpdAssign<%= caseAutoId %>" class="clsptr fnt13" style="font-size:11px;cursor:text;" <?php if(defined('ROLE') && ROLE == 1 && array_key_exists('Change Assigned to', $roleAccess) && $roleAccess['Change Assigned to'] == 0){}else{ ?>onclick="displayAssignToMem(<%= '\'' + caseAutoId + '\'' %>, <%= '\'' + projUniq + '\'' %>,<%= '\'' + caseAssgnUid + '\'' %>,<%= '\'' + caseUniqId + '\'' %>)"<?php } ?>><%= getdata.Easycase.asgnShortName %></span>
					<% } else { %>
            ...
            <?php } ?>
				...
				<ul class="dropdown-menu sett_dropdown-caret">
					...
					<% } %>
                            ...
                            <div class="fr">
                        <div class="fr">
				 <div class=" wip view_icon"><a title ="Latest status" href="javascript:void(0);" onclick="statusReport(<%= '\'' + projId + '\'' %>, <%= '\'' + caseNo + '\'' %>,<%= '\'' + caseAutoId + '\'' %>,<%= '\'' + caseUniqId + '\'' %>);">View</a></div>
			</div>
                             </div>
                        <div class="cb"></div>
	...
	<% if(!morecontent){%>
	<div id="loader_<%= taskallkey.replace(/\s/g,'') %>" style="text-align:center;font-size:12px;display:none;"><img src="<?php echo HTTP_ROOT;?>img/images/del.gif" alt="<?php echo __("loading");?>..." title="<?php echo __("loading");?>..."/><br/><?php echo __("Loading"); ?>...</div>
	</div>
<% }} %>
<?php if(defined('CR') && CR == 1 && SES_CLIENT ==1 && $this->Format->get_client_permission('task')==1){ 
    /**Not Show create task */
