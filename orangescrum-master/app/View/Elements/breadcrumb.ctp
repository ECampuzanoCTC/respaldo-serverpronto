...
<?php if(CONTROLLER == "projects" && (PAGE_NAME == "manage")) { ?>
    ...
    <li class="kanbn dashborad-view-type" id="select_view">
        <a href="<?php echo HTTP_ROOT.'projects/manage';?>"><div id="cview_btn" class="btn gry_btn kan30" title="<?php echo __('Card View'); ?>"><!--<img src="<?php echo HTTP_ROOT; ?>img/prj_icon.png" style="width:20px;margin-top:-8px;margin-left:-2px" />-->
		<span class="cmn-ednew-sprite proj"></span></div></a>
    ...
    <?php //} ?>
<?php } ?>
<?php if(CONTROLLER == "projects" && (PAGE_NAME == "details")) { ?>
	<li><?php echo __("Projects"); ?></li>
	<li><?php echo __("Details"); ?></li>
<?php } ?>
<?php if(CONTROLLER == "users" && (PAGE_NAME == "manage")) { ?>
	...
	<li><?php echo __("Manage"); ?></li>
        <?php if(defined('DBRD') && DBRD == 1 ){ ?>
        <li class="kanbn dashborad-view-type" id="select_view">
            <a href="<?php echo HTTP_ROOT.'users/manage';?>"><div id="cview_btn" class="btn gry_btn kan30" title="<?php echo __('Card View'); ?>"><!--<img src="<?php echo HTTP_ROOT; ?>img/prj_icon.png" style="width:20px;margin-top:-8px;margin-left:-2px" />-->
			<span class="cmn-ednew-sprite proj"></span>
			</div></a>
            <a href="<?php echo HTTP_ROOT.'users/manage/list-view';?>"><div id="lview_btn" class="btn gry_btn kan30" style="border-radius:0 3px 3px 0"  title="<?php echo __("List View"); ?>"><i class="icon-list-view"></i></div></a>
        </li>
    <?php } ?>
<?php } ?>
...
<?php } ?>
    ...
    <li><?php echo __("User Role Management"); ?></li>
<?php } ?>

    <?php if (CONTROLLER == "Expense" && PAGE_NAME == "expensebudget") { ?>
    <li class="under-icon"><?php echo __('Expense'); ?></li>
<?php } ?>

<?php if (CONTROLLER == "Wiki" && PAGE_NAME == "wikidetails") { ?>
    <li class="under-icon"><?php echo __('Wiki'); ?></li>
<?php } ?>


<?php if((CONTROLLER == "easycases" && PAGE_NAME == "dashboard") || (CONTROLLER == "milestones" && (PAGE_NAME == "milestone" || PAGE_NAME=='milestonelist')) || (CONTROLLER == "users" && (PAGE_NAME == "activity")) || (CONTROLLER == "LogTimes" && PAGE_NAME == "time_log") || (CONTROLLER == "invoices" && PAGE_NAME == "invoice") || (CONTROLLER == "projects" && PAGE_NAME == "task-status") || (CONTROLLER == "Ganttchart") || (CONTROLLER == "Expense") || (CONTROLLER == "Wiki")) {?>
	...
	<?php } else {
			if(count($getallproj)=='1'){
			    ...
			    $swPrjVal = $this->Format->shortLength($projName,20); 
                            $project_name = $swPrjVal != "" ?$this->Format->mb_ucfirst($swPrjVal):$this->Format->mb_ucfirst($getallproj['0']['Project']['name']);  ?>
			<a href="javascript:void(0);" onclick="view_project_menu('<?php echo PAGE_NAME;?>');" data-toggle="dropdown" class="option-toggle" id="prj_ahref">
			    <div class="prjnm_ttc"><span id="pname_dashboard" class="ttc "><?php echo $project_name; ?></span></div>
			    <i class="caret"></i>
			</a>
            <?php } ?>
	...
	<?php } ?>
        <?php if(CONTROLLER == "easycases" && PAGE_NAME == "mydashboard"){ ?>
            <li class="exec_dash_icon"><span style="display:inline-block;margin-right: 5px" class="dash_icon"><img src="<?php echo HTTP_ROOT?>img/printer-img.png" onclick="PrintDiv_Dash()" title="Print the page" style="cursor:pointer"></span>
            <span class="dash_icon" style="display:inline-block;margin-right: 5px"><img src="<?php echo HTTP_ROOT?>img/help_icon.png" onclick="showHelpContent()" title="Help Content" style="cursor:pointer"></span>
            </li>
...
<?php if(CONTROLLER == "reports" && (PAGE_NAME == "glide_chart" || PAGE_NAME == "chart" || PAGE_NAME == "hours_report")) { ?>
	...
	<?php if(defined('ROLE') && ROLE == 1 && array_key_exists('View Kanban', $roleAccess) && $roleAccess['View Kanban'] == 0){}else{ ?>
	...
	<?php if(defined('ROLE') && ROLE == 1 && array_key_exists('View Calendar', $roleAccess) && $roleAccess['View Calendar'] == 0){}else{ ?>
	...
	<?php if($this->params['plugin'] == 'Timelog' && $this->params['controller'] == 'LogTimes' && $this->params['action']  == 'time_log'){?>
		...
		<div class="fl">
			<input type="text" class="smal_txt" placeholder="From Date" readonly  style="width:115px;height:30px;font-size:13px !important;background:#FFFF99;" id="logstrtdt" value="<?php echo $frm; ?>"/> <span>-</span>
			<input type="text" class="smal_txt" placeholder="To Date" readonly style="width:115px;height:30px;font-size:13px !important;background:#FFFF99;" id="logenddt" value="<?php echo $to; ?>"/>
		</div>
