...
#status_toggle table.tsk_tbl th{padding:10px;border-bottom:2px solid #ddd}
#status_toggle table.tsk_tbl td{padding:14px;padding:10px;}
.file_upload #holder_detl {padding: 14px;height:90px;}
</style>
<% } %>
<div id="t_<%= csUniqId %>" class="task_detail">
	<div class="page-wrapper">
		...
		<div class="row">
				 ...
				 <div class="col-lg-4 task_elements">
					<table cellpadding="0" cellspacing="0">
						<tr>
							...
							<td>
                                <div id="typdiv<%= csAtId %>" class="fl typ_actions <% if(showQuickAct==1){ %> <?php if(defined('ROLE') && ROLE == 1 && array_key_exists('Change Other Details of Task', $roleAccess) && $roleAccess['Change Other Details of Task'] == 0){}else{ ?>dropdown<?php } ?><% } %>">
						...
						<tr>
							...
							<td>
                                <div id="pridiv<%= csAtId %>" data-priority ="<%= csPriRep %>" class="pri_actions <%= protyCls %><% if(showQuickAct==1){ %> <?php if(defined('ROLE') && ROLE == 1 && array_key_exists('Change Other Details of Task', $roleAccess) && $roleAccess['Change Other Details of Task'] == 0){}else{ ?>dropdown<?php } ?><% } %>">
                            ...
                            <% } else { %>
                                <td>
                                    ...
                                    <div id="stsdiv_<%= csAtId %>" data-priority ="<%= csLgndRep %>" class=" dropdown">
                                                                        <span style="font-weight:bold;color:<%= csLgndcolor %>"  class="quick_action" <?php if(defined('ROLE') && ROLE == 1 && array_key_exists('Change Status of Task', $roleAccess) && $roleAccess['Change Status of Task'] == 0){}else{ ?>data-toggle="dropdown" <?php } ?> ><%= csLgndName %></span>
					...
					<table cellpadding="0" cellspacing="0">
						<tr>
							...
							<% if(csLgndRep != 3 && csLgndRep != 5 && taskTyp.id !== "10"){ %>
                                ...
                                <?php if(defined('ROLE') && ROLE == 1 && array_key_exists('Change Other Details of Task', $roleAccess) && $roleAccess['Change Other Details of Task'] == 0){}else{ ?>
		  ...
		  <div class="col-lg-12">
                      <span class="fr ">
                          <a href="javascript:void(0);" onclick="statusCollapse(this)" >Collapse <span class="rep_st_icn"></span></a>
                      </span>
                      <h4 class="wrapword">Status Report &nbsp;</h4>
                            <div id="status_toggle" class="details_task_block" style="height: 400px;max-height: 260px;overflow-y: auto;padding: 10px;
border-bottom: 1px solid #f0f0f0;">
                            <table width="100%" class="tsk_tbl compactview_tbl">
                                ...
                                for(var statusKey in Status){
                                    var getdata = Status[statusKey];
                                %>
                                <tr>
                                    <td><%= i %></td>
                                    <td><%= getdata.status_name %></td>
                                    <td><%= getdata.actual_dt_created %></td>
                                    <td><%= getdata.last_dt_updated %></td>
                                    <td><%= getdata.duration %></td>
		...
		<div class="reply_task_block" id="reply_box<%= csAtId %>">
			...
			<div class="fl col-lg-10">
				...
				<div class="col-lg-12 m-marg-20">
					...
					<div class="col-lg-6 fl">
						...
						<span id="hiddrpdwnstatus<%= csAtId %>">
                                                <select class="select form-control fl" style="width:170px;" <?php if(defined('ROLE') && ROLE == 1 && array_key_exists('Change Status of Task', $roleAccess) && $roleAccess['Change Status of Task'] == 0){ echo 'disabled'; }?> onchange="valforlegend(this.value,'legend<%= csAtId %>')" >
					...
					<div class="col-lg-6 fl">
						...
						<select name="data[Easycase][assign_to]" id="CS_assign_to<%= csAtId %>" class="form-control fl" style="width:170px" onchange="select_reply_user(<%= '\''+csAtId+'\'' %>,this);" <?php if(defined('ROLE') && ROLE == 1 && array_key_exists('Change Assigned to', $roleAccess) && $roleAccess['Change Assigned to'] == 0){echo "disabled" ; } ?> >
						<% if(countJS(allMems)) {
				...
				<div class="col-lg-12 fl tskmore" id="tskmore_<%= csAtId %>" style="display:block">
					...
					<div class="col-lg-6 fl">
						<div class="fl lbl-font16" style="margin:10px 16px 0 0"><?php echo __("Completed"); ?>:</div>
					...
					<form class="upload<%= csAtId %> attch_form" id="file_upload<%= csAtId %>" action="<?php echo HTTP_ROOT; ?>easycases/fileupload/" method="POST" enctype="multipart/form-data">
						<div style="margin-top:10px">
							...
							<div class="lbl-font16 attch_ipad"><?php echo __("Attachment(s)"); ?>:</div>
							<div id="holder_detl">
	...
	<div class="col-lg-3 fl col_task case_det_rt">
		...
		<% } %>
        <?php if(defined('ROLE') && ROLE == 1 && array_key_exists('Download Task', $roleAccess) && $roleAccess['Download Task'] == 0){}else{ ?>
		...
		<div>
			<div class="asign_block">
				...
				<div id="case_dtls_asgn<%= csAtId %>" class="fl asgn_actions <% if(showQuickAct==1){ %> dropdown<% } %>">
                    <span <% if(showQuickAct==1){ %> class="quick_action" <% } %> <?php if(defined('ROLE') && ROLE == 1 && array_key_exists('Change Assigned to', $roleAccess) && $roleAccess['Change Assigned to'] == 0){}else{ ?> data-toggle="dropdown" onclick="displayAssignToMem(<%= '\'' + csAtId + '\'' %>, <%= '\'' + projUniqId + '\'' %>,<%= '\'' + asgnUid + '\'' %>,<%= '\'' + csUniqId + '\'' %>,<%= '\'details\'' %>,<%= '\'' + csNoRep + '\'' %>)"<?php } ?>><?php echo __("Assigned To"); ?></span>
					<% if(showQuickAct==1){ %>
		...
		<div class="task_due_dt">
			...
			<div id="case_dtls_start<%= csAtId %>" class="fl due_actions <% if(showQuickActDD==1){ %> dropdown<% } %>">
					<% if(csStartDtFmt) { %>
                    <div title="<%= csStartDtFmtT %>" rel="tooltip" <% if(showQuickActDD==1){ %> <?php if(defined('ROLE') && ROLE == 1 && array_key_exists('Change Other Details of Task', $roleAccess) && $roleAccess['Change Other Details of Task'] == 0){}else{ ?>class="fl duequick_action stdudtfont" data-toggle="dropdown"<?php } ?><% } else { %> class="fl stdudtfont" <% } %>><%= csStartDtFmt %></div>
					...
					<ul class="dropdown-menu quick_menu">
						 	<li class="pop_arrow_new"></li>
                                                    <?php /*<li><a href="javascript:void(0);" onclick="detChangeStartDate(<%= '\'' + csAtId + '\', \'00/00/0000\', \'No Start Date\', \'' + csUniqId + '\', \'' + csNoRep + '\'' %>)"><?php echo __('No Start Date');?></a></li>
							 ...
							 */ ?><li>
								<a href="javascript:void(0);" class="cstm-dt-option" id="cust_start_date" data-csatid="<%= csAtId %>">
		...
		<div class="task_due_dt">
			...
			<div id="case_dtls_due<%= csAtId %>" class="fl due_actions <% if(showQuickActDD==1){ %> dropdown<% } %>">
				...
				<ul class="dropdown-menu quick_menu">
					<li class="pop_arrow_new"></li>
				<?php /*	<li><a href="javascript:void(0);" onclick="detChangeDueDate(<%= '\'' + csAtId + '\', \'00/00/0000\', \'No Due Date\', \'' + csUniqId + '\', \'' + csNoRep + '\'' %>)"><?php echo __('No Due Date');?></a></li>
					...
					*/ ?><li>
						<a href="javascript:void(0);" class="cstm-dt-option" id="cust_due_date" data-csatid="<%= csAtId %>">
