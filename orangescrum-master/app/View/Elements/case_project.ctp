...
.tsk_sts{position:relative}
.tsk_sts div:first-child{margin-left: 20px;}
.tsk_sts .wip.view_icon a{position: absolute;
left: 0;
width: 20px;
height: 14px;
margin: 0;
background:url("<?php echo HTTP_ROOT; ?>img/view-aye.png") no-repeat !important;
...
vertical-align: top;
cursor:pointer;}
.tsk_sts .wip.view_icon a:hover{background-position: 0px 0px !important;}
.tsk_tbl{table-layout:fixed}
#caseViewSpan table.tsk_tbl tr.tab_tr td.task_wd {
    width: 42%;
}
#caseViewSpan table.tsk_tbl tr.tab_tr td.assign_wd_td {
    width: 16%;
}
</style>
<?php ?>
<table width="100%" class="tsk_tbl compactview_tbl">
    <tr style="" class="tab_tr">
        <td style="width:19%" class="all_td">
            <div class="dropdown fl">
                <input type="checkbox" class="fl chkAllTsk" id="chkAllTsk" />
                <div class="all_chk"></div>
                <ul class="dropdown-menu" id="dropdown_menu_chk">
                    <?php if(defined('ROLE') && ROLE == 1 && array_key_exists('Change Status of Task', $roleAccess) && $roleAccess['Change Status of Task'] == 0){}else{ ?>
                    ...
                    <?php if(defined('ROLE') && ROLE == 1 && array_key_exists('Move to Project', $roleAccess) && $roleAccess['Move to Project'] == 0){}else{ ?>
       ...
       <% if(GrpBy != "") { %> </a> <% } %>
        ...
        <td class="assign_wd_td">
             <% if(GrpBy == "") { %>
             <a class="sortcaseAt" href="javascript:void(0);" title="<?php echo __('Milestone');?>" onclick="ajaxSorting(<%= '\'milestone\', ' + caseCount + ', this' %>);" > <% } %>
                <div class="fl"><?php echo __('Milestone');?></div><div class="<% if(GrpBy == "") { %> tsk_sort <% } %> fl <% if(typeof csAtSrt != 'undefined' && csAtSrt != "") { %>tsk_<%= csAtSrt %><% } %>"></div>
         <% if(GrpBy != "") { %> </a> <% } %>
        </td>
        <td class="assign_wd_td">
             <% if(GrpBy == "") { %>
	...
	for(var caseKey in caseAll){
		...
		var casemilestone = getdata.Milestone.mname==null?_('No Milestone',true):getdata.Milestone.mname;
		...
		if(projUniq=='all') { %>
    <tr>
        <td colspan="6" align="left" class="tkt_pjname"><div class="<% if(count!=1) {%>y_day<% } %>"><%= getdata.Easycase.pjname %></div></td>
    </tr>
    <% 		}
		if(groupby && groupby!='date'){
			if(groupby=='status' && (headerlegend != prvGrpvalue)){%>
    <tr><td colspan="6" align="left" class="curr_day">
    ...
    <tr><td colspan="6" align="left" class="curr_day"><%= easycase.getColorPriority(getdata.Easycase.priority) %></td></tr>
    <%	prvGrpvalue = getdata.Easycase.priority;
			}else if(groupby=='due_date' && (getdata.Easycase.csDueDate !=prvGrpvalue)){%>
    <tr><td colspan="6" align="left" class="curr_day"><%= getdata.Easycase.csDueDate %></td></tr>
    <%	prvGrpvalue= getdata.Easycase.csDueDate;
			}else if(groupby=='assignto' && (getdata.Easycase.assign_to !=prvGrpvalue)){%>
    <tr><td colspan="6" align="left" class="curr_day"><%= getdata.Easycase.asgnShortName %></td></tr>
    <%	prvGrpvalue= getdata.Easycase.assign_to;
			}
		}else{
			if(getdata.Easycase.newActuldt && getdata.Easycase.newActuldt!=0) {%>
    <tr>
        <td colspan="6" align="left" class="curr_day"><div class="<% if(count!=1 && !getdata.Easycase.pjname) {%>y_day<% } %>"><%= getdata.Easycase.newActuldt %></div></td>
    </tr>
    <%	}}
		var bgcol = "#F2F2F2";
    ...
    <tr class="tr_all" id="curRow<%= caseAutoId %>">
        ...
        <td <% if(groupby =='' || groupby !='priority'){%>class="pr_<%= easycase.getPriority(casePriority) %>"<% } %> valign="top">
		    ...
		    <% } %>
                    <% }
					if(getdata.Easycase.isactive == 1){  %>
                    <?php if(defined('ROLE') && ROLE == 1 && array_key_exists('Move to Milestone', $roleAccess) && $roleAccess['Move to Milestone'] == 0){}else{ ?>
                    ...
                    <li onclick="removeTask(<%= '\'' + caseAutoId + '\'' %>, <%= '\'' + caseNo + '\'' %>, <%= '\'\'' %>,<%= '\'' + projId + '\'' %>);" id="moveTask<%= caseAutoId %>" style=" <% if(caseFlag == 2){ %> display:block <% } else { %> display:none <% } %>">
                        <a href="javascript:void(0);"><div class="act_icon ctab_sprt_icon task_remove_mlst fl" title="<?php echo __("Move Task To Milestone"); ?>"></div><?php echo __("Remove from"); ?> <br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo __("Milestone"); ?></a>
                    </li>
                    <% } %>
                    <% if(getdata.Easycase.isactive == 1){
					...
					if(getdata.Easycase.isactive == 1){ %>
                    <?php if(defined('ROLE') && ROLE == 1 && array_key_exists('Archive Task', $roleAccess) && $roleAccess['Archive Task'] == 0){}else{ ?>
					...
					if(getdata.Easycase.isactive == 1){ %>
                    <?php if(defined('ROLE') && ROLE == 1 && array_key_exists('Delete Task', $roleAccess) && $roleAccess['Delete Task'] == 0){}else{ ?>
            ...
            <div class="dropdown fl" style="width:32px;">
                <div id="showUpdStatus<%= caseAutoId %>" class="type_<%= getdata.Easycase.csTdTyp[0] %> <% if(getdata.Easycase.isactive == 1){ %>clsptr<% } %> <% if($.inArray(getdata.Easycase.csTdTyp[0], ['dev', 'bug', 'upd']) == -1) { %>opcty4<% } %>" title="<%= _(getdata.Easycase.csTdTyp[1]) %>" <?php if(defined('ROLE') && ROLE == 1 && array_key_exists('Change Other Details of Task', $roleAccess) && $roleAccess['Change Other Details of Task'] == 0){}else{ ?>data-toggle="dropdown"<?php } ?>><% if(getdata.Easycase.csTdTyp[2]==0){ %><span style="margin-left: 9px;display: inline-block;overflow: hidden;text-overflow: ellipsis; white-space: nowrap;width: 40px;"><%= _(getdata.Easycase.csTdTyp[0])%></span><% } %></div>
            ...
            <%= easycase.getStatus(getdata.Easycase.type_id, getdata.Easycase.legend) %>
            <% } %>
            <div class=" wip view_icon"><a title ="Latest status" href="javascript:void(0);" onclick="statusReport(<%= '\'' + projId + '\'' %>, <%= '\'' + caseNo + '\'' %>,<%= '\'' + caseAutoId + '\'' %>,<%= '\'' + caseUniqId + '\'' %>);"></a></div>
            </div>
            <% } %>
        </td>
        ...
        <td class="title_det_wd">
            <div class="fl title_wd">
                ...
                <% if(RCT == 1 && (getdata.Easycase.is_recurring == 1 || getdata.Easycase.is_recurring == 2)){ %>
            ...
            <div class="fnt999 fl">
                ...
                <div class="fl" style="<% if(!getTotRep || getTotRep==0) { %>display:none<% } %>">
                    <div id="repno<%= count %>" data-task="<%= caseUniqId %>" class="fl bblecnt" title="Last 5 comments"></div>
					(<% if(getTotRep && getTotRep!=0) { %><%= getTotRep %><% } %>)
                </div>
            </div>
            <div class="cb"></div>
        </td>
        <td valign="top" style="padding-right:20px;text-align:left"><%= casemilestone %></td>
        ...
        <td valign="top">
            <div class="dropdown fl">
                <% if((projUniq != 'all')){ %>
                <span id="showUpdAssign<%= caseAutoId %>" title="edit Assign to" class="clsptr" <% if(getdata.Easycase.isactive == 1 && (getdata.Easycase.legend!=3 && getdata.Easycase.legend!=5) && getdata.Easycase.legend != custom_maxlgnd){ %> data-toggle="dropdown" <?php if(defined('ROLE') && ROLE == 1 && array_key_exists('Change Assigned to', $roleAccess) && $roleAccess['Change Assigned to'] == 0){}else{ ?>onclick="displayAssignToMem(<%= '\'' + caseAutoId + '\'' %>, <%= '\'' + projUniq + '\'' %>,<%= '\'' + caseAssgnUid + '\'' %>,<%= '\'' + caseUniqId + '\'' %>)" <?php } ?> <% } %>><%= getdata.Easycase.asgnShortName %><schckmaxlegendpan class="due_dt_icn"></span></span>
                <% } else { %>
        ...
        <td class="fnt12" valign="top">
            <div class="dropdown fl">
                <% 
                if(getdata.Easycase.isactive == 1 && (getdata.Easycase.legend!=3 && getdata.Easycase.legend!=5) && getdata.Easycase.legend != custom_maxlgnd){ %>
                <div class="fl" <% if(getdata.Easycase.isactive == 1 && (getdata.Easycase.legend!=3 && getdata.Easycase.legend!=5) && getdata.Easycase.legend != custom_maxlgnd){ %> <?php if(defined('ROLE') && ROLE == 1 && array_key_exists('Change Other Details of Task', $roleAccess) && $roleAccess['Change Other Details of Task'] == 0){}else{ ?>data-toggle="dropdown" original-title="edit Due Date" style="cursor:pointer"<?php } ?><% } %>>
                ...
                <ul class="dropdown-menu dudt_dropdown-caret">
                   ...
                   <?php /* <li><a href="javascript:void(0);" onclick="changeCaseDuedate(<%= '\'' + caseAutoId + '\', \'' + caseNo + '\'' %>); changeDueDate(<%= '\'' + caseAutoId + '\', \'' + mdyCurCrtd + '\', \'Today\', \'' + caseUniqId + '\'' %>)"><?php echo __("Today"); ?></a></li>
    ...
    <tr>
        <td colspan="6" align="center" style="padding:10px 0;color:#FF0000">

            <% if(case_type == 'cases' || case_type == '' || !isNaN(case_type)){
				if(filterenabled){%>
					<?php echo __("No Task"); ?>
            <% }else{ %>
...
<?php if(defined('CR') && CR == 1 && SES_CLIENT ==1 && $this->Format->get_client_permission('task')==1){ 
    ...
    if(defined('ROLE') && ROLE == 1 && array_key_exists('Create Task', $roleAccess) && $roleAccess['Create Task'] == 0){}else{?> 
