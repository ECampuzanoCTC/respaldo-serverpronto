...
<li>
	...
	<font onclick="checkboxdueDate(<%= '\''+duedateArr[i]+'\',\'text\'' %>);filterRequest(<%= '\'duedate\'' %>);" >&nbsp;<%= _(i) %></font>
    </a>
</li>
<% } %>
<% var dt = '';
...
<div id="custom_duedate" <% if(!dt){%>style="display:none;"<% } %>>
	<div  class="cdate_div_cls">
		<input type="text" id="duefrm"  value="<%= dt[0] %>" placeholder="<?php echo __('From'); ?>" class="form-control"/><br/>
