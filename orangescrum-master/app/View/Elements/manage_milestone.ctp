...
if(caseCount && caseCount != 0){
	...
	for(var caseKey in milestoneAll){
        ...
        if(max_sts_names.length !=0){
            for(var kys in max_sts_names){
                if(getdata.Milestone.project_id == kys)
                {
                    lgnd_name = max_sts_names[kys];
                }
            }
        }
         if(prjct_wrkflw.length !=0){
            for(var ky in prjct_wrkflw){
                if(getdata.Milestone.project_id == ky)
                {
                    wrkflw_id = prjct_wrkflw[ky];
	...
	%>
		<div class="col-lg-4 proj_mng_div">
			<div class="col-lg-12 contain usr_mng_div milestone_div">
                ...
                <div class="imprv_bar col-lg-12" rel="tooltip" title="<%= closed_cases %> <?php echo __('out of'); ?> <%= total_cases %> <?php echo __('task are closed'); ?>">
                ...
                <div class="user-details prj_details">
                    ...
                    <% if(wrkflw_id > 0) {%>
                    <div><b><%= closed_cases %></b> <%= _(lgnd_name) %> </div>
                    <% } else { %>
                    <div><b><%= closed_cases %></b> <%= _(lgnd_name) %> </div>
                    <div><b><%= resolved_cases %></b> <?php echo __('Resolved');?></div> 
                    <% } %>
                    </div>
                    <div class="cb"></div>
                ...
                <div class="last_updt prj_last_updt"><?php echo __('Last activity on');?> <%= locDT %></div>
...
-->			
 <?php if(defined('CR') && CR == 1 && SES_CLIENT ==1 && $this->Format->get_client_permission('milestone')==1){ 
                ...
                <div class="proj_mng">
				    <div class="fl">
                                        <% if(mlsttype!='0'){ %>
					<a href="javascript:void(0);" class="icon-add-task fl" onClick="addTaskToMilestone(this);" data-prj-id="<%= projId %>" data-id="<%= milestoneAutoId %>"><?php echo __('Add Task');?></a>
					<br /> <% } %>
					<% if(mlsttype=='0'){ %>
						<a href="javascript:void(0);" class="icon-restore-mlstn fl ellipsis-view" title="<?php echo __('Restore');?>" onClick="milestoneRestore(this);" data-uid="<%= milestoneUniqId %>" data-name="<%= milestoneTitle %>"><?php echo __('Restore');?></a>
					<% }else{ %>
						<a href="javascript:void(0);" class="icon-complete-mlstn fl ellipsis-view" title="<?php echo __('Complete');?>" onClick="milestoneArchive(this);" data-uid="<%= milestoneUniqId %>" data-name="<%= milestoneTitle %>"><?php echo __('Complete');?></a>
					<% } %>
					<% if(total_cases>0 && mlsttype!='0'){ %><br/>
						<a href="javascript:void(0);" class="icon-remov-task fl ellipsis-view" title="<?php echo __('Remove Task');?>" onClick="removeTaskFromMilestone(this);" data-prj-id="<%= projId %>" data-id="<%= milestoneAutoId %>"><?php echo  __('Remove Task');?></a>
					<% } %>
				    </div>
				    <div class="fr">
                        <% if(mlsttype!='0'){ %>
					<a href="javascript:void(0);" class="icon-edit-mlstn fl ellipsis-view" title="<?php echo __('Edit');?>" onClick="addEditMilestone(this);" data-id="<%= milestoneAutoId %>" data-uid="<%= milestoneUniqId %>" data-name="<%= milestoneTitle %>"><?php echo __('Edit');?></a>
					<br /><% } %>
					<a href="javascript:void(0);" class="icon-delete-mlstn fl ellipsis-view" title="<?php echo __('Delete');?>" onClick="delMilestone(this);" data-uid="<%= milestoneUniqId %>" data-name="<%= milestoneTitle %>"><?php echo __('Delete');?></a>
				    </div>
				</div>
                                            <?php } ?>
			</div>
...
else if(caseCount == 0 && file_srch == "" ){ %>
<% } 
else{ %>
<div style="color:#ff0000;text-align:center;padding-top:50px;font-size:20px"><?php echo __("Not Found"); ?> </div>
<% } %>
