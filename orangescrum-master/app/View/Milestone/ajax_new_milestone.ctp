        ...
        <tr>
            ...
            <td <?php if ($milearr['Milestone']['id']) { ?> style="text-align: left;" <?php } ?>>
                <?php if (!$milearr['Milestone']['id']) { ?>
                    <select id="project_id1" name="data[Milestone][project_id]" class="form-control" onchange="fetch_select()">
                        <option value="">--<?php echo __("Select"); ?>--</option>
                        <?php foreach ($projArr as $prjarr) { ?>
                            <option <?php
                            if ($projUid == $prjarr['Project']['uniq_id']) {
                ...
                <?php } else { ?>
                    <input type="hidden" name="data[Milestone][project_id]" id="project_id1" value="<?php echo $milearr['Milestone']['project_id']; ?>" data-pname="<?php echo $projArr[0]['Project']['name']; ?>" data-puniq="<?php echo $projArr[0]['Project']['uniq_id']; ?>">
        ...
        <tr>
            ...
            if (!empty($milearr['Milestone']['start_date']) && $milearr['Milestone']['start_date'] != '0000-00-00') {
                if ($GLOBALS['DateFormat'] == 2) {
                    $milearr['Milestone']['start_date'] = date('d M, Y', strtotime($milearr['Milestone']['start_date']));
                } else {
                    $milearr['Milestone']['start_date'] = date('M d, Y', strtotime($milearr['Milestone']['start_date']));
                }
            ...
            if (!empty($milearr['Milestone']['end_date']) && $milearr['Milestone']['end_date'] != '0000-00-00') {
                if ($GLOBALS['DateFormat'] == 2) {
                    $milearr['Milestone']['end_date'] = date('d M, Y', strtotime($milearr['Milestone']['end_date']));
                } else {
                    $milearr['Milestone']['end_date'] = date('M d, Y', strtotime($milearr['Milestone']['end_date']));
                }
        ...
        <tr>
            ...
            <td>
                <div class="fl">
                    ...
                    <span id="btn_mlstn">
                        <button type="button" value="Update" name="milestone" id="milestone" class="btn btn_blue" onclick="return validatemilestone();"><i class="icon-big-tick"></i><?php
                            if (!empty($edit)) {
                                echo __("Save");
                            } else {
                                echo __("Add");
...
<script>
    ...
    function validatemilestone() {
        ...
        if ($('#id').val()) {
            var proj_uniq = $('#project_id1').attr('data-puniq');
            var proj_name = $('#project_id1').attr('data-pname');
        } else {
            var proj_uniq = $('#project_id1 option[value="' + project_id + '"]').attr('data-puniq');
            var proj_name = $('#project_id1 option[value="' + project_id + '"]').attr('data-pname');
        }
