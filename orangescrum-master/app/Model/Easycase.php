...
class Easycase extends AppModel {
    ...
    function formatCases($caseAll, $caseCount, $caseMenuFilters, $closed_cases, $milestones, $projUniq, $usrDtlsArr, $frmt, $dt, $tz, $cq, $chk = null) {
        ...
        $curdtT = $tz->GetDateTime(SES_TIMEZONE, TZ_GMT, TZ_DST, TZ_CODE, GMT_DATETIME, "datetime");
            ...
            foreach ($caseAll as $caseKey => $getdata) {
                ...
                if (defined('TSG') && TSG != 1 && ($caseLegend == 3 || $caseLegend == 5)) {
                    ...
                    if ($caseDueDate != "NULL" && date('Y-m-d', strtotime($caseDueDate)) != "0000-00-00" && $caseDueDate != "" && date('Y-m-d', strtotime($caseDueDate)) != "1970-01-01" && $caseDueDate != "0000-00-00 00:00:00") {
                        ...
                        $caseDueDate = $tz->GetDateTime(SES_TIMEZONE, TZ_GMT, TZ_DST, TZ_CODE, $caseDueDate, "datetime");
                        $csDuDtFmtT = $dt->facebook_datestyle($caseDueDate);
                        $csDuDtFmt = $dt->dateFormatOutputdateTime_day($caseDueDate, $curCreated, 'time');
                    } else {
                        $csDuDtFmtT = '';
                        $csDuDtFmt = '';
                    }
                    ...
                    if ($caseDueDate != "NULL" && date('Y-m-d', strtotime($caseDueDate)) != "0000-00-00" && $caseDueDate != "" && date('Y-m-d', strtotime($caseDueDate)) != "1970-01-01" && $caseDueDate != "0000-00-00 00:00:00") {
                        ...
                        if ($caseDueDate < $curdtT) {
                            $csDuDtFmtT = $dt->facebook_datestyle($caseDueDate);
                            $days_difference = $dt->datediffernce($caseDueDate);
                            $csDuDtFmt = '<span class="over-due">' . __('Overdue by ', true) . $days_difference . '</span>';
                            $csDueDate = $dt->dateFormatOutputdateTime_day($caseDueDate, $curCreated, 'time');
                        } else {
                            $csDuDtFmtT = $dt->facebook_datestyle($caseDueDate);
                            $csDuDtFmt = $dt->dateFormatOutputdateTime_day($caseDueDate, $curCreated, 'time');
                            $csDueDate = $csDuDtFmt;
                ...
                $caseAll[$caseKey]['Easycase']['title'] = htmlentities($frmt->longstringwrap($getdata['Easycase']['title']), ENT_QUOTES, 'UTF-8');//htmlentities($frmt->convert_ascii($frmt->longstringwrap($getdata['Easycase']['title'])), ENT_QUOTES, 'UTF-8');
        ...
        foreach ($sqlcasedata as $caseKey => $getdata) {
            ...
            foreach ($rplyFilesArr as $fkey => $getFiles) {
                ...
                if (isset($getFiles['CaseFile']['upload_name']) && !empty($getFiles['CaseFile']['upload_name'])) {
                    $caseFileName = $getFiles['CaseFile']['upload_name']; //PRRB
                }
...
//            $milestones = $milestone_cls->find('all',array('conditions'=>array('isactive' =>$isActive,'project_id' =>$curProjId,'company_id' => SES_COMP)));
         ...
         //   echo "SELECT SQL_CALC_FOUND_ROWS `Milestone`.`id`,Milestone.assign_id,User.id,User.name,User.id,'User.name,`Milestone`.`title`,`Milestone`.`project_id`,`Milestone`.`end_date`,`Milestone`.`uniq_id`,`Milestone`.`isactive`,`Milestone`.`user_id`,COUNT(c.easycase_id) AS totalcases,GROUP_CONCAT(c.easycase_id) AS `caseids`  FROM milestones AS `Milestone` LEFT JOIN easycase_milestones AS c ON Milestone.id = c.milestone_id LEFT JOIN projects Project on Project.id=Milestone.project_id  WHERE `Milestone`.`isactive` =" . $isActive . " AND Milestone.project_id IN (SELECT ProjectUser.project_id FROM project_users AS ProjectUser LEFT JOIN projects AS Project ON ProjectUser.project_id=Project.id LEFT JOIN users AS User ON User.id=Milestone.user_id WHERE ProjectUser.user_id=" . SES_ID . " AND Project.isactive='1' AND ProjectUser.company_id='" . SES_COMP . "') AND Milestone.isactive=$isActive AND `Milestone`.`company_id` = " . SES_COMP . "  $milestone_search GROUP BY Milestone.id ORDER BY Milestone.title ASC LIMIT " . $mlimit . ',' . MILESTONE_PER_PAGE ;exit;
...
//$milestones = $milestone_cls->query("SELECT SQL_CALC_FOUND_ROWS `Milestone`.`id`,Milestone.assign_id,User.id,User.name,User.id,'User.name,`Milestone`.`title`,`Milestone`.`project_id`,`Milestone`.`end_date`,`Milestone`.`uniq_id`,`Milestone`.`isactive`,`Milestone`.`user_id`,COUNT(c.easycase_id) AS totalcases,GROUP_CONCAT(c.easycase_id) AS `caseids`  FROM milestones AS `Milestone` LEFT JOIN easycase_milestones AS c ON Milestone.id = c.milestone_id LEFT JOIN projects Project on Project.id=Milestone.project_id  WHERE `Milestone`.`isactive` =" . $isActive . " AND Milestone.project_id IN (SELECT ProjectUser.project_id FROM project_users AS ProjectUser LEFT JOIN projects AS Project ON ProjectUser.project_id=Project.id LEFT JOIN users AS User ON User.id=Milestone.user_id WHERE ProjectUser.user_id=" . SES_ID . " AND Project.isactive='1' AND ProjectUser.company_id='" . SES_COMP . "') AND Milestone.isactive=$isActive AND `Milestone`.`company_id` = " . SES_COMP . "  $milestone_search GROUP BY Milestone.id ORDER BY Milestone.title ASC LIMIT " . $mlimit . ',' . MILESTONE_PER_PAGE);
            if (!$milestones) {
