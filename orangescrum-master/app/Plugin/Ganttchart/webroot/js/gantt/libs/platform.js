...
function nearBestPosition(whereId, theObjId, centerOnEl) {
  ...
  if (el) {
    ...
    if (trueX && trueY) {
      target.css("left", left);      target.css("top", top);
    ...
    //patch to move calendar top of field
    setTimeout(function(){
        if (target.offset().top  + target.outerHeight() >= (($(window).height() - barHeight))){
            target.css("top", top-target.height()-40);
        }
    },1);
  }
}
...
function openBlackPopup(url,width,height,onCloseCallBack,iframeId){
  ...
  if(window.name!=iframeId)
    bg.css({backgroundImage:"url('"+HTTP_ROOT+"ganttchart/img/gantt/res/black_70.png')"});
  ...
  bdiv.append("<img src=\"../../img/gantt/res/closeBig.png\" style='cursor:pointer;position:absolute;right:-40px;top:30px;'>");
...
function createBlackPage(width,height,onCloseCallBack){
  ...
  var bg=$("<div>").attr("id","__blackpopup__");
  bg.css({position:'fixed',top:"0px",paddingTop:"50px", left:0,width:'100%',height:'100%',  backgroundImage:"url('"+HTTP_ROOT+"ganttchart/img/gantt/res/black_70.png')"});
  bg.append("<div id='bwinPopupd' name='bwinPopupd'></div>");
  bg.bringToFront();

  var ret=bg.find("#bwinPopupd");
  ret.css({width:width, height:height,top:10, "-moz-box-shadow":'1px 1px 6px #333333',overflow:'auto',"-webkit-box-shadow":'1px 1px 6px #333333', border:'0px solid #3DBB89', backgroundColor:"#fff", margin:"auto" });

  var bdiv= $("<div>").css({width:width,position:"relative",height:"0px", textAlign:"right", margin:"auto" });
  //var img=$("<img src='../../img/gantt/res/closeBig.png' style='cursor:pointer;position:absolute;right:-40px;top:5px;' title='close'>");
  //bdiv.append(img);
  //img.click( function(){bg.trigger("close");});
  var img = $('.close_popup_btn');
  img.live('click', function(){bg.trigger("close");});
