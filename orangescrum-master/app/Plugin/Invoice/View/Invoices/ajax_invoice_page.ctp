        ...
        <div class="col-sm-7">
            ...
            <div id="preview" style="display: inline-block;" >
                ...
                <?php if(isset($i['Invoice']['logo']) && trim($i['Invoice']['logo']) !='' && $this->Format->pub_file_exists(DIR_INVOICE_PHOTOS_S3_FOLDER. SES_COMP . '/', $i['Invoice']['logo'])) { ?>
                    <img src="<?php echo $this->Format->generateTemporaryURL(DIR_INVOICE_PHOTOS_S3 . SES_COMP . '/'.  $i['Invoice']['logo']);?>" style="max-height:100px;" alt="" />
                    <?php $invoice_logo = $i['Invoice']['logo'];?>
                <?php }elseif(isset($company['Company']['logo']) && trim($company['Company']['logo']) !='' && $this->Format->imageExists(DIR_USER_PHOTOS, $company['Company']['logo'])) { ?>
                    ...
                    <?php $invoice_logo = $company['Company']['logo'];?>
                <?php } else { ?>
            ...
            <div style="display:none;">
                ...
                <input type="hidden" name="logophoto" id="logophoto" value="<?php echo $invoice_logo;?>"/>
            </div>
            <br/>
                        ...
                        <div class="more_opt" id="more_opt1123123" style="display:none;">                            
                            <ul class="" style="display: block;width:270px;  max-height: 250px; overflow-y: auto; overflow-x: hidden;">
                                <li class="last anchor" style="border-bottom:1px solid #ccc;">
                                    <div><a onclick="add_customer();" class="anchor" style="color:#06C; font-size:12px; padding-left:0px;">+ <?php echo __('Add New Customer');?></a></div>
                                </li>
                            ...
                            <?php $c=0; $cntr=0;
                                ...
                                <tr id="row<?php echo $cntr; ?>" class="logrow">
                                    ...
                                    <td>
                                        <div class="invoice-right-data">
                                                <input type="text" class="form-control border-zero" id="edit_total_hours<?php print $cntr; ?>" name="edit_total_hours[<?php print $cntr; ?>]" 
                                                       value="<?php if (!empty($log['LogTime']['total_hours'])) {echo $this->Format->hourInSeconds_to_decimal($log['LogTime']['total_hours']);} ?>"
                            ...
                            <tr id="addMoretr">
                                <td colspan="6" style="border-bottom:0px; padding:0px; margin:0px;">
                                    <div style="position:relative">
                                        <div style="position:absolute; left:15px; top:10px;"><a href="javascript:void(0);" onclick="addLineItem();" class="addLine">+ <?php echo __("Add Line-item"); ?></a></div>
                                    </div>
                                </td>
                            </tr>
                        </table>
                        ...
                        <div class="fr" style="min-width:31%;">
                            ...
                            <div class="amt_final_block amt">
                                <span id="total-discount">
                                    <?php
                                    $grandDiscount = ($i['Invoice']['discount_type'] != 'Flat') ? (($grandTotal * floatval($i['Invoice']['discount'])) / 100) : ( floatval($i['Invoice']['discount']));
                            ...
                            <div class="amt_final_block amt">
                                <span id="total-tax">
                                    ...
                                    $invTax = ($taxable * floatval($i['Invoice']['tax'])) / 100;
                            ...
                            <div class="amt_final_block amt">
                                ...
                                <span id="final_edit_total_hours">
                                    <?php $final_amount = $grandTotal - $grandDiscount + $invTax;
    ...
    <div class="InvoiceDownloadEmail fl" style="display: block;" >        
        <div class=" relative fl" style="left:35%;width:350px;position:relative;">
            <div class="mr20 fl relative inv_bts" style="position:relative;">
                ...
                <button class="btn btn_blue mr0 fl no-border-radius saveInvoice" id="emailInvoice" onclick="saveInvoice('<?php echo $invoice_id;?>','export_email');" title="<?php echo __('Save & Send');?>"><?php echo __('Save & Send');?></button>
                ...
                <ul class="btn_invoice_munu noprint" id="btn_invoice_munu" style="z-index:1;display:none;">
                    <li class="" style="">
                        <a onclick="javascript:saveInvoice('<?php echo $invoice_id;?>','export_pdf');" class="anchor btn btn_blue no-border-radius mr0 saveInvoice" title="<?php echo __('Save & Download');?>"><?php echo __('Save & Download');?></a>
                    </li>
                    <li class="" style="display:list-item;text-align:center;  border-bottom: 1px solid #ccc;">
                        <a onclick="javascript:saveInvoice('<?php echo $invoice_id;?>','close');" class="anchor btn btn_blue no-border-radius mr0 saveInvoice" title="<?php echo __('Save & Close');?>"><?php echo __('Save & Close');?></a>
                    </li>
                    <li class="" style="display:list-item;text-align:center;">
                        <a onclick="javascript:saveInvoice('<?php echo $invoice_id;?>','createnew');" class="anchor btn btn_blue no-border-radius mr0 saveInvoice" title="<?php echo __('Save & New');?>"><?php echo __('Save & New');?></a>
                    </li>
                </ul>
                <div class="cb"></div>
            </div>
            <div class="or_cancel cancel_on_direct_pj" style="padding:5px 0  5px 0;">or <a class="anchor" id="cancelInvoice" onclick="cancelInvoice();" title="<?php echo __('Cancel');?>"><?php echo __('Cancel');?></a></div>

        </div>
        <div class="or_cancel fr" style="padding:10px 10px 0 0;">
            <a onclick="javascript:saveInvoice('<?php echo $invoice_id;?>','preview');" class="anchor  fr" title="<?php echo __('Preview &amp; Print');?>"><?php echo __('Preview &amp; Print');?></a>
        </div>
...
<div class="invoice-date">
    ...
    <?php foreach($activity as $key => $val){?>
        <?php if ($val['InvoiceActivity']['activity'] == 'create') { ?>
            <li>
                <div class="bullet-blk"></div>
                <div class="inv-dt"><?php echo $this->Format->display_activity_log($val);?></div>
                <div><b><?php echo $this->Format->get_date($val['InvoiceActivity']['created'])?></b></div>
            </li>
        <?php } else{ ?>
            <li class=" invo_view inv_lst">
                <div class="inv-vw">&mdash; <?php echo $this->Format->display_activity_log($val);?></div>
                <div class="s-time"><i><?php echo $this->Format->get_date($val['InvoiceActivity']['created'])?></i></div>
...
<script type="text/javascript">
    ...
    function deleteInvoiceTimeLog(v) {
        //alert(v)
        if (confirm("<?php echo __('Are you sure you want to delete this time log ?');?>")) {
    ...
    $(document).ready(function() { 
        ...
        $("#edit_issue_date").datepicker({
            ...
            onClose: function( selectedDate ) {
               invoices.set_due_date(selectedDate);
                $("#edit_due_date").datepicker( "option", "minDate", selectedDate);
            },
        });
