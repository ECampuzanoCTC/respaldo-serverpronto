<div class="timelog-table" style="border:0px none;">
    <div class="timelog-table-head">
            ...
            <div class="fr mr5 relative" style="">
                <?php echo $this->Form->button(__('Create Invoice'), array('type' => 'button', 'class' => 'btn btnaddto invoice-btn fl no-border-radius', 'onclick' => 'addToInvoice();','disabled'=>'disabled')); ?>
    ...
    <div class="timelog-detail-tbl">
        ...
        <table cellpadding="3" cellspacing="4">
            <tr>
                ...
                <th style="width:10%">
                    <a title="<?php echo __('Date');?>" onclick="invoices.ajaxSorting('timelog', 'date', this);" class="anchor">
                ...
                <th style="width:10%">
                    <a title="<?php echo __('Resource');?>" onclick="invoices.ajaxSorting('timelog', 'resource', this);" class="anchor">
                ...
                <th style="width:15%">
                    <a title="<?php echo __('Task');?>" onclick="invoices.ajaxSorting('timelog', 'task', this);" class="anchor">
                ...
                <th style="width:10%">
                    <a title="<?php echo __('Type');?>" onclick="invoices.ajaxSorting('timelog', 'type', this);" class="anchor">
            ...
            <?php if (!empty($logtimes)) { ?>
                <?php foreach ($logtimes as $log) { ?>
                    <tr>
                        ...
                        <?php $log['Easycase']['title'] = $this->Format->formatTitle($log['Easycase']['title']);?>
                        <td <?php if(!empty($log['Easycase']['title'])){ print 'rel="tooltip"'; }?> title="<?php echo $log['Easycase']['title'];?>">
                            <div class="ellipsis-view" style="width:125px;" ><?php echo $log['Easycase']['title']; ?></div>
                        </td>
                        <?php $log['Easycase']['title'] = $this->Format->formatTitle($log['Easycase']['title']);?>
                        <td title="<?php echo $log['LogTime']['description']; ?>" <?php if(!empty($log['LogTime']['description'])){ ?> rel="tooltip" <?php } ?> >
                           <div class="ellipsis-view" style="width:145px;" > <?php echo $log['LogTime']['description']; ?></div>
                        </td>
                        <td><?php echo __($log['Type']['name'],true); ?></td>
            ...
            <div class="fr mr5 relative">
                <?php echo $this->Form->button(__('Create Invoice'), array('type' => 'button', 'class' => 'btn btnaddto invoice-btn fl no-border-radius', 'onclick' => 'addToInvoice();','disabled'=>'disabled')); ?>
                <span class="invoice-extra-block second_caret">
                    <button class="btn toggle-invoice-opts fl no-border-radius"><i class="caret"></i></button>
                    <ul class="dropdown-menu crt-invoice-menu noprint" id="second_caret">
                        <li class="" style="display:list-item;text-align:center;">
                            <a onclick="javascript:showInvoicePage('','New Invoice','new');" class="anchor create-invoice-without-unbilled-time"><?php echo __('Create Invoice without Unbilled Time'); ?></a>
                        </li>
                    </ul>
