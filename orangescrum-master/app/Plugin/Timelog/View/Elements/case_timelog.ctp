...
<style type="text/css">
    ...
    .timelog-detail-tbl tr {border-left: 1px solid #cccccc;border-right: 1px solid #cccccc;}
</style>
    ...
    <div class="timelog-table-head">
        <div>
            ...
            <div class="fr tl-msg-btn">
                ...
                <a href="javascript:void(0);" class="fl btn btn_blue aply_btn" id="btn-reset-timelog" style="margin: 0px 10px;"><?php echo __("Reset"); ?></a>
                <% } %>
                <div class="logmore-btn fr" style="<% if(SHOWTIMELOG=='No'){ %>display:none;<% } %>">
                    <% if (typeof csLgndRep != 'undefined' && (csLgndRep!=3 && csLgndRep!=5)) { %>
                    <?php
                    if (defined('ROLE') && ROLE == 1 && array_key_exists('Start Timer', $roleAccess) && $roleAccess['Start Timer'] == 0) {
                        
                    } else {
                        ?> 
                        <a class="<%=logtimes.page%> anchor log-more-time" onclick="startTimer(<%= '\'' + logtimes.task_id + '\'' %>,<%= '\'' + escape(logtimes.task_title) + '\'' %>,<%= '\'' + logtimes.task_uniqId + '\'' %>,<%= '\'' + logtimes.project_uniqId + '\'' %>,<%= '\'' + logtimes.project_name + '\'' %>)"><?php echo __("Start Timer"); ?></a>
                    <?php } ?>
                    <% } %>
                    <?php
                    if (defined('ROLE') && ROLE == 1 && array_key_exists('Manual Time Entry', $roleAccess) && $roleAccess['Manual Time Entry'] == 0) {
                        ...
                        ?> 
                        <a class="<%=logtimes.page%> anchor" onclick="createlog(<%= logtimes.task_id %>,'<%= escape(logtimes.task_title)%>',0,<%= '\'' + logtimes.project_uniqId + '\'' %>)"><?php echo __("Log more time"); ?><span class="sprite btn-clock"></span></a>
                    <?php } ?>		</div>
                <div class="showreplylog ht_log" style="<% if(SHOWTIMELOG!='No'){ %>display:none;<% } %>margin-left:35px;margin-top:0px;">
                    <a href="javascript:void(0);" style="font-size:13px;text-decoration:none;" onclick="showreplytimelog();">
                        <span class="fl">Expand Time Log</span>
                    </a>
                </div>
            </div>
            <div class="cb"></div>
...
<div class="hidetablelog" style="<% if(SHOWTIMELOG=='No'){ %>display:none;<% } %>">
    <div class="timelog-detail-tbl">
        <table cellpadding="3" cellspacing="4">
            ...
            if (DATEFORMAT == 2) {
            var start_time= getdata.LogTime.start_time;
            var end_time = getdata.LogTime.end_time;
            } else {
            var start_time=format_24hr_to_12hr(getdata.LogTime.start_time);
            var  end_time = format_24hr_to_12hr(getdata.LogTime.end_time);
            } 

            %>
            <tr class="timelog-hover-block">
                <?php /* <td><%= formatDate('M dd, yy',getdata.LogTime.start_datetime) %></td> */ ?>
                <td><%= formatDate(date_format,getdata[0].start_datetime_v1) %></td>
                ...
                <% } %>
                <td><%= shortLength(formatText(nl2br(getdata.LogTime.description)),20) %></td>
                ...
                <td class="relative">
                    ...
                    <div class="timelog-opts edtdltlog" data-logid="<%= getdata.LogTime.log_id %>">
                        ...
                        <% } %>
                        <a class="anchor edit_time_log" <?php
                        if (defined('ROLE') && ROLE == 1) {
                            if (array_key_exists('Edit Timelog Entry', $roleAccess) && $roleAccess['Edit Timelog Entry'] == 1) {
                                ?> onclick="editTimelog(this)" <?php
                               }
                           } else {
                               ?> onclick="editTimelog(this)" <?php } ?> ><span class="fl sprite note"></span></a>
                        <a class="anchor delete_time_log" <?php
                        if (defined('ROLE') && ROLE == 1) {
                            if (array_key_exists('Delete Timelog Entry', $roleAccess) && $roleAccess['Delete Timelog Entry'] == 1) {
                                ?> onclick="deletetimelog(this);" <?php
                               }
                           } else {
                               ?> onclick="deletetimelog(this);" <?php } ?>><span class="fl sprite delete"></span></a>
                    </div>
                    <% } %>
                </td>
                <% if(typeof logtimes.page != "undefined" && logtimes.page == 'timelog'){ %>
            ...
            <% }else{ %>
            <tr>
                <td colspan="8"><?php echo __("No records"); ?>......</td>
            </tr>
    ...
    <% if(typeof logtimes.page != "undefined" && logtimes.page == 'timelog'){ %>
    <% } else{ %>
    <div class="timelog-table-head" style="display:none;margin: 0px;border: 1px solid #ccc;border-top-width: 0px;">
    ...
    <div style="border-bottom:1px solid #ccc;">
        <div class="fr ht_log">
            <a href="javascript:void(0);" onclick="hidereplytimelog();"><span class="fl"><?php echo __("Hide Time Log"); ?></span><span class="fl sprite up-btn"></span></a>
        </div>
        <div class="cb"></div>
    </div>
    <% } %>
