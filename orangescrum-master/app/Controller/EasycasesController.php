...
#### DELETING CASES CREATED BY LOGGED IN USER OR LOGGED IN AS ADMIN USER #######
    ...
    function archive_file() {
        ...
        $reply_task = $this->Easycase->find('first', array('fields' => array('Easycase.case_no'), 'conditions' => array('Easycase.id' => $getFiles['CaseFile']['easycase_id'])));
        $post_task = $this->Easycase->find('first', array('conditions' => array('Easycase.case_no' => $reply_task['Easycase']['case_no'], 'Easycase.project_id' => $getFiles['CaseFile']['project_id'], 'Easycase.istype' => 1)));
    ...
    function ajaxpostcase($oauth_arg = NULL) {
        ...
        $ses_comp = !empty($oauth_arg['company_id']) ? $oauth_arg['company_id'] : SES_COMP;
        $oauth_return = 0;
        ...
        if (trim($CS_project_id)) {
            ...
            $this->Easycase->recursive = -1;
            $project_estimate_hr = $this->Project->find('first', array('conditions' => array('Project.company_id' => $ses_comp, 'Project.isactive' => 1, 'Project.uniq_id' => $CS_project_id), 'fields' => array('Project.estimated_hours', 'Project.id', 'Project.end_date')));
            $arr['prjct_estmt_hr'] = (int) $project_estimate_hr['Project']['estimated_hours'];
            $arr['pjrjct_id'] = $project_estimate_hr['Project']['id'];
            $arr['prjct_enddate'] = $project_estimate_hr['Project']['end_date'];
            /* $conds = "SELECT SUM(Easycase.estimated_hours) AS estimated_hrs FROM easycases AS Easycase WHERE Easycase.project_id=".$project_estimate_hr['Project']['id']." AND Easycase.istype = 1";
              $tot_etimate_hr = $this->Easycase->query($conds);
              $arr['total_tsk_estmt_hr'] = ceil(($tot_etimate_hr[0][0]['estimated_hrs'])/3600); */
            if (defined('GNC') && GNC == 1) {
    ...
    function downloadfiles($files = NULL, $type = NULL) {
        ...
        if (empty($getFiles)) {
            ...
            if (isset($getFiles['CaseFile']['upload_name']) && !empty($getFiles['CaseFile']['upload_name'])) {
                $chkfile = $getFiles['CaseFile']['upload_name'];
            }
        } else {
            if (isset($getFiles['CaseFile']['upload_name']) && !empty($getFiles['CaseFile']['upload_name'])) {
                $chkfile = $getFiles['CaseFile']['upload_name'];
        ...
        if (!empty($getFiles) || ($type != NULL)) {
            $this->layout = 'ajax';
            $this->Format->downloadFile($files, $chkfile);
            exit;
        } else {
...
//					}
    ...
    function dashboard() {
        ...
        if (isset($_COOKIE['MILESTONE'])) {
            $caseMilestone = $_COOKIE['MILESTONE'];
        } else {
            $caseMilestone = "all";
        }
        if (isset($_COOKIE['PROJECT'])) {
            $caseProject = $_COOKIE['PROJECT'];
        } else {
            $caseProject = "all";
        }
        ...
        #echo $caseMilestone;exit;
        $this->set('caseMilestone1', $caseMilestone);
        $this->set('caseProject', $caseProject);
        ...
        $searchData = $this->SearchFilter->find('list', array('fields' => array('SearchFilter.id', 'SearchFilter.name'), 'conditions' => array('SearchFilter.user_id' => SES_ID)));
        $user = $this->User->find('first', array('fields' => array('User.id', 'User.custom_active_dashboard_tab'), 'conditions' => array('User.id' => SES_ID)));
        $custom_active_dashboard_tab = !empty($user['User']['custom_active_dashboard_tab']) ? $user['User']['custom_active_dashboard_tab'] : 0;
        $newSearchData = array();

        if (!empty($searchData) && !empty($custom_active_dashboard_tab)) {
            $i = 1;
            foreach ($searchData as $skey => $sval) {
                if ($custom_active_dashboard_tab & $i) {
                    $data = $this->SearchFilter->find('first', array('conditions' => array('SearchFilter.id' => $skey)));
                    $arr = json_decode($data['SearchFilter']['json_array']);
                    $count = $this->SearchFilter->getCount($arr, PROJ_UNIQ_ID, '', '', '');
                    $newSearchData[$skey] = $sval . '(' . $count . ')';
                }
                $i = $i + $i;
            }
        }
        $this->set(compact('newSearchData'));
    }
            ...
            foreach ($caseAll as $key => $getdata) {
                if (isset($getdata['CaseFile']['downloadurl']) && trim($getdata['CaseFile']['downloadurl'])) {
                    ...
                    if ($frmt->validateImgFileExt($getdata['CaseFile']['file'])) {
                        if (USE_S3 == 0) {
                            if (isset($getdata['CaseFile']['upload_name']) && !empty($getdata['CaseFile']['upload_name'])) {
                                $caseAll[$key]['fileurl'] = HTTP_CASE_FILES . $getdata['CaseFile']['upload_name'];
                            } else {
                                $caseAll[$key]['fileurl'] = HTTP_CASE_FILES . $getdata['CaseFile']['file'];
                            }
                        } else {
                            if (isset($getdata['CaseFile']['upload_name']) && !empty($getdata['CaseFile']['upload_name'])) {
                                $caseAll[$key]['fileurl'] = $frmt->generateTemporaryURL(DIR_CASE_FILES_S3 . $getdata['CaseFile']['upload_name']);
                            } else {
                    ...
                    if (isset($getdata['CaseFile']['upload_name']) && !empty($getdata['CaseFile']['upload_name'])) {
                        $caseAll[$key]['link_url'] = HTTP_ROOT . "easycases/download/" . $getdata['CaseFile']['upload_name'];  //PRRB
                    } else {
    ...
    function case_project() {
        ...
        $caseMilestone = $this->params['data']['caseMilestone']; // Filter by Milestone
        $caseProject = $this->params['data']['caseProject']; // Filter by Project
        ...
        $lgnd_val = '';
        $workflow_id = 0;
        $last_projroleId = $current_projroleId = "";
        if ($projUniq != 'all') {
            $this->loadModel('ProjectUser');
            $this->ProjectUser->unbindModel(array('belongsTo' => array('User')));
            // $projlastupdate = $this->ProjectUser->find('first', array('conditions' => array('ProjectUser.user_id' => SES_ID, 'Project.isactive' => 1, 'ProjectUser.company_id' => SES_COMP), 'fields' => array('Project.id', 'Project.short_name', 'ProjectUser.id', 'ProjectUser.role_id'),'order'=>"ProjectUser.dt_visited DESC", 'recursive' => 3));
        ...
        #echo "last=>".$last_projroleId."<br/>current=>".$current_projroleId ;exit;
        if (defined('ROLE') && ROLE == 1) {
            $resCaseProj['last_projroleId'] = $last_projroleId;
            $resCaseProj['current_projroleId'] = $current_projroleId;
        }
        ...
        } elseif ($sortby == 'milestone') {
            $caseAtsort = strtolower($sortorder);
            $orderby = "mname " . $sortorder;
        } elseif ($sortby == 'project') {
            $caseAtsort = strtolower($sortorder);
            $orderby = "Pname " . $sortorder;
                    ...
                    if (trim($commonCaseId) != '') {
                        ...
                        if ($allowed == 'No') {
                            ...
                            if ($done) {
                                ...
                                if (defined('GINV') && GINV == 1 && $csLeg == 3) {
                                    $data = array('task_id' => $caseStsId, 'project_id' => $closeStsPid);
                                    $response = $this->requestAction('Invoice/invoices/autoGenerateInvoiceOnTaskClose', array('pass' => $data));
                                }

                                $caseuniqid = $this->Format->generateUniqNumber();
                                $this->Easycase->query("INSERT INTO easycases SET uniq_id='" . $caseuniqid . "', user_id='" . SES_ID . "', format='2', istype='2', actual_dt_created='" . GMT_DATETIME . "', case_no='" . $caseStsNo . "', project_id='" . $closeStsPid . "', type_id='" . $closeStsTyp . "', priority='" . $closeStsPri . "', status='" . $csSts . "', legend='" . $csLeg . "',previous_legend='" . $csLeg . "', dt_created='" . GMT_DATETIME . "'");
...
//                    $caseAll = $this->Easycase->query("SELECT SQL_CALC_FOUND_ROWS Easycase.*,User.short_name,IF((Easycase.assign_to = 0 OR Easycase.assign_to =" . SES_ID . "),'Me',User.short_name) AS Assigned FROM ( SELECT  Easycase.*,EasycaseMilestone.id AS Emid, EasycaseMilestone.milestone_id AS Em_milestone_id,EasycaseMilestone.user_id AS Em_user_id,EasycaseMilestone.id_seq,Milestone.id as Mid,Milestone.title AS Mtitle ,Milestone.end_date,Milestone.isactive AS Misactive,Milestone.project_id AS Mproject_id,Milestone.uniq_id AS Muinq_id FROM easycases as Easycase,easycase_milestones AS EasycaseMilestone,milestones AS Milestone WHERE EasycaseMilestone.easycase_id=Easycase.id AND Milestone.id=EasycaseMilestone.milestone_id AND Easycase.istype='1' AND " . $clt_sql . " " . $cond_easycase_actuve . " AND " . $qrycheck . " AND Easycase.project_id!=0 AND Easycase.project_id IN (SELECT ProjectUser.project_id FROM project_users AS ProjectUser,projects as Project WHERE ProjectUser.user_id=" . SES_ID . " AND ProjectUser.project_id=Project.id AND Project.isactive='1' AND ProjectUser.company_id='" . SES_COMP . "') " . $searchcase . " " . trim($qry) . " AND EasycaseMilestone.easycase_id=Easycase.id AND EasycaseMilestone.project_id IN (SELECT ProjectUser.project_id FROM project_users AS ProjectUser,projects as Project WHERE ProjectUser.user_id=" . SES_ID . " AND ProjectUser.project_id=Project.id AND Project.isactive='1')" . $msQuery . " ) AS Easycase LEFT JOIN users User ON Easycase.assign_to=User.id ORDER BY Easycase.end_date ASC,Easycase.Mtitle ASC," . $orderby . " LIMIT $limit1,$limit2");
                ...
                if ($projUniq == 'all') {
                    ...
                    if (trim($caseMilestone) && $caseMilestone != "all") {
                        $qry1 = $this->Format->milestoneFilter($caseMilestone);
                    }
                    if (trim($caseProject) && $caseProject != "all") {
                        $proIn = $this->Format->projectFilter($caseProject);
...
//                        $caseAll = $this->Easycase->query("SELECT SQL_CALC_FOUND_ROWS Easycase.*,User.short_name,IF((Easycase.assign_to = 0 OR Easycase.assign_to =" . SES_ID . "),'Me',User.short_name) AS Assigned FROM ( SELECT * FROM easycases as Easycase WHERE Easycase.istype='1' AND " . $clt_sql . " " . $cond_easycase_actuve . " AND Easycase.project_id!=0 AND Easycase.project_id IN (SELECT ProjectUser.project_id FROM project_users AS ProjectUser,projects as Project WHERE ProjectUser.user_id=" . SES_ID . " AND ProjectUser.project_id=Project.id AND Project.isactive='1' AND ProjectUser.company_id='" . SES_COMP . "') " . $searchcase . " " . trim($qry) . "  ORDER BY  Easycase.dt_created DESC) AS Easycase LEFT JOIN users User ON Easycase.assign_to=User.id ORDER BY " . $orderby . " LIMIT $limit1,$limit2");
                            $req_sql = "SELECT SQL_CALC_FOUND_ROWS Easycase.*,Project.name as Pname,EasycaseMilestone.milestone_id as mid,Milestone.title as mname,User.short_name,IF((Easycase.assign_to =" . SES_ID . "),'Me',User.short_name) AS Assigned FROM ( SELECT * FROM easycases as Easycase WHERE istype='1' AND " . $clt_sql . " " . $cond_easycase_actuve . " AND Easycase.project_id!=0  AND Easycase.project_id IN ($proIn) " . $searchcase . " " . trim($qry) . " ) AS Easycase LEFT JOIN users User ON Easycase.assign_to=User.id " . $case_join . " LEFT JOIN easycase_milestones EasycaseMilestone ON Easycase.id=EasycaseMilestone.easycase_id LEFT JOIN milestones Milestone ON Milestone.id=EasycaseMilestone.milestone_id LEFT JOIN projects AS Project ON Easycase.project_id=Project.id" . $milstone_filter_condition . $qry1 . " ORDER BY " . $orderby . "  LIMIT $limit1,$limit2";
                            $caseAll = $this->Easycase->query($req_sql);
                        } else {
...
//                        $caseAll = $this->Easycase->query("SELECT SQL_CALC_FOUND_ROWS Easycase.*,User.short_name,IF((Easycase.assign_to = 0 OR Easycase.assign_to =" . SES_ID . "),'Me',User.short_name) AS Assigned FROM ( SELECT * FROM easycases as Easycase WHERE Easycase.istype='1' AND " . $clt_sql . " " . $cond_easycase_actuve . " AND Easycase.project_id!=0 AND Easycase.project_id IN (SELECT ProjectUser.project_id FROM project_users AS ProjectUser,projects as Project WHERE ProjectUser.user_id=" . SES_ID . " AND ProjectUser.project_id=Project.id AND Project.isactive='1' AND ProjectUser.company_id='" . SES_COMP . "') " . $searchcase . " " . trim($qry) . "  ORDER BY  Easycase.project_id DESC) AS Easycase LEFT JOIN users User ON Easycase.assign_to=User.id ORDER BY " . $orderby . " LIMIT $limit1,$limit2");
                    ...
                    if (trim($caseMilestone) && $caseMilestone != "all") {
                        $qry1 = $this->Format->milestoneFilter($caseMilestone);
                        $filterenabled = 1;
                        $req_sql = "SELECT SQL_CALC_FOUND_ROWS Easycase.*,Project.name as Pname,EasycaseMilestone.milestone_id as mid,Milestone.title as mname,User.short_name,IF((Easycase.assign_to =" . SES_ID . "),'Me',User.short_name) AS Assigned FROM ( SELECT * FROM easycases as Easycase WHERE istype='1' AND " . $clt_sql . " " . $cond_easycase_actuve . " AND Easycase.project_id='$curProjId' AND Easycase.project_id!=0  " . $searchcase . " " . trim($qry) . " ) AS Easycase LEFT JOIN users User ON Easycase.assign_to=User.id " . $case_join . "  JOIN easycase_milestones EasycaseMilestone ON Easycase.id=EasycaseMilestone.easycase_id LEFT JOIN milestones Milestone ON Milestone.id=EasycaseMilestone.milestone_id LEFT JOIN projects AS Project ON Easycase.project_id=Project.id " . $milstone_filter_condition . $qry1 . " ORDER BY " . $orderby . $mileSton_orderby . "  LIMIT $limit1,$limit2";
                        $caseAll = $this->Easycase->query($req_sql);
                    } else {
//                        $req_sql = "SELECT SQL_CALC_FOUND_ROWS Easycase.*,Project.name as Pname,EasycaseMilestone.milestone_id as mid,Milestone.title as mname,User.short_name,IF((Easycase.assign_to =" . SES_ID . "),'Me',User.short_name) AS Assigned FROM ( SELECT * FROM easycases as Easycase WHERE istype='1' AND " . $clt_sql . " " . $cond_easycase_actuve . " AND Easycase.project_id='$curProjId' AND Easycase.project_id!=0  " . $searchcase . " " . trim($qry) . " ) AS Easycase LEFT JOIN users User ON Easycase.assign_to=User.id " . $case_join . "  JOIN easycase_milestones EasycaseMilestone ON Easycase.id=EasycaseMilestone.easycase_id  LEFT JOIN projects AS Project ON Easycase.project_id=Project.id " . $milstone_filter_condition . $qry1 . " ORDER BY " . $orderby . $mileSton_orderby . "  LIMIT $limit1,$limit2";
                        $req_sql = "SELECT SQL_CALC_FOUND_ROWS Easycase.*,Project.name as Pname,EasycaseMilestone.milestone_id as mid,Milestone.title as mname,User.short_name,IF((Easycase.assign_to =" . SES_ID . "),'Me',User.short_name) AS Assigned FROM ( SELECT * FROM easycases as Easycase WHERE istype='1' AND " . $clt_sql . " " . $cond_easycase_actuve . " AND Easycase.project_id='$curProjId' AND Easycase.project_id!=0  " . $searchcase . " " . trim($qry) . " ) AS Easycase LEFT JOIN users User ON Easycase.assign_to=User.id " . $case_join . "  JOIN easycase_milestones EasycaseMilestone ON Easycase.id=EasycaseMilestone.easycase_id LEFT JOIN milestones Milestone ON Milestone.id=EasycaseMilestone.milestone_id LEFT JOIN projects AS Project ON Easycase.project_id=Project.id " . $milstone_filter_condition . $qry1 . " ORDER BY " . $orderby . $mileSton_orderby . "  LIMIT $limit1,$limit2";
                        $caseAll = $this->Easycase->query($req_sql);
//                        $caseAll2 = $this->Easycase->query($req_sql2);
//                        $caseAll = $caseAll1 +$caseAll2;
...
//           echo $req_sql; exit;
    ...
    function case_details($oauth_arg = NULL, $api_flag = '') {
        ...
        if (array_key_exists('gantt_start_date', $getPostCase[0]['Easycase'])) {
            if ($getPostCase['0']['Easycase']['gantt_start_date'] != "NULL" && $getPostCase['0']['Easycase']['gantt_start_date'] != "") {
                $caseStartDate = $tz->GetDateTime($timezone_id, $timezone_GMT, $timezone_DST, $timezone_code, $getPostCase['0']['Easycase']['gantt_start_date'], "datetime");
            } else {
        ...
        $curdtT = $tz->GetDateTime($timezone_id, $timezone_GMT, $timezone_DST, $timezone_code, GMT_DATETIME, "datetime");
        ...
        if (array_key_exists('gantt_start_date', $getPostCase[0]['Easycase'])) {
            ...
            if ($caseTypeRep == 10 || $caseLegendRep == 3 || $caseLegendRep == 5) {
                if ($caseStartDate != "NULL" && $caseStartDate != "0000-00-00" && $caseStartDate != "" && $caseStartDate != "1970-01-01") {
                    ...
                    $csStartDtFmt = $dt->dateFormatOutputdateTime_day($caseStartDate, $curDateTz, 'time');
                ...
                if ($caseStartDate != "NULL" && $caseStartDate != "0000-00-00" && $caseStartDate != "" && $caseStartDate != "1970-01-01") {
                    $csStartDtFmtT = $dt->facebook_datetimestyle($caseStartDate);
                    // echo $csStartDtFmtT."<br/>";
                    $csStartDtFmt = $dt->dateFormatOutputdateTime_day($caseStartDate, $curDateTz, 'time');
                    // echo $csStartDtFmt;exit;
        ...
        if ($caseTypeRep == 10 || $caseLegendRep == 3 || $caseLegendRep == 5) {
            ...
            if ($caseDuDate != "NULL" && date('Y-m-d', strtotime($caseDuDate)) != "0000-00-00" && $caseDuDate != "" && date('Y-m-d', strtotime($caseDuDate)) != "1970-01-01" && $caseDuDate != "0000-00-00 00:00:00") {
                $caseDuDate = $tz->GetDateTime($timezone_id, $timezone_GMT, $timezone_DST, $timezone_code, $caseDuDate, "datetime");
                $csDuDtFmtT = $dt->facebook_datestyle($caseDuDate);
                $csDuDtFmt = $dt->dateFormatOutputdateTime_day($caseDuDate, $curDateTz, 'time');
...
//        echo "SELECT  Easycase.actual_dt_created,DISTINCT Easycase.previous_legend,Easycase.dt_created,Easycase.user_id FROM easycases AS Easycase WHERE Easycase.project_id=$ProjId AND Easycase.case_no=$curCaseNo  GROUP BY  Easycase.actual_dt_created ORDER BY Easycase.actual_dt_created"; exit;
        $states_data = $this->Easycase->query("SELECT  Easycase.actual_dt_created, Easycase.previous_legend,Easycase.dt_created,Easycase.user_id FROM easycases AS Easycase WHERE Easycase.project_id=$ProjId AND Easycase.case_no=$curCaseNo  GROUP BY  Easycase.actual_dt_created ORDER BY Easycase.actual_dt_created");
//        $states_data = $this->Easycase->find('all', array('fields' => array('DISTNICT Easycase.previous_legend','Easycase.id', 'Easycase.actual_dt_created','Easycase.dt_created','Easycase.user_id'), 'conditions' => array('Easycase.project_id' => $ProjId, 'Easycase.case_no' => $curCaseNo),'group'=>array('Easycase.previous_legend')));
        $this->Project->unbindModel(array('hasMany' => array('ProjectUser'), 'hasAndBelongsToMany' => array('User')));
        $prjArr = $this->Project->find('first', array('conditions' => array('Project.id' => $ProjId), 'fields' => array('Project.id', 'Project.workflow_id')));
        if ($prjArr['Project']['workflow_id'] != 0) {
            $this->loadModel('TaskStatusGroup.Workflow');
            $statuses = $this->Workflow->find('first', array('conditions' => array('Workflow.id' => $prjArr['Project']['workflow_id'])));
            $status = array();
            foreach ($statuses['Status'] as $skey => $svalue) {
                $status[$svalue['id']] = "<font style='color:" . $svalue['color'] . "'>" . ($svalue['name']) . "</font>";
            }
            $status_arr = Hash::combine($statuses['Status'], "{n}.id", "{n}.seq_order");
        } else {
            $this->loadModel('Status');
            $statuses = $this->Status->find('all', array('conditions' => array('Status.workflow_id' => 0)));
            $status = array();
            foreach ($statuses as $skey => $svalue) {
                $status[$svalue['Status']['id']] = "<font style='color:" . $svalue['Status']['color'] . "'>" . ($svalue['Status']['name']) . "</font>";
            }
            $status_arr = Hash::combine($statuses, "{n}.Status.id", "{n}.Status.seq_order");
        }

        $max_id = array_keys($status_arr, max($status_arr));
        $state_record['Status'] = array();
        foreach ($states_data as $lkey => $lval) {
            if ($lval['Easycase']['previous_legend'] == $states_data[$lkey + 1]['Easycase']['previous_legend']) {
                continue;
            } else {
                $state_record['Status'][$lkey]['status_name'] = !empty($lval['Easycase']['previous_legend']) ? $status[$lval['Easycase']['previous_legend']] : 'NA';
                $start_date = !empty($lval['Easycase']['actual_dt_created']) ? $this->Tmzone->GetDateTime(SES_TIMEZONE, TZ_GMT, TZ_DST, TZ_CODE, $lval['Easycase']['actual_dt_created'], "datetime") : '';

                $state_record['Status'][$lkey]['actual_dt_created'] = $dt->dateFormatOutputdateTime_day($start_date, $curDateTz);
                $today = $this->Tmzone->GetDateTime(SES_TIMEZONE, TZ_GMT, TZ_DST, TZ_CODE, GMT_DATETIME, "datetime");
                $end_date = !empty($states_data[$lkey + 1]['Easycase']['actual_dt_created']) ? $this->Tmzone->GetDateTime(SES_TIMEZONE, TZ_GMT, TZ_DST, TZ_CODE, $states_data[$lkey + 1]['Easycase']['actual_dt_created'], "datetime") : $today;
                if ($max_id['0'] != $lval['Easycase']['previous_legend']) {
                    $state_record['Status'][$lkey]['last_dt_updated'] = !empty($states_data[$lkey + 1]['Easycase']['actual_dt_created']) ? $dt->dateFormatOutputdateTime_day($end_date, $curDateTz) : $dt->dateFormatOutputdateTime_day($end_date, $curDateTz) . " (Cont..)";
                } else {
                    $state_record['Status'][$lkey]['last_dt_updated'] = !empty($states_data[$lkey + 1]['Easycase']['actual_dt_created']) ? $dt->dateFormatOutputdateTime_day($end_date, $curDateTz) : $dt->dateFormatOutputdateTime_day($end_date, $curDateTz);
                }
                $durations = $this->Format->datediffernce($start_date, $end_date);
                if ($max_id['0'] != $lval['Easycase']['previous_legend']) {
                    $state_record['Status'][$lkey]['duration'] = !empty($states_data[$lkey + 1]['Easycase']['actual_dt_created']) ? $durations : $durations . " (Cont..)";
                } else {
                    $state_record['Status'][$lkey]['duration'] = !empty($states_data[$lkey + 1]['Easycase']['actual_dt_created']) ? $durations : $durations;
                }
                $state_record['Status'][$lkey]['username'] = !empty($lval['Easycase']['user_id']) ? $users[$lval['Easycase']['user_id']] : 'NA';
            }
        }
        $caseDetail['Status'] = $state_record['Status'];
        if (intval($oauth_return)) {
            return $caseDetail;
        } else {
    ...
    function ajax_case_menu() {
        ...
        if ($prjUniqIdCsMenu != 'all') {
            ...
            // echo 'SELECT COUNT(DISTINCT Easycase.id) as hpcount FROM easycases Easycase WHERE Easycase.isactive = 1 AND ' . $clt_sql . ' AND Easycase.istype= 1 AND Easycase.project_id =' . $proj_id . ' AND Easycase.priority = 0 ' . $qry . $searchcase;exit;
            $caseHighPri = $this->Easycase->query('SELECT COUNT(DISTINCT Easycase.id) as hpcount FROM easycases Easycase WHERE Easycase.isactive = 1 AND ' . $clt_sql . ' AND Easycase.istype= 1 AND Easycase.project_id =' . $proj_id . ' AND Easycase.type_id != 10 AND Easycase.priority = 0 ' . $qry . $searchcase);
        ...
        if ($prjUniqIdCsMenu == 'all') {
            ...
            $caseHighPri = $this->Easycase->query('SELECT COUNT(DISTINCT Easycase.id) as hpcount FROM easycases Easycase WHERE Easycase.isactive = 1 AND Easycase.istype= 1 AND ' . $clt_sql . ' AND Easycase.project_id IN(' . $idlist . ') AND Easycase.type_id != 10 AND Easycase.priority = 0 ' . $qry . $searchcase);
        }
        $resCaseMenu = array();
    ...
    function ajax_case_status() {
        ...
        } elseif ($this->params['data']['page_type'] == 'ajax_milestone') {
            $this->loadModel('Milestone');
            $this->loadModel('EasycaseMilestone');
//            pr($proj_uniq_id); exit;
            if ($proj_uniq_id != 'all' && $proj_uniq_id != '') {
                $this->Project->unBindModel(array(hasMany => array('ProjectUser'), hasAndBelongsToMany => array('User')));
                $projects = $this->Project->find('first', array('fields' => array('Project.id'), 'conditions' => array('Project.uniq_id' => $proj_uniq_id)));
                $proId = $projects['Project']['id'];
                $this->Milestone->bindModel(array('hasMany' => array('EasycaseMilestone' => array('foreignKey' => 'milestone_id'))));
                $milestones = $this->Milestone->find('all', array('fields' => array('Milestone.id', 'Milestone.uniq_id', 'Milestone.company_id', 'Milestone.title'), 'conditions' => array('Milestone.project_id' => $proj_id, 'Milestone.company_id' => SES_COMP, 'Milestone.isactive' => 1), 'order' => array('Milestone.title' => 'ASC')));
            } else {
                $this->Milestone->bindModel(array('hasMany' => array('EasycaseMilestone' => array('foreignKey' => 'milestone_id'))));
                $milestones = $this->Milestone->find('all', array('fields' => array('Milestone.id', 'Milestone.uniq_id', 'Milestone.company_id', 'Milestone.title'), 'conditions' => array('Milestone.company_id' => SES_COMP, 'Milestone.isactive' => 1), 'order' => array('Milestone.title' => 'ASC')));
//            $asnArr = $this->Easycase->query("SELECT DISTINCT User.id, User.name, User.email, User.istype,User.email,User.short_name,User.dt_last_login,  (select count(Easycase.id) from easycases as Easycase" . $mlstnQ1 . " where Easycase.assign_to = User.id and Easycase.istype='1' and " . $clt_sql . " AND User.isactive='1' and Easycase.isactive='1' " . $mlstnQ2 . $projQry . " " . trim($qry) . ") as cases FROM users as User,project_users as ProjectUser,company_users as CompanyUser,projects as Project WHERE CompanyUser.user_id=ProjectUser.user_id AND CompanyUser.is_active='1' AND CompanyUser.company_id='" . SES_COMP . "' $projQryMem  AND Project.id=ProjectUser.project_id AND User.isactive='1' AND ProjectUser.user_id=User.id ORDER BY User.name ASC");
            }
            $this->set(compact('milestones'));
            $this->set('proj_uniq_id', $proj_uniq_id);
            $this->set('proj_id', $proj_id);
            $this->set('CookieMile', $_COOKIE['MILESTONE']);
            $this->render('ajax_milestone', 'ajax');
        } elseif ($this->params['data']['page_type'] == 'ajax_project') {
            $this->loadModel('Project');
            $this->Project->unBindModel(array(hasMany => array('ProjectUser'), hasAndBelongsToMany => array('User')));
            $this->Project->bindModel(array('hasMany' => array('EasycaseMilestone' => array('foreignKey' => 'project_id'))));
            $projects = $this->Project->find('all', array('fields' => array('Project.id', 'Project.uniq_id', 'Project.company_id', 'Project.name', 'Project.short_name'), 'condtions' => array('Project.company_id' => SES_COMP, 'Project.isactive' => 1), 'order' => array('Project.name' => 'ASC')));
            $this->set(compact('projects'));
            $this->set('proj_uniq_id', $proj_uniq_id);
            $this->set('proj_id', $proj_id);
            $this->set('CookiePro', $_COOKIE['Project']);
            $this->render('ajax_project', 'ajax');
        ...
        if ($this->params['data']['page_type'] == 'ajax_utilization_status') {
            ...
            if (defined('TSG') && TSG == 1) {
                foreach ($status_arry as $kst => $vst) {
                    $status_arry[$kst] = $vst . " - ".__("Default Task Status Group");
                }
                $this->Project->recursive = -1;
                $prjArr = $this->Project->find('all', array('conditions' => array('Project.company_id' => SES_COMP, 'Project.isactive' => 1), 'fields' => array('Project.id', 'Project.workflow_id')));
                // echo "<pre>";print_r($prjArr);exit;
    ...
    // to change the estimate hr of task if total estimate hour of project greater then the project estimate hour
    function change_tsk_est_hr() {
        $this->layout = 'ajax';
        //echo "<pre>"; print_r($this->request);exit;
        $caseId = $this->request->data['task_id'];
        $estHour = $this->request->data['estimated_hr'];
    ...
    function ajax_change_estHour() {
        ...
        if ($allowed == 'No') {
            ...
            if ($getCase) {
                ...
                $dataeasycase = $this->Easycase->query($sqldata);
                $caseuniqid = $this->Format->generateUniqNumber();
                $this->Easycase->query("INSERT INTO easycases SET uniq_id='" . $caseuniqid . "', case_no = '" . $dataeasycase[0]['easycases']['case_no'] . "',  case_count=0, project_id='" . $dataeasycase[0]['easycases']['project_id'] . "', user_id='" . SES_ID . "', updated_by='" . SES_ID . "', type_id='" . $dataeasycase[0]['easycases']['type_id'] . "', priority='" . $dataeasycase[0]['easycases']['priority'] . "', title='', message='', estimated_hours='" . $estHour . "', hours='0', assign_to='" . $dataeasycase[0]['easycases']['assign_to'] . "', istype='2',format='2', status='" . $dataeasycase[0]['easycases']['status'] . "', previous_legend='" . $dataeasycase[0]['easycases']['previous_legend'] . "', legend='" . $dataeasycase[0]['easycases']['legend'] . "', isactive=1, dt_created='" . GMT_DATETIME . "',actual_dt_created='" . GMT_DATETIME . "',reply_type='6'");
                $val['success'] = 'success';
                if (defined('GTLG') && GTLG == 1) {
    ...
    function ajax_change_AssignTo() {
        ...
        $sqldata = "SELECT * FROM `easycases` WHERE `id`='" . $caseId . "' ";
        $dataeasycase = $this->Easycase->query($sqldata); //print_r($dataeasycase);
        $sqldata1 = "SELECT * FROM `easycases` WHERE `case_no`='" . $dataeasycase[0]['easycases']['case_no'] . "' ORDER BY id DESC LIMIT 1";
        $dataeasycase1 = $this->Easycase->query($sqldata1); //print_r($dataeasycase);
        $caseuniqid = $this->Format->generateUniqNumber();
        $this->Easycase->query("INSERT INTO easycases SET uniq_id='" . $caseuniqid . "', case_no = '" . $dataeasycase[0]['easycases']['case_no'] . "', 	case_count=0, project_id='" . $dataeasycase[0]['easycases']['project_id'] . "', user_id='" . SES_ID . "', updated_by=0, type_id='" . $dataeasycase[0]['easycases']['type_id'] . "', priority='" . $dataeasycase[0]['easycases']['priority'] . "', title='', message='', hours='0', assign_to='" . $assignId . "', istype='2',format='2', status='" . $dataeasycase[0]['easycases']['status'] . "', legend='" . $dataeasycase[0]['easycases']['legend'] . "',previous_legend='" . $dataeasycase1[0]['easycases']['previous_legend'] . "', isactive=1, dt_created='" . GMT_DATETIME . "',actual_dt_created='" . GMT_DATETIME . "',reply_type=2");
    ...
    function ajax_change_StartDate() {
        ...
        if ($duedt != '00/00/0000' && $duedt != '') {
            $strtdt = date("Y-m-d H:i:s", strtotime($duedt));
            $due_date = $this->Tmzone->convert_to_utc(SES_TIMEZONE, TZ_GMT, TZ_DST, TZ_CODE, $strtdt, "datetime");
        ...
        if ($getCase) {
            ...
            if ($allowed_due_date == 'Yes') {
                if (strtotime($getCase['Easycase']['due_date']) > strtotime($due_date)) {
                    // print_r(strtotime($getCase['Easycase']['due_date']));echo "<br/>";print_r(strtotime($due_date));exit;
                    ...
                    if ($duedt == "00/00/0000") {
                        ...
                        if ($text == "Today") {
                            ...
                            $val['top'] = $dt->dateFormatOutputdateTime_day($strtdt, $curDateTz, 'time');
                            $val['details'] = "<b>" . $dt->dateFormatOutputdateTime_day($strtdt, $curDateTz, 'time') . "</b>";
                        }
                    }
                    if (defined('GTLG') && GTLG == 1) {
                        $postParam = $this->Easycase->find('first', array('conditions' => array('Easycase.id' => $caseId)));
                    ...
                    $val = array('success' => 0, 'message' => 'Start date should not be more than due date');
                }
            } else {
                $val = array('success' => 0, 'message' => 'Start date should not be more than ' . $dt->dateFormatOutputdateTime_day($min_due_date, GMT_DATETIME, 'time'));
            }
            if ($val) {
                echo json_encode($val);
            }
    ...
    function ajax_change_DueDate() {
        ...
        if (defined('TSG') && TSG == 1) {
            $getCase = $this->Easycase->find('first', array(
                ...
                'fields' => array('id', 'gantt_start_date', 'due_date')
            ));
        } else {
            $getCase = $this->Easycase->find('first', array(
                'conditions' => array(
        ...
        if ($getCase) {
            ...
            if ($allowed_due_date == 'Yes') {
                if (strtotime($getCase['Easycase']['gantt_start_date']) < strtotime($due_date)) {
                    ...
                    if ($duedt == "00/00/0000" || $duedt == "0000-00-00 00:00:00") {
                        ...
                        if ($text == "Today") {
                            ...
                            $val['top'] = $dt->dateFormatOutputdateTime_day($duedt, $curDateTz, 'time');
                            $val['details'] = "<b>" . $dt->dateFormatOutputdateTime_day($duedt, $curDateTz, 'time') . "</b>";
                        }
                    }
                    if (defined('GTLG') && GTLG == 1) {
                        $postParam = $this->Easycase->find('first', array('conditions' => array('Easycase.id' => $caseId)));
                    ...
                    $top = $dt->dateFormatOutputdateTime_day($getCase['Easycase']['due_date'], $curDateTz, 'time');
                    $val = array('success' => 0, 'message' => 'Due date should not be more  than Start date', 'top' => $top);
                }
            } else {
                $val = array('success' => 0, 'message' => 'Due date should be more than ' . $dt->dateFormatOutputdateTime_day($min_due_date, GMT_DATETIME, 'date'));
            }
            if ($val) {
                echo json_encode($val);
            }
...
//		$sql = "SELECT Easycase.*, Project.name FROM easycases as Easycase ,projects as Project, LEFT JOIN easycase_milestones as EasycaseMilestone ON Easycase.id=EasycaseMilestone.easycase_id LEFT JOIN milestones as Milestone on Milestone.id=EasycaseMilestone.milestone_id as EastcaseMilestone on  WHERE Easycase.project_id=Project.id AND Easycase.istype = 1 AND Easycase.title != '' AND Easycase.isactive='1' AND Easycase.project_id!=0 AND Easycase.project_id = '" . $curProjId . "' ".$qry." ORDER BY Easycase.dt_created ASC";
        ...
        foreach ($case_lists as $case_list) {
            ...
            if (trim($case_list['Easycase']['due_date']))
                $due_date = '"' .$frmt->chngdate($case_list['Easycase']['due_date']) . '"';
            $updated1 = $tz->GetDateTime(SES_TIMEZONE, TZ_GMT, TZ_DST, TZ_CODE, $case_list['Easycase']['dt_created'], "datetime");
    ...
    function ajaxemail($oauth_arg = NULL) {
        ...
        if (isset($this->data['type'])) {
            ...
            if (strstr($data['caseid_list'], ',') || trim($data['caseid_list'], ',')) {
                ...
                foreach ($commonArrId as $commonCaseId) {
                    if (trim($commonCaseId)) {
                        ...
                        $getEmailUser = $this->getAllNotifyUser($data['projId'], $emailUsers);
            ...
            if ($data['caseIstype'] == 1) {
                $getEmailUser = $this->getAllNotifyUser($data['projId'], $data['emailUser'], 'new');
            } else {
                $getEmailUser = $this->getAllNotifyUser($data['projId'], $data['emailUser'], 'reply');
            }//End
    ...
    function ajax_common_breadcrumb() {
        ...
        //For Milestone
        if (isset($this->params['data']['caseMilestone']) && $this->params['data']['caseMilestone']) {
            $case_milestone = $this->params['data']['caseMilestone'];
        } elseif ($_COOKIE['MILESTONE']) {
            $case_milestone = $_COOKIE['MILESTONE'];
        }

        if ($case_milestone && $case_milestone != "all") {
            $miles = '';
            $data = $this->Format->caseMileList($case_milestone);
            if (strstr($case_milestone, "-")) {
                $expst5 = explode("-", $case_milestone);
                $milestone = $this->Format->caseMileList($expst5);
                foreach ($milestone as $key => $st5) {
                    $miles .= "<div class='fl filter_opn' rel='tooltip' title='Milestone' onclick='openfilter_popup(1,\"dropdown_menu_all_filters\");allfiltervalue(\"milestone\");'>" . $st5 . "<a href='javascript:void(0);' onclick='common_reset_filter(\"milestone\",\"" . $key . "\",this);' class='fr'>X</a></div>";
                }
            } else {
                $miles = "<div class='fl filter_opn' rel='tooltip' title='Milestone' onclick='openfilter_popup(1,\"dropdown_menu_all_filters\");allfiltervalue(\"milestone\");'>" . $this->Format->caseMileList($case_milestone) . "<a href='javascript:void(0);' onclick='common_reset_filter(\"milestone\",\"" . $case_milestone . "\",this);' class='fr'>X</a></div>";
            }
            $arr['case_milestone'] = $miles;
            $val = 1;
        } else {
            $arr['case_milestone'] = 'All';
        }
        //For Project
        if (isset($this->params['data']['caseProject']) && $this->params['data']['caseProject']) {
            $case_project = $this->params['data']['caseProject'];
        } elseif ($_COOKIE['PROJECT']) {
            $case_project = $_COOKIE['PROJECT'];
        }

        if ($case_project && $case_project != "all") {
            $pros = '';
//            $data = $this->Format->caseProjectList($case_project);
            if (strstr($case_milestone, "-")) {
                $expst5 = explode("-", $case_project);
                $project = $this->Format->caseProjectList($expst5);
                foreach ($project as $key => $st5) {
                    $pros .= "<div class='fl filter_opn' rel='tooltip' title='Project' onclick='openfilter_popup(1,\"dropdown_menu_all_filters\");allfiltervalue(\"project\");'>" . $st5 . "<a href='javascript:void(0);' onclick='common_reset_filter(\"project\",\"" . $key . "\",this);' class='fr'>X</a></div>";
                }
            } else {
                $pros = "<div class='fl filter_opn' rel='tooltip' title='Project' onclick='openfilter_popup(1,\"dropdown_menu_all_filters\");allfiltervalue(\"project\");'>" . $this->Format->caseProjectList($case_project) . "<a href='javascript:void(0);' onclick='common_reset_filter(\"project\",\"" . $case_project . "\",this);' class='fr'>X</a></div>";
            }
            $arr['case_project'] = $pros;
            $val = 1;
        } else {
            $arr['case_project'] = 'All';
        }
...
#### EDITED TEXT OF REPLY ARE SAVED ###########	
    ...
    function ajax_save_filter() {
        ...
        if ($case_milestone && $case_milestone != "all") {
            if (strstr($case_milestone, "-")) {
                $expst6 = explode("-", $case_milestone);
                foreach ($expst6 as $st6) {
                    $asns1 .= $this->Format->caseBcMems($st6) . ", ";
                }
            } else {
                $asns1 = $this->Format->caseBcMems($case_milestone) . ", ";
            }
            $arr['case_milestone'] = trim($asns1, ', ');
            $val = 1;
        } else {
            $arr['case_milestone'] = 'All';
        }
        ...
        $duedate = $_COOKIE['DUE_DATE'];
        $this->set('milestone', $arr['case_milestone']);
        ...
        if ($caseUid) {
            ...
            if ($casedetails) {
                ...
                if ($casedetails['Easycase']['due_date']) {
                    $casedetails['Easycase']['due_date'] = date('m/d/Y H:i', strtotime($casedetails['Easycase']['due_date']));
                ...
                if ($casedetails['Easycase']['gantt_start_date']) {
                    $casedetails['Easycase']['gantt_start_date'] = $tmzone->GetDateTime(SES_TIMEZONE, TZ_GMT, TZ_DST, TZ_CODE, $casedetails['Easycase']['gantt_start_date'], "datetime");
                    // $casedetails['Easycase']['gantt_start_date'] = date('M d, D h:i ',  strtotime($start_date));
                }
...
//		    }
    ...
    function kanban_task() {
        ...
        $caseMilestone = $this->data['caseMilestone']; // Filter by AssignTo
        ...
        if ($projUniq) {
            ...
            if ($projUniq == 'all') {
                ...
                if (trim($caseMilestone) && $caseMilestone != "all") {
                    $qry1 = $this->Format->kanbanmilestoneFilter($caseMilestone);
                }
//                echo $qry1; exit;
                ...
                if (is_array($statuses) && !empty($statuses) && $morecontent == '') {
                    foreach ($statuses as $k => $status) {
                        ...
                        $sql = "SELECT Easycase.*,User.short_name,IF((Easycase.assign_to = 0 OR Easycase.assign_to =" . SES_ID . "),'Me',User.short_name) AS Assigned "
                                ...
                                . "AND Easycase.title LIKE '%$search_key%' " . $searchcase . " " . trim($qry) . " " . $qry1 . " " . " ) AS Easycase "
                ...
                if ($workflow_id == 0) {
                    if (($morecontent && $morecontent == 'newTask') || !$morecontent) {
                        $caseAll['newTask'] = $this->Easycase->query("SELECT Easycase.*,User.short_name,IF((Easycase.assign_to = 0 OR Easycase.assign_to =" . SES_ID . "),'Me',User.short_name) AS Assigned FROM ( SELECT Easycase.* FROM easycases as Easycase LEFT JOIN easycase_milestones AS EasycaseMilestone ON Easycase.id= EasycaseMilestone.easycase_id WHERE istype='1' " . $cond_easycase_actuve . " AND Easycase.project_id='$curProjId' AND Easycase.project_id!=0  AND Easycase.title LIKE '%$search_key%' " . $searchcase . " " . trim($qry) . " " . $qry1 . " " . " ) AS Easycase LEFT JOIN users User ON Easycase.assign_to=User.id WHERE Easycase.legend = 1 AND " . $clt_sql . " ORDER BY dt_created DESC LIMIT $newTask_limit,$limit2");
                    ...
                    if ($morecontent && $morecontent != '') {
                        ...
                        foreach ($workflow['Status'] as $k => $status) {
                            ...
                            $resCaseProj[$x] = count($caseAll[$status['name']]);
...
//        pr($frmtCaseAll);
    ...
    function getEstimateSpentTime($proj_id, $search_key, $searchcase, $qry, $cond_easycase_actuve, $legend, $all_prj = 0) {
        ...
        if (defined('TLG') && TLG != 1) {
            ...
            // echo "SELECT TIME_FORMAT(SEC_TO_TIME(SUM(Easycase.estimated_hours)), '%H:%i') estd_hrs,TIME_FORMAT(SEC_TO_TIME(SUM(LogTime.total_hours)),'%H:%i') as sphours FROM easycases as Easycase LEFT JOIN log_times as LogTime ON Easycase.id=LogTime.task_id WHERE Easycase.istype='1' " . $cond_easycase_actuve . " AND " . $prj_cond . " AND Easycase.title LIKE '%$search_key%' " . $searchcase . " " . trim($qry) . " AND " . $legend_cond . " AND " . $clt_sql;exit;
            $est_spt_hours = $this->Easycase->query("SELECT TIME_FORMAT(SEC_TO_TIME(SUM(Easycase.estimated_hours)), '%H:%i') estd_hrs,TIME_FORMAT(SEC_TO_TIME(SUM(LogTime.total_hours)),'%H:%i') as sphours FROM easycases as Easycase LEFT JOIN log_times as LogTime ON Easycase.id=LogTime.task_id WHERE Easycase.istype='1' " . $cond_easycase_actuve . " AND " . $prj_cond . " AND Easycase.title LIKE '%$search_key%' " . $searchcase . " AND " . $legend_cond . " AND " . $clt_sql);
        }
    ...
    function mydashboard() {
        ...
        if (defined('DBRD') && DBRD == 1) {
            if (SES_TYPE < 3) {
                ...
                $allcond = array('conditions' => array('ProjectUser.company_id' => SES_COMP, 'User.isactive' => 1), 'fields' => array('DISTINCT  User.id', 'CONCAT(User.name,User.last_name) AS Uname'), 'order' => array('User.name ASC'));
                $this->ProjectUser->bindModel(array('belongsTo' => array('User')));
                $UserProjArr = $this->ProjectUser->find('all', $allcond);
                //$user_lst = $this->User->find('list',array('conditions' => array('User.isactive' =>1),'fields' => array('User.id','User.name')));
                $user_lists = array();
                $uid = array();
                $uid = Hash::extract($UserProjArr, '{n}.User.id');
                $usrprjctlst = $this->ProjectUser->find('all', array('conditions' => array('ProjectUser.company_id' => SES_COMP, 'ProjectUser.user_id' => $uid), 'fields' => array('ProjectUser.project_id', 'ProjectUser.user_id'), 'order' => array('ProjectUser.project_id ASC')));
                $userprjcts = array();
                foreach ($usrprjctlst as $ku => $vu) {
                    $userprjcts[$vu['ProjectUser']['user_id']][] = $vu['ProjectUser']['project_id'];
                }
                foreach ($UserProjArr as $ku => $uv) {
                    $user_lists[$uv['User']['id']]['Name'] = $uv[0]['Uname'];
                    $user_lists[$uv['User']['id']]['Projects'] = implode(',', $userprjcts[$uv['User']['id']]);
                }
                if ($this->Auth->user('is_client') == 1) {
                ...
                if ((defined('TLG') && TLG == 1) || (defined('TPAY') && TPAY == 1) || (defined('GTLG') && GTLG == 1)) {
                    ...
                    $totalhours = $this->Format->format_time_hr_min($cntlog[0][0]['secds']);
                } else {
                    $count_sql = 'SELECT sum(Easycase.hours) as secds '
                            . 'FROM easycases AS Easycase '
                            . 'WHERE Easycase.project_id IN (SELECT ProjectUser.project_id FROM project_users AS ProjectUser, projects As Project WHERE ProjectUser.company_id=' . SES_COMP . ' AND ProjectUser.project_id=Project.id AND Project.isactive=1) '
                            . 'AND Easycase.isactive=1 ' . $clt_sql;
                ...
                $this->set('user_lsts', $user_lists);
            } else if (SES_TYPE == 3) {
                $this->loadModel('ProjectUser');
                $this->loadModel('CaseFile');
                $prjcnt = $this->ProjectUser->find('count', array('conditions' => array('ProjectUser.company_id' => SES_COMP, 'ProjectUser.user_id' => SES_ID), 'fields' => array('ProjectUser.project_id')));
                $activeprjs = $this->ProjectUser->find('count', array('conditions' => array('ProjectUser.company_id' => SES_COMP, 'ProjectUser.user_id' => SES_ID, 'Project.isactive' => 1)));
                $completedprjs = $this->ProjectUser->find('count', array('conditions' => array('ProjectUser.company_id' => SES_COMP, 'ProjectUser.user_id' => SES_ID, 'Project.isactive' => 2)));
                $last30dayscreatedprjs = $this->ProjectUser->find('count', array('conditions' => array('ProjectUser.company_id' => SES_COMP, 'ProjectUser.user_id' => SES_ID, "DATE(Project.dt_created) BETWEEN '" . $dates['strddt'] . "' AND '" . $dates['enddt'] . "' ")));
                #$users = $this->CompanyUser->find('all', array('conditions' => array('CompanyUser.company_id' => SES_COMP, 'CompanyUser.is_active' => 1,'CompanyUser.user_id' => SES_ID), 'fields' => array('CompanyUser.user_id')));
                #$projectIds = Hash::extract($activeprjs, '{n}.Project.id');
                $usedspace = $this->CaseFile->find('all', array('conditions' => array('CaseFile.company_id' => SES_COMP, 'CaseFile.user_id' => SES_ID, 'CaseFile.isactive' => 1), 'fields' => array('SUM(CaseFile.file_size) AS file_size ')));
                $clt_sql = "";
                $this->ProjectUser->bindModel(array('belongsTo' => array('User')));
                ...
                $prjct_list = $this->Project->find('list', array('conditions' => array('Project.id' => $usr_prjcts, 'Project.isactive' => 1), 'fields' => array('Project.id', 'Project.name')));
                $user_name = $this->User->findById(SES_ID);
                $user_lst = array(SES_ID => array('Name' => $user_name['User']['name'], 'Projects' => implode(',', $usr_prjcts)));
                if ($this->Auth->user('is_client') == 1) {
                    $clt_sql = " AND ((Easycase.client_status = " . $this->Auth->user('is_client') . " AND Easycase.user_id = " . $this->Auth->user('id') . ") OR Easycase.client_status != " . $this->Auth->user('is_client') . ") ";
                }
                if ((defined('TLG') && TLG == 1) || (defined('TPAY') && TPAY == 1) || (defined('GTLG') && GTLG == 1)) {
                    $this->loadModel('LogTime');
                    /* find total billable and non-billable time */
                    $projQry = " AND LogTime.project_id IN (SELECT ProjectUser.project_id FROM project_users AS ProjectUser,projects as Project WHERE ProjectUser.company_id=" . SES_COMP . " AND ProjectUser.user_id=" . SES_ID . " AND ProjectUser.project_id=Project.id AND Project.isactive='1') ";
                    $count_sql = 'SELECT sum(total_hours) as secds '
                            . 'FROM log_times AS `LogTime` '
                            . "LEFT JOIN easycases AS Easycase ON Easycase.id=LogTime.task_id AND LogTime.project_id=Easycase.project_id "
                            . 'WHERE Easycase.isactive =1 ' . $projQry . ' ' . $clt_sql . '  ';
                    ...
                    $count_sql = 'SELECT sum(Easycase.hours) as secds '
                            . 'FROM easycases AS Easycase '
                            . 'WHERE Easycase.project_id IN (SELECT ProjectUser.project_id FROM project_users AS ProjectUser, projects As Project WHERE ProjectUser.company_id=' . SES_COMP . ' AND ProjectUser.user_id=' . SES_ID . ' AND ProjectUser.project_id=Project.id AND Project.isactive=1) '
                            . 'AND Easycase.isactive=1 ' . $clt_sql;
                    $cntlog = $this->Easycase->query($count_sql);
                    $totalhours = $cntlog[0][0]['secds'];
                }

                $totalTasks_sql = "SELECT count(id) as total_tasks from easycases as Easycase where Easycase.isactive = 1 AND Easycase.istype = 1 AND Easycase.project_id IN (SELECT ProjectUser.project_id FROM project_users AS ProjectUser,projects as Project WHERE ProjectUser.company_id=" . SES_COMP . " AND ProjectUser.user_id=" . SES_ID . " AND ProjectUser.project_id=Project.id AND Project.isactive='1') " . $clt_sql . "";
                $total_tasks = $this->Easycase->query($totalTasks_sql);
                $closedTasks_sql = "SELECT count(id) as closed_tasks from easycases as Easycase where Easycase.isactive = 1 AND Easycase.istype = 1 AND Easycase.legend = 3 AND Easycase.project_id IN (SELECT ProjectUser.project_id FROM project_users AS ProjectUser,projects as Project WHERE ProjectUser.company_id=" . SES_COMP . " AND ProjectUser.user_id=" . SES_ID . " AND ProjectUser.project_id=Project.id AND Project.isactive='1') " . $clt_sql . "";
                $closed_tasks = $this->Easycase->query($closedTasks_sql);
                $dates = $this->Format->date_filter('last30days', $curDateTime);
                $this->set('prjcnt', $prjcnt);
                $this->set('actvprjcnt', $activeprjs);
                $this->set('cmpldprjcnt', $completedprjs);
                $this->set('last30dayscreatedprjcnt', $last30dayscreatedprjs);
                $this->set('usrcnt', 1);
                $this->set('usedspace', number_format(($usedspace[0][0]['file_size'] / 1024), 2));
                $this->set('totalhours', $totalhours);
                $this->set('totaltasks', $total_tasks[0][0]['total_tasks']);
                $this->set('closedtasks', $closed_tasks[0][0]['closed_tasks']);
                $this->set('prjct_list', $prjct_list);
                $this->set('user_lsts', $user_lst);
            }
            $this->render('new_dashboard');
        } else {
...
//		$completedtask  = $getcompletedtask[0]['Easycase']['completed_task'];
        ...
        foreach ($getPostCase AS $key => $case_list) {
            ...
            $arr[] = $crted = $this->Format->dateTimeFormat($temp_dat);
    ...
    function downloadexpensePdf() {
        ...
        $mode = $this->request->params['pass'][2];
        $expensePdfFileName = $this->request->params['pass'][0];

        $i = $this->Expense->findById($id);
        $invoice_no = $this->Format->seo_url($i['Invoice']['invoice_no'], '_');
        ...
        $file = WWW_ROOT . 'files' . DS . 'expense_pdfs' . DS . $expensePdfFileName . '.pdf';
        if (file_exists($file)) {
            $this->response->file($file, array('download' => $mode == 'preview' ? false : true, 'name' => urlencode(basename($file))));
            return $this->response;
        } else {
    ...
    function expensePdf($id, $company_id, $layout, $expense_file_name, $exportType) {
        $this->layout = false;
        $this->loadModel('Expense');
        $this->loadModel('ExpenseDetail');
        $this->loadModel('ExpenseCategory');
        $this->loadModel('ExpenseAttachment');
        ...
        if ($exportType == "selectedExport") {
            $expenseIdsArray = json_decode($id, true);
            foreach ($expenseIdsArray as $key => $value) {
                $ExpenseIdVal = explode("####", $value)[0];
                $SubExpenseIdVal = explode("####", $value)[1];

                $mainExpenseArray[$ExpenseIdVal][] = $SubExpenseIdVal;
            }

            foreach ($mainExpenseArray as $mkey => $mval) {
                foreach ($mval as $mkey1 => $mval1) {
                    $getResult = $this->Expense->query("SELECT `exp`.*, `subexp`.* FROM `expenses` as `exp`, `expense_details` as `subexp` WHERE `exp`.`id`='" . $mkey . "' AND `exp`.`id`=`subexp`.`expense_id` AND `subexp`.`id`='" . $mval1 . "'");
                    $i[$mkey]['Expense'] = $getResult[0]['exp'];
                    $i[$mkey]['ExpenseDetail'][] = $getResult[0]['subexp'];
                }
            }
            $i = array_values($i);
        } elseif ($exportType == "mainExport") {
            $i = $this->Expense->find('all', array('conditions' => array('id' => $id)));
        }

        #echo "<pre>";print_r($i);exit;
        $comp_detais = $this->Company->query("SELECT * FROM companies AS Company WHERE Company.id =" . SES_COMP);
        //$i['Invoice']['logo'] = trim($comp_detais[0]['Company']['logo']);

        if (SES_COMP) {
            $com_id = $com_id['Company']['id'];
            $project_list = $this->Project->find('list', array('fields' => array('Project.id', 'Project.name'), 'condtions' => array('Project.company_id' => $com_id)));
        }
        #echo "<pre>";print_r($project_list);exit;
        $this->set('i', $i);
        $this->set(compact('project_list'));
        //$this->set('company_id', $company_id);
        $this->set('company_name', $comp_detais[0]['Company']['name']);
        $this->set('layout', $layout);
        if (defined('USE_WKHTMLTOPDF') && USE_WKHTMLTOPDF == 0) {
            $this->render('expense_mpdf');
        } else {
            $this->render('expense_mpdf');
        }
    }
    ...
    /* function to get the project estimated hour and sumation of estimate hour of all task */
    ...
    function getTotEsthr() {
        $this->layout = "ajax";
        $this->loadModel('Project');
        // $crnt_est_hr = $this->data['estHour'];
        $arr = array();
        $CS_project_id = $this->data['proj_uniq_id'];
        //echo $CS_project_id;
        $project_estimate_hr = $this->Project->find('first', array('conditions' => array('Project.company_id' => SES_COMP, 'Project.isactive' => 1, 'Project.uniq_id' => $CS_project_id), 'fields' => array('Project.estimated_hours', 'Project.id')));
        $arr['prjct_estmt_hr'] = (int) $project_estimate_hr['Project']['estimated_hours'];
        $arr['projId'] = $project_estimate_hr['Project']['id'];
        //print_r($project_estimate_hr);exit;
        $this->Easycase->recursive = -1;
        $conds = "SELECT SUM(Easycase.estimated_hours) AS estimated_hrs FROM easycases AS Easycase WHERE Easycase.project_id=" . $project_estimate_hr['Project']['id'] . " AND Easycase.istype = 1";
        $tot_etimate_hr = $this->Easycase->query($conds);
        $arr['total_tsk_etimate_hr'] = ceil(($tot_etimate_hr[0][0]['estimated_hrs']) / 3600);
    ...
    function saveFilter() {
        ...
        $json_arr['MILESTONE'] = ($data['SearchFilter']['MILESTONE']) ? $data['SearchFilter']['MILESTONE'] : '';
    ...
    function status_report() {
        $this->layout = 'ajax';
        if (!empty($this->request->data['caseNo']) && !empty($this->request->data['projId'])) {
            $caseNo = $this->request->data['caseNo'];
...
//            pr($users); exit;
//        echo "SELECT  Easycase.actual_dt_created,DISTINCT Easycase.previous_legend,Easycase.dt_created,Easycase.user_id FROM easycases AS Easycase WHERE Easycase.project_id=$ProjId AND Easycase.case_no=$curCaseNo  GROUP BY  Easycase.actual_dt_created ORDER BY Easycase.actual_dt_created"; exit;
            $states_data = $this->Easycase->query("SELECT  Easycase.actual_dt_created, Easycase.previous_legend,Easycase.dt_created,Easycase.user_id FROM easycases AS Easycase WHERE Easycase.project_id=$projId AND Easycase.case_no=$caseNo  GROUP BY  Easycase.actual_dt_created ORDER BY Easycase.actual_dt_created");
//        $states_data = $this->Easycase->find('all', array('fields' => array('DISTNICT Easycase.previous_legend','Easycase.id', 'Easycase.actual_dt_created','Easycase.dt_created','Easycase.user_id'), 'conditions' => array('Easycase.project_id' => $ProjId, 'Easycase.case_no' => $curCaseNo),'group'=>array('Easycase.previous_legend')));
            $this->Project->unbindModel(array('hasMany' => array('ProjectUser'), 'hasAndBelongsToMany' => array('User')));
            $prjArr = $this->Project->find('first', array('conditions' => array('Project.id' => $projId), 'fields' => array('Project.id', 'Project.workflow_id')));
            if ($prjArr['Project']['workflow_id'] != 0) {
            ...
            foreach ($states_data as $lkey => $lval) {
                if ($lval['Easycase']['previous_legend'] == $states_data[$lkey + 1]['Easycase']['previous_legend']) {
                    ...
                    $state_record[$lkey]['Status']['status_name'] = !empty($lval['Easycase']['previous_legend']) ? $status[$lval['Easycase']['previous_legend']] : 'NA';
                    $start_date = !empty($lval['Easycase']['actual_dt_created']) ? $this->Tmzone->GetDateTime(SES_TIMEZONE, TZ_GMT, TZ_DST, TZ_CODE, $lval['Easycase']['actual_dt_created'], "datetime") : '';

                    $state_record[$lkey]['Status']['actual_dt_created'] = $dt->dateFormatOutputdateTime_day($start_date, $curDateTz);
                    $today = $this->Tmzone->GetDateTime(SES_TIMEZONE, TZ_GMT, TZ_DST, TZ_CODE, GMT_DATETIME, "datetime");
                    $end_date = !empty($states_data[$lkey + 1]['Easycase']['actual_dt_created']) ? $this->Tmzone->GetDateTime(SES_TIMEZONE, TZ_GMT, TZ_DST, TZ_CODE, $states_data[$lkey + 1]['Easycase']['actual_dt_created'], "datetime") : $today;
                    if ($max_id['0'] != $lval['Easycase']['previous_legend']) {
                        $state_record[$lkey]['Status']['last_dt_updated'] = !empty($states_data[$lkey + 1]['Easycase']['actual_dt_created']) ? $dt->dateFormatOutputdateTime_day($end_date, $curDateTz) : $dt->dateFormatOutputdateTime_day($end_date, $curDateTz) . " (Cont..)";
                    } else {
                        $state_record[$lkey]['Status']['last_dt_updated'] = !empty($states_data[$lkey + 1]['Easycase']['actual_dt_created']) ? $dt->dateFormatOutputdateTime_day($end_date, $curDateTz) : $dt->dateFormatOutputdateTime_day($end_date, $curDateTz);
                    }
                    $durations = $this->Format->datediffernce($start_date, $end_date);
                    if ($max_id['0'] != $lval['Easycase']['previous_legend']) {
                        $state_record[$lkey]['Status']['duration'] = !empty($states_data[$lkey + 1]['Easycase']['actual_dt_created']) ? $durations : $durations . " (Cont..)";
                    } else {
                        $state_record[$lkey]['Status']['duration'] = !empty($states_data[$lkey + 1]['Easycase']['actual_dt_created']) ? $durations : $durations;
                    }
                    $state_record[$lkey]['Status']['username'] = !empty($lval['Easycase']['user_id']) ? $users[$lval['Easycase']['user_id']] : 'NA';
                }
            }
//            sort($state_record);
            // echo "<pre>";print_r($state_record);exit;
            $this->set(compact('state_record'));
            $this->set(compact('caseUniqId'));
        }
    }

    function update_post_comment() {
        $projects = $this->Project->find('list', array('fields' => array('id', 'workflow_id')));
        foreach ($projects as $key => $value) {
            $this->loadModel('Status');
            $statuses = $this->Status->find('first', array('fields' => array('id'), 'conditions' => array('workflow_id' => $value), 'order' => array('seq_order ASC')));
            $data = $this->Easycase->UpdateAll(array('previous_legend' => $statuses['Status']['id']), array('istype' => 1, 'project_id' => $key));
            $this->Easycase->UpdateAll(array('previous_legend' => 'legend'), array('istype' => 2, 'project_id' => $key));
//            pr($value);
//            pr($data); exit;
        }
        exit;
    ...
    function kanbanview_comments() {
        $this->layout = '';
        if (!empty($this->data['uid'])) {
            $uid1 = $this->data['uid'];
            $task = $this->Easycase->find('first', array('conditions' => array('Easycase.uniq_id' => $uid1)));
            $comments_count = $this->Easycase->find('count', array('conditions' => array('Easycase.case_no' => $task['Easycase']['case_no'], 'Easycase.project_id' => $task['Easycase']['project_id'], 'Easycase.case_count' => 0, 'Easycase.istype' => 2)));
            $comments = $this->Easycase->find('all', array('conditions' => array('Easycase.case_no' => $task['Easycase']['case_no'], 'Easycase.project_id' => $task['Easycase']['project_id'], 'Easycase.case_count' => 0, 'Easycase.istype' => 2), 'limit' => '5'));
            $comment_arr = array();
            foreach ($comments as $key => $val) {
                ...
                if (empty($desp)) {
                    $desp = !empty($val["Easycase"]['message']) ? $val["Easycase"]['message'] : __('No Comment',true);
                }
