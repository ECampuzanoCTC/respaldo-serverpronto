...
class UsersController extends AppController {
    ...
    function company($img = null) {
        ...
        if ($photo && $info) {
            ...
            $this->Session->write("SUCCESS", __("Company photo removed successfully"));
        ...
        if (isset($this->request->data['Company'])) {
            ...
            if (isset($this->request->data['Company']['photo'])) {
                ...
                if ($photo_name == "ext") {
                    $this->Session->write("ERROR", __("Company logo should be an image file"));
                    $this->redirect(HTTP_ROOT . "users/company");
                } elseif ($photo_name == "size") {
                    $this->Session->write("ERROR", __("Company logo size cannot excceed 1mb"));
                    $this->redirect(HTTP_ROOT . "users/company");
                }
            }
            if (trim($this->request->data['Company']['name']) == "") {
                $this->Session->write("ERROR", __("Name cannot be left blank"));
                $this->redirect(HTTP_ROOT . "users/company");
            } else {
                ...
                $this->Session->write("SUCCESS", __("Company updated successfully"));
                $this->redirect(HTTP_ROOT . "users/company");
            }
        }
...
//			}
        ...
        if (isset($this->request->data['DailyupdateNotification'])) {
            ...
            $this->Session->write("SUCCESS", __("Notification settings saved successfully"));
    ...
    function manage($usrview = NULL) {
        ...
        if (isset($_GET['del']) && trim(urldecode($_GET['del'])) != "") {
            ...
            //End 
            $this->Session->write("SUCCESS", __("user deleted successfully"));
        ...
        if (isset($_GET['act']) && trim(urldecode($_GET['act'])) != "") {
            if (($GLOBALS['Userlimitation']['subscription_id'] == 1) && (strtolower($GLOBALS['Userlimitation']['user_limit']) != "unlimited")) {
                if ($GLOBALS['usercount'] >= $GLOBALS['Userlimitation']['user_limit']) {
                    $this->Session->write("ERROR", __("Sorry! User cannot be enabled. User Limit Exceeded!"));
            ...
            //End 
            $this->Session->write("SUCCESS", __("User enabled successfully"));
        ...
        if (isset($_GET['deact']) && trim(urldecode($_GET['deact'])) != "") {
            ...
            //End 
            $this->Session->write("SUCCESS", __("User disabled successfully"));
        ...
        if (isset($_GET['grant_admin']) && trim(urldecode($_GET['grant_admin'])) != "") {
            ...
            $this->Session->write("SUCCESS", __("Granted admin privilege"));
            $this->redirect(HTTP_ROOT . "users/manage");
        }
        ...
        if (isset($_GET['grant_client']) && trim(urldecode($_GET['grant_client'])) != "") {
            ...
            if ($usr) {
                //print_r($usr);exit;
                $id = $usr['User']['id'];
                $CompanyUser->query("UPDATE company_users as CompanyUser SET CompanyUser.is_client='1' WHERE CompanyUser.user_id='" . $id . "' AND CompanyUser.company_id='" . SES_COMP . "' AND CompanyUser.user_type!='1'");
                //for single company only
                $CompanyUser->updateUserPerm(SES_COMP, $usr['User']['id'], 4);
                $this->Session->write("SUCCESS", __("Granted client privilege"));
                $this->redirect(HTTP_ROOT . "users/manage");
            } else {
                $this->redirect(HTTP_ROOT . "users/manage");
            }
        ...
        if (isset($_GET['revoke_client']) && trim(urldecode($_GET['revoke_client'])) != "") {
            ...
            if ($usr) {
                ...
                $id = $usr['User']['id'];
                //print_r($id);exit;			
                $CompanyUser->query("UPDATE company_users as CompanyUser SET CompanyUser.is_client='0' WHERE CompanyUser.user_id='" . $id . "' AND CompanyUser.company_id='" . SES_COMP . "' AND CompanyUser.user_type!='1'");
                //for single company only
                $CompanyUser->updateUserPerm(SES_COMP, $usr['User']['id'], 0);
                $this->Session->write("SUCCESS", __("Revoked client privilege"));
                $this->redirect(HTTP_ROOT . "users/manage");
            } else {
                $this->redirect(HTTP_ROOT . "users/manage");
            }
        ...
        if (isset($_GET['revoke_admin']) && trim(urldecode($_GET['revoke_admin'])) != "") {
            ...
            $this->Session->write("SUCCESS", __("Revoked admin privilege"));
            $this->redirect(HTTP_ROOT . "users/manage");
        }
        if (isset($_GET['resend']) && trim(urldecode($_GET['resend'])) != "") {
            ...
            if ($invit['UserInvitation']['user_id']) {
                ...
                $loggedin_users = $this->Format->getUserNameForWithooutEmailActive($this->Auth->User("id"));
                ...
                if (!$getUser['User']['password']) {
                    ...
                    $subject = $fromName . " " . __("Invited you to join on Orangescrum", true);
                }
                ...
                if ($this->Sendgrid->sendgridsmtp($this->Email)) {
                    ...
                    $this->Session->write("SUCCESS", __("Invitation resent to") . " '" . $getUser['User']['email'] . "'");
                    $this->redirect(HTTP_ROOT . "users/manage/?role=invited");
                }
            }
        }
        ...
        if ($role == "invited") {
            $userArr = $this->User->query("SELECT SQL_CALC_FOUND_ROWS * FROM company_users AS CompanyUser LEFT JOIN users AS User ON CompanyUser.user_id=User.id WHERE CompanyUser.company_id=" . SES_COMP . "  AND CompanyUser.is_active ='2' AND User.email!='' AND (" . $search_query . ") ORDER BY User.dt_created $limit");
        } else if ($role == "clients" && defined('INV') && INV == 1) {
            $conditionts = array('User.is_client' => true);
            $userArr = $this->User->find('all', array('conditions' => $conditionts, 'recursive' => false));
        } else {
            $userArr = $this->User->query("SELECT SQL_CALC_FOUND_ROWS *,Role.role_name FROM users AS User LEFT JOIN user_roles as Role ON Role.id = User.role_id,company_users AS CompanyUser WHERE User.id=CompanyUser.user_id AND CompanyUser.company_id='" . SES_COMP . "' " . trim($query) . " ORDER BY User.dt_last_login DESC $limit");
        }
        ...
        foreach ($userArr as $key => $usrall) {
            ...
            if (($role != 'invited') && ($usrall['CompanyUser']['is_active'] != 2)) {
                ...
                $userArr[$key]['User']['role'] = $usrall['CompanyUser']['role_id'] != 0 ? $hFormat->getRoleofUser($usrall['CompanyUser']['role_id']) : "";
            } else {
            ...
            if ($role == 'invited') {
                ...
                $userArr[$key]['User']['role'] = $usrall['CompanyUser']['role_id'] != 0 ? $hFormat->getRoleofUser($usrall['CompanyUser']['role_id']) : "User";
        ...
        if (defined('INV') && INV == 1) {
            $conditionts = array('User.is_client' => true);
            $invoiceClientRecords = $this->User->find('all', array('conditions' => $conditionts, 'recursive' => false));
            $totalCLients = sizeof($invoiceClientRecords);
            $this->set('totalclients', $totalCLients);
            $this->set('invoiceClientRecords', $invoiceClientRecords);
        }
        ...
        if (isset($_GET['resetpassword']) && $_GET['resetpassword']) {
            ...
            if (count($getData)) {
                ...
                if ($this->Sendgrid->sendGridEmail(FROM_EMAIL, $to, $subject, $message, "ResetPassword")) {
                    ...
                    $this->Session->write("SUCCESS", __("Password of") . " '" . $name . "' " . __("reset successfully"));
                    $this->redirect(HTTP_ROOT . "users/manage/");
                }
            }
        }
    ...
    function forgotpassword() {
        if (!empty($this->request->data) && empty($this->request->data['User']['repass']) && empty($this->request->data['User']['newpass'])) {
            ...
            if ($getUsrData && is_array($getUsrData) && count($getUsrData)) {
                ...
                $this->Email->delivery = EMAIL_DELIVERY;
                $this->Email->to = $to;
                $this->Email->subject = __("Forgot Password Request", true);
        ...
        if ($this->request->data['User']['email'] && !$userlimit) {
            ...
            if (strstr($this->request->data['User']['email'], ",")) {
                ...
                if (strtolower($GLOBALS['Userlimitation']['user_limit']) != 'unlimited' && ($total_new_users > $GLOBALS['Userlimitation']['user_limit'])) {
                    if (SES_TYPE == 3) {
                        $this->Session->write("ERROR", __("Sorry! you have exceeded the user limit"));
                    } else {
                        $this->Session->write("ERROR", __("Sorry! This account exceeded the user limit."));
                    }
                    $this->redirect(HTTP_ROOT);
                    exit;
                }
                ...
                if (!$err) {
                    ...
                    $this->Session->write("SUCCESS", __("Invitation sent to Successfully"));
                    ...
                    $this->Session->write("ERROR", __("Invitation Failed. Please try again!"));
                    $this->redirect(HTTP_ROOT . "users/manage/");
                }
            } else {
                ...
                if ($userid && $userid != $this->Auth->User("id")) {
                    ...
                    if ($UserInvitation->save($InviteUsr)) {
                        ...
                        if (!$resend) {
                            ...
                            $cmpnyUsr['CompanyUser']['is_active'] = 2;
                            if(defined('ROLE') && ROLE ==1){
                            $cmpnyUsr['CompanyUser']['role_id'] = $cmpnyUsr['CompanyUser']['role_id'] = (isset($this->request->data['User']['role_id']) && !empty($this->request->data['User']['role_id'])) ? $this->request->data['User']['role_id'] :3;
                            }
                            $cmpnyUsr['CompanyUser']['created'] = GMT_DATETIME;
                            if ($CompanyUser->saveAll($cmpnyUsr)) {
                                $comp_user_id = $CompanyUser->getLastInsertID();
                        ...
                        $this->Session->write("SUCCESS", __("Invitation sent to") . " '" . $this->request->data['User']['email'] . "'");
                        if ($_SERVER['HTTP_REFERER'] == HTTP_ROOT . 'onbording') {
                            $this->redirect(HTTP_ROOT . "onbording");
                            exit;
                        }
    ...
    function project_menu() {
        ...
        if ($limit != "all") {
            ...
            $allProjArr = $ProjectUser->query("select SQL_CALC_FOUND_ROWS DISTINCT p.name,p.id,p.uniq_id as uniq_id,(select count(ec.id) from easycases as ec where ec.istype='1' AND ec.isactive='1' AND " . $clt_sql . " AND pu.project_id=ec.project_id " . trim($qry) . ") as count from projects as p, project_users as pu where p.id=pu.project_id and pu.user_id='" . SES_ID . "' and pu.company_id='" . SES_COMP . "' AND p.isactive='1' ORDER BY p.name ASC LIMIT 0,$limit");
        } else {
            $allProjArr = $ProjectUser->query("select SQL_CALC_FOUND_ROWS DISTINCT p.name,p.id,p.uniq_id as uniq_id,(select count(ec.id) from easycases as ec where ec.istype='1' AND ec.isactive='1' AND " . $clt_sql . " AND pu.project_id=ec.project_id  " . trim($qry) . ") as count from projects as p, project_users as pu where p.id=pu.project_id and pu.user_id='" . SES_ID . "' and pu.company_id='" . SES_COMP . "' AND p.isactive='1' ORDER BY p.name ASC");
        }
    ...
    public function login($demo = NULL, $email = NULL, $pass = NULL, $first_login = 0) {
        ...
        if (!empty($this->request->data) || !empty($email)) {
            ...
            if (($this->Auth->login() || isset($usrLogin['User']['id'])) && $this->Auth->user('id')) {
                ...
                $sql = "SELECT CompanyUser.company_id FROM company_users CompanyUser , companies Companies WHERE Companies.id = CompanyUser.company_id AND CompanyUser.user_id=" . $this->Auth->user('id') . " AND CompanyUser.is_active=1 GROUP BY CompanyUser.company_id";
                $CompanyUser = $this->CompanyUser->query($sql);
                if ($this->Auth->user('isactive') == 2 || empty($CompanyUser)) {
                    $cookie = array();
                ...
                if (isset($_SESSION['demorequest'])) {
                    $d_t_val = $_SESSION['demorequest'];
                    unset($_SESSION['demorequest']);
                    $this->redirect($redirect . $d_t_val);
                    exit;
                }
                ...
                $this->Session->setFlash(__("Email or Password is invalid!"), 'default', array('class' => 'error'));
            ...
            if (strstr($demo, '___')) {
                ...
                if ($upd_user) {
                    ...
                    if ($t_demo[1] == "NOT_UPDATE") {
                        $this->set("update_email_message", '<span style="color:red">"' . $t_emal . '" ' . __("email already exists") . '!.</span>');
                    } else {
                        $upd_user['User']['update_random'] = '';
                        $this->User->save($upd_user);
                        $this->set("update_email_message", '<span style="color:green">' . __("Now you can login using") . ' "' . $t_emal . '"</span>');
                    }
                }
            }
        }
    ...
    function saveNewRole() {
        ...
        $data = array();
        $existing = $this->UserRole->find('first', array('conditions' => array('UserRole.role_name' => $role, 'UserRole.company_id' => array(SES_COMP, 0))));
        if (!empty($existing)) {
            $data['error'] = 1;
            $data['msg'] = 'This Role already exists.';
        } else {
            $roleData = array();
            $roleData['role_name'] = $role;
            $roleData['company_id'] = SES_COMP;
            if ($this->UserRole->save($roleData)) {
                $data['success'] = 1;
                $data['id'] = $this->UserRole->getLastInsertId();
                $data['msg'] = 'Role is saved successfully';
            } else {
                $data['error'] = 1;
                $data['msg'] = 'Could not save role. Please try again.';
            }
        }
    ...
    /*
     * Author Satyajeet
     * TO save new Business Unit
     */

    function saveNewUnit() {
        $business_unit = $this->request->data['business_unit'];
        $this->loadModel('BusinessUnit');
        $data = array();
        $existing = $this->BusinessUnit->find('first', array('conditions' => array('BusinessUnit.business_unit' => $business_unit, 'BusinessUnit.company_id' => array(SES_COMP, 0))));
        if (!empty($existing)) {
            $data['error'] = 1;
            $data['msg'] = 'This Business Unit already exists.';
        } else {
            $unitData = array();
            $unitData['business_unit'] = $business_unit;
            $unitData['company_id'] = SES_COMP;
            $unitData['dt_created'] = GMT_DATETIME;
            if ($this->BusinessUnit->save($unitData)) {
                $data['success'] = 1;
                $data['id'] = $this->BusinessUnit->getLastInsertId();
                $data['msg'] = 'Business Unit is saved successfully';
            } else {
                $data['error'] = 1;
                $data['msg'] = 'Could not save business unit. Please try again.';
            }
        }
        echo json_encode($data);
        exit;
    ...
    function saveNewTech() {
        ...
        $data = array();
        $existing = $this->Technology->find('first', array('conditions' => array('Technology.technology' => $technology, 'Technology.company_id' => array(SES_COMP, 0))));
        if (!empty($existing)) {
            $data['error'] = 1;
            $data['msg'] = 'This Technology already exists.';
        } else {
            $techData = array();
            $techData['business_unit'] = $technology;
            $techData['company_id'] = SES_COMP;
            $techData['dt_created'] = GMT_DATETIME;
            if ($this->Technology->save($techData)) {
                $data['success'] = 1;
                $data['id'] = $this->Technology->getLastInsertId();
                $data['msg'] = 'Technology is saved successfully';
            } else {
                $data['error'] = 1;
                $data['msg'] = 'Could not save technology. Please try again.';
            }
        }
        echo json_encode($data);
        exit;
    ...
    function profile($img = null) {
        ...
        if ($photo && $info) {
            ...
            if ($checkPhoto) {
                ...
                $this->Session->write("SUCCESS", __("Profile photo removed successfully"));
        ...
        if (isset($this->request->data['User'])) {
            if (trim($this->request->data['User']['email']) == "") {
                $this->Session->write("ERROR", __("Email cannot be left blank"));
                $this->redirect(HTTP_ROOT . "users/profile");
                ...
                if (!$is_cmpinfo) {
                    ...
                    $this->Session->write("ERROR", __("Opps! Email address already exists."));
                    $this->redirect(HTTP_ROOT . "users/profile");
                }
            }
            $photo_name = '';
            if (isset($this->request->data['User']['photo'])) {
                ...
                if ($photo_name == "ext") {
                    $this->Session->write("ERROR", __("Opps! Invalid file format! The formats supported are gif, jpg, jpeg & png."));
                    $this->redirect(HTTP_ROOT . "users/profile");
                } elseif ($photo_name == "size") {
                    $this->Session->write("ERROR", __("Profile photo size cannot excceed 1mb"));
                    $this->redirect(HTTP_ROOT . "users/profile");
                }
            }
            if (trim($this->request->data['User']['name']) == "") {
                ...
                $this->request->data['User']['currency'] = $this->request->data['currency'];
                $this->request->data['User']['role_id'] = isset($this->request->data['role']) ? $this->request->data['role'] : 0;
                ...
                if ($email_update) {
                    $this->Session->write("SUCCESS", __("Profile updated successfully.<br />A confirmation link has been sent to") . " '{$email_update}'.");
                } else {
                    $this->Session->write("SUCCESS", __("Profile updated successfully"));
                }
                $this->redirect(HTTP_ROOT . "users/profile");
            }
        }
            ...
            if ($UserData && $UserData['User']['update_email']) {
                ...
                if ($user_email) {
                    ...
                    if ($this->User->save($UserData)) {
                        $this->loadModel('CompanyUser');
                        //for single company only
                        $this->CompanyUser->updateUserPerm(SES_COMP, $UserData['User']['id'], 1);
                    }
    ...
    function send_update_email_noti($user = null, $upd_email) {
        if ($user) {
            ...
            $to = $upd_email;
            $Name = $user['User']['name'];
            $subject = __("Orangescrum Login Email ID Confirmation", true);
            $this->Email->delivery = EMAIL_DELIVERY;
            $this->Email->to = $to;
            $this->Email->subject = $subject;
        ...
        if (isset($this->request->data['User']) && $this->request->data['User']['changepass'] == 1) {
            if ($this->request->data['submit_Pass'] == 'Change') {
                if (trim($this->request->data['User']['old_pass']) == "") {
                    $this->Session->write("ERROR", __("Old password cannot be left blank!"));
            ...
            if ($this->request->data['User']['old_pass']) {
                ...
                if ($passwordArr['User']['password'] != md5($this->request->data['User']['old_pass'])) {
                    $this->Session->write("ERROR", __("Please enter correct old password."));
                    $this->redirect(HTTP_ROOT . "users/changepassword");
                }
                if (trim($this->request->data['User']['pas_new']) == "") {
                    $this->Session->write("ERROR", __("New password cannot be left blank!"));
                    $this->redirect(HTTP_ROOT . "users/changepassword");
                }
                if (trim($this->request->data['User']['pas_retype']) == "") {
                    $this->Session->write("ERROR", __("Re-type password cannot be left blank!"));
                    $this->redirect(HTTP_ROOT . "users/changepassword");
                }
                if ($this->request->data['User']['pas_new'] != $this->request->data['User']['pas_retype']) {
                    $this->Session->write("ERROR", __("Re-type password do not match!"));
                    $this->redirect(HTTP_ROOT . "users/changepassword");
                }
            }
            ...
            if ($this->request->data) {
                ...
                $this->Session->write("SUCCESS", __("Password changed successfully"));
                $this->redirect(HTTP_ROOT . "users/changepassword");
            }
        }
    }
    ...
    function ajax_activity() {
        ...
        if ($total != 0) {
            foreach ($activity as $k => $val) {
                $activity[$k]['User']['color'] = $fmt->getProfileBgColr($val['User']['id']);
                $activity[$k]['User']['sht_name'] = mb_substr(trim(ucfirst($val['User']['name'])), 0, 1, "utf-8");
            }
            ...
            foreach ($frmtActivity['activity'] as $key => $value) {
                foreach ($value as $k => $val) {
                    $frmtActivity['activity'][$key][$k]['User']['color'] = $fmt->getProfileBgColr($val['User']['id']);
                    $frmtActivity['activity'][$key][$k]['User']['sht_name'] = mb_substr(trim($val['User']['name']), 0, 1, "utf-8");
                }
    ...
    function activity_pichart() {
        ...
        $prjArr = $this->Project->find('first', array('conditions' => array('Project.uniq_id' => $project_id), 'fields' => array('Project.id', 'Project.workflow_id')));
        if ($prjArr['Project']['workflow_id'] != 0) {
            $wflDetails = $this->Workflow->find('first', array('conditions' => array('Workflow.id' => array($prjArr['Project']['workflow_id'], '0'))));
        }
        if (isset($wflDetails)) {
            $legend_arr = Hash::combine($wflDetails['Status'], '{n}.id', '{n}.name');
            $color_arr = Hash::combine($wflDetails['Status'], '{n}.id', '{n}.color');
        } else {
            $legend_arr = array(1 => __('New', true), 2 => __('In Progress', true), 3 => __('Closed', true), 4 => __('In Progress', true), 5 => __('Resolved', true));
            $color_arr = array(1 => '#F19A91', 2 => '#8dc2f8', 3 => '#8ad6a3', 4 => '#8dc2f8', 5 => '#f3c788');
        }
        $cond = "";
        if ($project_id == 'all') {
            $cond = '';
        ...
        $sql = "SELECT legend, count(*) AS cnt FROM easycases WHERE project_id !=0 " . $cond . " AND istype=1 AND legend != 0 AND " . $clt_sql . "  GROUP BY legend ORDER BY FIELD(legend,1,6,2,4,5,3)";
                ...
                foreach ($projectid as $pid) {
                    ...
                    if ($checkAvlMembr == 0) {
                        ...
                        if (defined('ROLE') && ROLE == 1) {
                            ...
                            $roleId = $CompanyUser->find('first', array('conditions' => array('CompanyUser.company_id' => SES_COMP, 'CompanyUser.user_id' => $userid), 'fields' => 'CompanyUser.role_id'));
                            $ProjectUser->query("INSERT INTO project_users SET id='" . $lastid . "',user_id=" . $userid . ",project_id=" . $pid . ",company_id='" . SES_COMP . "',role_id='" . $roleId['CompanyUser']['role_id'] . "',dt_visited='" . GMT_DATETIME . "'");
                            ...
                            if (!empty($actions)) {
                                foreach ($actions as $k => $action) {
                                    ...
                                    $action['ProjectAction']['project_id'] = $pid;
                ...
                if ($this->request->data['CaseTemplate']['update_temp'] == 1) {
                    if (isset($this->request->data['User']['id'])) {
                        ...
                        if ($this->CaseTemplate->save($this->request->data)) {
                            $this->Session->write("SUCCESS", __("Template updated successfully"));
                            $this->redirect(HTTP_ROOT . "users/manage_template");
                        } else {
                            $this->Session->write("ERROR", __("Template can't be updated"));
                            $this->redirect(HTTP_ROOT . "users/add_template/" . $this->request->data['User']['id']);
                        }
                    } else {
                        if ($this->CaseTemplate->save($this->request->data)) {
                            $this->Session->write("SUCCESS", __("Template added successfully"));
                            $this->redirect(HTTP_ROOT . "users/manage_template");
                        } else {
                            $this->Session->write("ERROR", __("Template can't be added"));
    ...
    function manage_template() {
        if (isset($this->params['pass'][0]) && $this->params['pass'][0]) {
            ...
            $this->CaseTemplate->delete();
            $this->Session->write("SUCCESS", __("Deleted successfully"));
            $this->redirect(HTTP_ROOT . "users/manage_template");
        }
...
//                    pr($info);exit;
    ...
    function categorytab() {
        ...
        $SearchFilterList = $this->SearchFilter->find('list', array('SearchFilter.id', 'SearchFilter.name', array('SearchFilter.user_id' => SES_ID)));
        $this->set(compact('SearchFilterList'));
        ...
        $this->set(compact('custom_active_dashboard_tab'));
    }

    function ajax_savecategorytab() {
        if ($this->data['is_ajaxflag']) {
            $this->layout = 'ajax';
            $tabvalue = !empty($this->data['tabvalue']) ? $this->data['tabvalue'] : 0;
            $custtabvalue = !empty($this->data['cust_tabvalue']) ? $this->data['cust_tabvalue'] : 0;
            $data['User']['id'] = SES_ID;
            $data['User']['active_dashboard_tab'] = $tabvalue;
            $data['User']['custom_active_dashboard_tab'] = $custtabvalue;
    ...
    function email_notifications() {
        ...
        if ($this->request->data) {
            ...
            $this->Session->write("SUCCESS", __("Notifications changed successfully"));
            $this->redirect(HTTP_ROOT . "users/email_notifications");
        }
    }
...
//			pr($this->request);exit;
            ...
            $this->Session->write("SUCCESS", __("Reports changed successfully"));
            $this->redirect(HTTP_ROOT . "users/email_reports");
        }
    }

    function mycompany($img = null) {
        if ($img != null) {
            $photo = urldecode($img);
            ...
            if ($img_photo) {
                ...
                $checkPhoto = $this->Company->find('count', array('conditions' => array('Company.logo' => $photo, 'Company.id' => SES_COMP)));
                if ($checkPhoto) {
                    if (unlink(DIR_USER_PHOTOS . $photo)) {
                        $this->Company->id = SES_COMP;
                        if ($this->Company->saveField('logo', '')) {
                            $this->Session->write("SUCCESS", __("Company logo removed successfully"));
        ...
        if (isset($this->request->data['Company'])) {
            if (trim($this->request->data['Company']['name']) == "") {
                ...
                $this->request->data['Company']['id'] = SES_COMP;
                if (defined('INV') && INV == 1) {
                    $this->request->data['Company']['logo'] = trim($this->request->data['Company']['logo']);
                }
                $Company->save($this->request->data);
                $this->Session->write("SUCCESS", __("Company updated successfully"));
                $this->redirect(HTTP_ROOT . "users/mycompany");
...
//	}
    ...
    function generateMsgAndSendUsMail($pjnames, $userid, $projUniqId, $comp) {
        ...
        if (stristr($pjnames, ",")) {
            $multiple = 1;
            $subject = __("You have been added to multiple projects on Orangescrum", true);
        } else {
            $subject = __("You have been added to", true) . " " . $pjnames . " " . __("on Orangescrum", true);
        }
    ...
    function default_view() {
        ...
        $milestone_views = $this->TaskView->find('list', array('conditions' => array('TaskView.name' => 'Milestone'), 'fields' => array('TaskView.id', 'TaskView.sub_name')));
        $this->set('milestone_views', $milestone_views);
//        pr($milestone_views); exit;
...
//        $this->set('kanban_views', $kanban_views);
        ...
        $data = $this->DefaultTaskView->find('first', array('conditions' => array('DefaultTaskView.company_id' => SES_COMP, 'DefaultTaskView.user_id' => SES_ID), 'fields' => array('DefaultTaskView.milestone_view_id', 'DefaultTaskView.task_view_id', 'DefaultTaskView.timelog_view_id', 'DefaultTaskView.kanban_view_id', 'DefaultTaskView.project_view_id', 'DefaultTaskView.id')));
        if (is_array($data) && count($data) > 0) {
            $taskview = $data['DefaultTaskView']['task_view_id'];
//            $kanbanview = $data['DefaultTaskView']['kanban_view_id'];
            $milestoneview = $data['DefaultTaskView']['milestone_view_id'];
...
//        $this->set('kanbanview', $kanbanview);
    ...
    function saveDefaultView() {
        $this->loadModel('DefaultTaskView');
        $data = array();
        $data['DefaultTaskView']['id'] = !empty($this->request->data['default_view_id']) ? $this->request->data['default_view_id'] : '';
        ...
        $data['DefaultTaskView']['milestone_view_id'] = $this->request->data['DefaultView']['milestoneviews'];
//        $data['DefaultTaskView']['kanban_view_id'] = $this->request->data['DefaultView']['kanbanview'];
...
//        pr($data);exit;
        if ($this->DefaultTaskView->save($data)) {
            $this->Session->write("SUCCESS", "Default Views set successfully.");
        } else {
            $this->Session->write("ERROR", "Default Views can not be set.");
        }
    ...
    public function cost_settings() {
        ...
        //echo "<pre>";print_r($this->request->params['named']['prjct']);exit;
        $proj_cond = array();
        if (isset($this->request->params['named']['prjct']) && !empty($this->request->params['named']['prjct'])) {
            $proj_cond = array('Project.name LIKE' => '%' . $this->request->params['named']['prjct'] . '%');
            $this->set('search_text', ucwords($this->request->params['named']['prjct']));
        }
        //  $projectRates = $this->Project->find('all', array('conditions' => array('Project.company_id' => SES_COMP, 'Project.isactive' => 1,$proj_cond, 'Project.rate !=' => NULL), 'fields' => array('name', 'id', 'rate')));
        $projects = $this->Project->find('list', array('conditions' => array('company_id' => SES_COMP, $proj_cond, 'isactive' => 1), 'fields' => array('id', 'name')));
        ...
        $roles = $this->Project->find('all', array('conditions' => array('Project.company_id' => SES_COMP, 'Project.isactive' => 1, $proj_cond), 'fields' => array('Project.name,Project.currency')));
        # echo "<pre>";print_r($roles);exit;
      //  $this->ProjectUser->bindModel(array('belongsTo' => array('User')));
        $UserProjArr = array();
        foreach ($roles as $kr => $vr) {
            $allcond = array('joins' => array(
                array(
                    'table' => 'users',
                    'alias' => 'User',
                    'type' => 'inner',
                    'conditions' => array('ProjectUser.user_id=User.id')
                )),'conditions' => array('ProjectUser.company_id' => SES_COMP, 'ProjectUser.project_id' => $vr["Project"]["id"], 'User.isactive' => 1), 'fields' => array('User.id', 'CONCAT(User.name,User.last_name) AS Uname'), 'order' => array('ProjectUser.user_id ASC'));
            $UserProjArr[$vr["Project"]["id"]] = $this->ProjectUser->find('all', $allcond);
        }
      //   echo "<pre>";print_r($UserProjArr);exit;
        foreach ($roles as $kkr => $vvr) {
            foreach ($UserProjArr as $ku => $vu) {
                if ($vvr['Project']['id'] == $ku) {
                    foreach ($vu as $kvu => $vvu) {
                        $roles[$kkr]["User"][$vvu["User"]["id"]] = $vvu[0]["Uname"];
                    }
                }
            }
        }
        // echo "<pre>";print_r($roles);print_r($UserProjArr);exit;
        $this->set(compact(array('roles', 'projects', 'projectRates')));
    }

    /* function to save the cost settings */

    function save_cost_settings() {
        if (SES_COMP == '') {
            $this->Session->write("ERROR", "Company not found");
            $this->redirect(HTTP_ROOT . 'cost-settings');
        }
        $insrt_data = array();
        # echo "<pre>";print_r($this->request->data);exit;
        if (isset($this->request->data['search_project']) && !empty($this->request->data['search_project']) && !isset($this->request->data['is_searched'])) {
            $this->redirect(array(
                'controller' => 'users', 'action' => 'cost_settings', 'prjct' => $this->request->data['search_project']));
            exit;
        }
        ...
        if (isset($this->request->data['role']) && !empty($this->request->data['role'])) {
            $this->loadModel('RoleRate');
            foreach ($this->request->data['role'] as $id => $role) {
                foreach ($role as $kr => $vr) {
                    $insrt = array();
                    if (isset($vr['id']) && !empty($vr['id'])) {
                        $insrt['id'] = $vr['id'];
                    }
                    if ((isset($vr['user_id']) && !empty($vr['user_id'])) && (isset($vr['role_id']) && !empty($vr['role_id']))) {
                        $insrt['user_id'] = $vr['user_id'];
                        $insrt['role_id'] = $vr['role_id'];
                        $insrt['project_id'] = $vr['project_id'];
                        $insrt['type_id'] = $vr['type_id'];
                        $insrt['rate'] = $vr['rate'];
                        $insrt['actual_rate'] = $vr['actual_rate'];
                        $insrt['profit'] = $vr['profit'];
                        $insrt['company_id'] = SES_COMP;
                        $insrt['dt_created'] = GMT_DATETIME;

                        array_push($insrt_data, $insrt);
                    }
                }
            }
            # echo "<pre>";print_r($insrt_data);print_r($this->request->data);exit;
            $this->RoleRate->saveAll($insrt_data);
        }

        $success = "Cost setting updated successfully.";

        $this->Session->write("SUCCESS", $success);
        $this->redirect(HTTP_ROOT . 'cost-settings');
    }

    function delete_roledata() {
        $this->layout = "ajax";
        $this->loadModel("RoleRate");
        $role_id = $this->data['roleId'];
        $msg = "";
        if ($role_id) {
            if ($this->RoleRate->delete($role_id)) {
                $msg = "success";
            } else {
                $msg = "fail";
            }
    ...
    /* function to export the user data */

    function export_user_listing($srch = Null) {
        ...
        if (isset($srch) && !empty($srch)) {
            $user_srch = htmlentities(strip_tags($srch));
        }

        $usr_role_name = "Active";
        if ($role == "invited") {
            $query.= " AND UserInvitation.is_active = '1'";
            $usr_role_name = "Invited";
        } else {
            if (!$role || $role == 'all') {
                $query.= " AND (CompanyUser.is_active = '1')";
            } else {
                if ($role == 2) {
                    ...
                    $query.= " AND CompanyUser.is_active = '0'";
                    $usr_role_name = "Disabled";
                }
            }
        }

        $search_query = "User.name LIKE '%" . $user_srch . "%' OR User.last_name LIKE '%" . $user_srch . "%' OR User.email LIKE '%" . $user_srch . "%' OR User.short_name LIKE '%" . $user_srch . "%'";
        if ($user_srch) {
            $user_srch = addslashes(urldecode(htmlentities(strip_tags($user_srch))));
            $query.= " AND (User.name LIKE '%" . $user_srch . "%' OR User.last_name LIKE '%" . $user_srch . "%' OR User.email LIKE '%" . $user_srch . "%' OR User.short_name LIKE '%" . $user_srch . "%')";
        }
        ...
        if ($role == "invited") {
            //echo "SELECT SQL_CALC_FOUND_ROWS * FROM company_users AS CompanyUser LEFT JOIN users AS User ON CompanyUser.user_id=User.id WHERE CompanyUser.company_id=" . SES_COMP . "  AND CompanyUser.is_active ='2' AND User.email!='' AND (" . $search_query . ") ORDER BY User.name ASC";exit;
            $userArr = $this->User->query("SELECT SQL_CALC_FOUND_ROWS * FROM company_users AS CompanyUser LEFT JOIN users AS User ON CompanyUser.user_id=User.id WHERE CompanyUser.company_id=" . SES_COMP . "  AND CompanyUser.is_active ='2' AND User.email!='' AND (" . $search_query . ") ORDER BY User.name ASC");
        } else if ($role == "clients" && defined('INV') && INV == 1) {
            $conditionts = array('User.is_client' => true);
            $userArr = $this->User->find('all', array('conditions' => $conditionts, 'recursive' => false));
        } else {
            $userArr = $this->User->query("SELECT SQL_CALC_FOUND_ROWS *,Role.role_name FROM users AS User LEFT JOIN user_roles as Role ON Role.id = User.role_id,company_users AS CompanyUser WHERE User.id=CompanyUser.user_id AND CompanyUser.company_id='" . SES_COMP . "' " . trim($query) . " ORDER BY User.name ASC ");
        }
        $tot = $this->User->query("SELECT FOUND_ROWS() as total");
        $totUser = count($userArr);
        $arrusr = array();
        ...
        $print_to_csv = "User Name,Email,Projects,Total Project(s),User Type,Company Designation,Status\n";
        if (count($userArr)) {
            foreach ($userArr as $k => $user) {
                ...
                if ($role == 'invited') {
                    $usr_typ_name = __('User(Invited)', true);
                    $usr_status = __('Invited', true);
                } else if ($role == 'disable') {
                    $usr_status = __('Inactive', true);
                }
                ...
                $print_to_csv.= ucfirst($user['User']['name']) . "," . $user['User']['email'] . "," . $user['User']['all_projects'] . "," . $user['User']['total_project'] . "," . $usr_typ_name . "," . $user['Role']['role_name'] . "," . $usr_status . "\n";
            }
        }

        $filename = "User_Listing_" . $usr_role_name . "_User" . date("m-d-Y_H-i-s", time());
    ...
    /* function for salary management of users */

    function salary_settings() {

        $this->loadModel('User');
        $this->loadModel('RoleRate');
        $this->loadModel('Company');
        $company_details = $this->Company->find('first', array('conditions' => array('Company.id' => SES_COMP)));
        $work_hour_per_week = (int) ($company_details['Company']['work_hour'] * $company_details['Company']['work_days']);
        $workhours_years = (int) ($work_hour_per_week * 52);
        // echo "<pre>";print_r($company_details);exit;
        //   echo $work_hour_per_week."<br/>".$workhours_years;exit;
        if ($this->request->data) {
            // echo "<pre>";print_r($this->request->data);exit;
            ...
            foreach ($data['id'] as $kd => $vd) {
                $this->User->create();
                $add_data['User']['id'] = $data['id'][$kd];
                $add_data['User']['salary'] = $data['salary'][$kd];
                if ($this->User->save($add_data)) {
                    $user_id = $this->User->id;
                    //  echo "<pre>";print_r($user_id);exit;
                    $hourly_rate = (int) ($data['salary'][$kd] / $workhours_years);
                    $this->RoleRate->updateAll(array('actual_rate' => $hourly_rate), array('user_id' => $user_id));
                }
            }
            $this->Session->write("SUCCESS", __("Salary Saved Successfully"));
            $this->redirect(HTTP_ROOT . "salary-settings");
        } else {
            $query = "SELECT CONCAT(User.name,User.last_name) as Name ,User.id AS user_id ,User.salary AS user_salary FROM users as User LEFT JOIN company_users as CompanyUser ON User.id = CompanyUser.user_id"
                    . " WHERE User.isactive = 1 ORDER BY User.name ASC";
    ...
    function getHourlyRate() {
        ...
        if ($this->request->data) {
            ...
            if (!empty($user_details['User']['salary']) && $user_details['User']['salary'] != 0) {
                $workhours_years = (int) ($work_hour_per_week * 52);
                $hourly_rate = (int) ($user_details['User']['salary'] / $workhours_years);
            } else {
                $hourly_rate = 0;
            }
            echo $hourly_rate;
            exit;
            //   echo $user_details['User']['salary']."<br/>".$workhours_years."<br/>".$hourly_rate;
            //  echo "<pre>";print_r($hourly_rate);exit;
        }
    }

    /* function for displaying default rates for invoice pro addon */

    function default_rates() {
        $this->loadModel('DefaultRate');
        $this->loadModel('User');
        $default_rate_list = $this->DefaultRate->find('all');

        $this->set('default_rate', $default_rate_list);
        $qry = "SELECT DISTINCT User.id, CONCAT(User.name, User.last_name) as Name, User.short_name FROM users as User, company_users As CompanyUser WHERE CompanyUser.user_id=User.id AND CompanyUser.company_id =" . SES_COMP . " AND CompanyUser.is_active =1 ORDER BY User.Name ASC";
        $usr_lst = $this->Easycase->query($qry);
        $user_list = array();
        foreach ($usr_lst as $ku => $vu) {
            $user_list[$vu["User"]["id"]] = $vu[0]["Name"];
        }
        $this->set('user_list', $user_list);
    }

    function save_default_rates() {
        $this->loadModel('DefaultRate');

        if (isset($this->request->data['default_rates']) && !empty($this->request->data['default_rates'])) {
            $data = $this->request->data['default_rates'];
            $insrt_data = array();
            $i = 0;
            foreach ($data as $kd => $dflt_rate) {

                $insrt = array();

                //  echo "<pre>";print_r($insrt);
                if (!empty($dflt_rate['role_id']) && !empty($dflt_rate['rate']) && $dflt_rate['task_type_id'] != 0 && !empty($dflt_rate['user_id'])) {
                    // if(isset($dflt_rate['id']) && !empty($dflt_rate['id'])){
                    $insrt_data['DefaultRate'][$i]['id'] = $dflt_rate['id'];
                    //  }/*else{
                    //  $insrt_data['DefaultRate'][$i]['id'] = '' ;
                    // } //*/
                    $insrt_data['DefaultRate'][$i]['role_id'] = $dflt_rate['role_id'];
                    $insrt_data['DefaultRate'][$i]['task_type_id'] = $dflt_rate['task_type_id'];
                    $insrt_data['DefaultRate'][$i]['user_id'] = $dflt_rate['user_id'];
                    $insrt_data['DefaultRate'][$i]['rate'] = $dflt_rate['rate'];
                    //  array_push($insrt_data,$insrt);
                }
                $i++;
            }

            #  echo "<pre>";print_r($this->request->data['default_rates']);print_r($insrt_data);exit;
            if (!empty($insrt_data['DefaultRate'])) {
                $this->DefaultRate->saveAll($insrt_data['DefaultRate']);
            }
            $success = "Default Rate updated successfully.";
            $this->Session->write("SUCCESS", $success);
            $this->redirect(HTTP_ROOT . 'default-rates');
        } else {
            $success = "Default Rate failed to update.";
            $this->Session->write("ERROR", $success);
            $this->redirect(HTTP_ROOT . 'default-rates');
        }
    }

    /* Delete the default rate row */

    function delete_default_rate() {
        $this->layout = "ajax";
        $this->loadModel("DefaultRate");
        $dflt_rate_id = $this->data['dflt_rateId'];
        $msg = "";
        if ($dflt_rate_id) {
            if ($this->DefaultRate->delete($dflt_rate_id)) {
                $msg = "success";
            } else {
                $msg = "fail";
            }
    ...
    function check_default_rate() {
        $this->loadModel('DefaultRate');
        $role_id = $this->request->data['role_id'];
        $task_type_id = $this->request->data['task_type_id'];
        $user_id = $this->request->data['user_id'];

        $default_rate = $this->DefaultRate->find('first', array('conditions' => array('DefaultRate.role_id' => $role_id, 'DefaultRate.task_type_id' => $task_type_id, 'DefaultRate.user_id' => $user_id), 'fields' => array('DefaultRate.rate')));
        if ($default_rate) {
            echo $default_rate['DefaultRate']['rate'];
        }
        exit;
    }

    function date_time_view() {
        $user_info = $this->User->find('first',array("fields"=>array("User.id",'User.time_format','User.date_format'),'conditions'=>array('User.id'=>SES_ID)));
        $this->set('time_format',$user_info['User']['time_format']);
        $this->set('date_format',$user_info['User']['date_format']);
    }
    ...
    function saveDateTime() {
        ...
        if (!empty($this->request->data)) {
            $data['User']['id'] = SES_ID;
            $data['User']['time_format'] = $this->request->data['DateTimeFormat']['time_format'];
            $data['User']['date_format'] = $this->request->data['DateTimeFormat']['date_format'];
            $data['User']['dt_updated'] = GMT_DATETIME;
            if ($this->User->save($data['User'])) {
                $this->Session->write("SUCCESS", "Date time format set successfully.");
            } else {
                $this->Session->write("ERROR", "Date time format can not be set.");
            }
        }
        $this->redirect(HTTP_ROOT . 'users/date_time_view');
    }
