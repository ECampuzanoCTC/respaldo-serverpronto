...
class InstallController extends AppController {
                ...
                if(empty($zipfilespresent)){
                    ...
                    if(empty($addonExists)){
                        ...
                        if($addon == 'timelogpayment' || $addon == 'timeloggold'){
                            ...
                            $ptlgExists = $this->Addon->find('first', array('conditions' => array('OR'=>array('Addon.name' => 'TLG','Addon.name'=>'TPAY'))));
                            if(empty($ptlgExists)){
                                    $tlgData = NULL;
                                    $tlgData['name'] = array('TLG','TPAY');
                                    $tlgData['isactive'] = array(1,1);
                                    $tlgData['dt_created'] = array(GMT_DATETIME,GMT_DATETIME);
                                    $this->Addon->saveAll($tlgData);
                            } else if(!empty($tlgExists) && $tlgExists['Addon']['name'] != 'TLG'){
                                    $tlgData = NULL;
                                    $tlgData['name'] = 'TLG';
                                    $tlgData['isactive'] = 1;
                                    $tlgData['dt_created'] = GMT_DATETIME;
                                    $this->Addon->saveAll($tlgData);
                            } else if(!empty($tlgExists) && $tlgExists['Addon']['name'] != 'TPAY'){
                                    $tlgData = NULL;
                                    $tlgData['name'] = 'TPAY';
                                    $tlgData['isactive'] = 1;
                                    $tlgData['dt_created'] = GMT_DATETIME;
    ...
    function import_addonTable(){
        ...
        if(isset($this->request->data) && !empty($this->request->data)){
            $addon = $this->request->data['addon'];
            $arr = array();
            if($addon == 'mobileapi' || $addon == 'executivedashboard'){
                $arr['success'] = '1';
            ...
            if($addon === 'ganttchart' && GTLG !== 1 && DBRD !== 1){
                $db = ConnectionManager::getDataSource('default');
                $db->execute('ALTER TABLE `easycases` ADD `gantt_start_date` DATETIME NULL DEFAULT NULL AFTER `assign_to`;');
            ...
            if(($addon == 'invoicepro' || $addon == 'timeloggold') && DBRD !== 1){
