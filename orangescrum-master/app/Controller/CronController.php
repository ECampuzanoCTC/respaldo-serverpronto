...
class CronController extends AppController {
                    ...
                    $leg = array(2, 3, 4, 5);
                        ...
                        'conditions' => array(
                            ...
                            'Easycase.type_id !=' => 10,
                                "OR" => array(
                            'Easycase.legend' => $leg,
                                    "Easycase.legend NOT IN ($status_condition)",
                                ),
                            $arr
                        ),
    ...
    /*
     * Author Satyajeet
     * To send Owner and admin Invoice weekly alert email
     */
    function InvoiceWeeklyAlert(){
        $this->loadModel('Addon');
        $inv = $this->Addon->find('first', array('conditions' => array('Addon.name' => 'INV'), 'fields' => array('Addon.isactive')));
        if(!empty($inv) && $inv['Addon']['isactive'] == 1){
            $this->loadModel('InvoiceSetting');
            $settings = $this->InvoiceSetting->find('first', array('conditions' => array(), 'fields' => array('InvoiceSetting.weekly_alert')));
            if($settings['InvoiceSetting']['weekly_alert'] == 1){
                $this->loadModel('Invoice');
                $startDate = GMT_DATETIME;
                $endDate = date("Y-m-d H:i:s", (strtotime($startDate) + (7 * 24 * 60 * 60)));
                $days_diff = (strtotime($endDate) - strtotime($startDate)) / (24 * 60 * 60);
                $this->set('from', $startDate);
                $this->set('to', $endDate);
                $this->set('days_diff', $days_diff);
                for ($i = 0; $i < $days_diff; $i++) {
                    $sevendays[] = date('Y-m-d', strtotime($startDate. '+' . $i . ' day'));
                }
                ...
                array('belongsTo' => array(
                        ...
                        'InvoiceCustomer' => array(
                            'className' => 'InvoiceCustomer',
                            'dependent' => true,
                            'foreignKey' =>'customer_id'
                        )
                    )
                ));
                $invoice = $this->Invoice->find('all', array('conditions' => array('Invoice.due_Date >=' => $startDate, 'Invoice.due_date <=' => $endDate, 'Invoice.is_paid' => 0), 'fields' => array('Invoice.*', 'DATE(Invoice.due_date) AS due_date', 'Project.name', 'CONCAT(InvoiceCustomer.first_name, " ", InvoiceCustomer.last_name) AS customer'), 'order' => 'Invoice.due_date ASC'));
                $this->Invoice->unbindModel(array('belongsTo' => array('Project', 'InvoiceCustomer')));
                $invoice = Hash::combine($invoice, "{n}.0.due_date", "{n}");
                ...
                $emailUsers = $this->CompanyUser->find('all', array('conditions' => array('CompanyUser.user_type <' => 3), 'fields' => array('User.email')));
                $emailUsers = Hash::extract($emailUsers, "{n}.User.email");
                $this->Email->delivery = 'smtp';
                $this->Email->to = $emailUsers;
                $this->Email->from = FROM_EMAIL_NOTIFY;
                $this->Email->subject = "Orangescrum - Invoice Weekly Alert from ".date('d M, Y', strtotime($startDate))." to ".date('d M, Y', strtotime($endDate));
                $this->Email->sendAs = 'html';
                $this->Email->template = "invoice_weekly_alert";
                $this->set('invoices', $invoice);
                $this->Sendgrid->sendgridsmtp($this->Email);
                echo "Success";
            }else{
                echo "You have not turned this notification on in Invoice Settings page.";
            }
        }else{
            if(empty($inv)){
                echo "You have not installed Invoice Add-on yet.";
            }else{
                echo "Invoice Add-on is not active.";
            }
        }
        exit;
    }
    ...
    /*
     * Author Satyajeet
     * To send Owner and admin Invoice alert email on due date
     */
    function InvoiceDuedateAlert(){
        $this->loadModel('Addon');
        $inv = $this->Addon->find('first', array('conditions' => array('Addon.name' => 'INV'), 'fields' => array('Addon.isactive')));
        if(!empty($inv) && $inv['Addon']['isactive'] == 1){
            $this->loadModel('InvoiceSetting');
            ...
            if($settings['InvoiceSetting']['weekly_alert'] == 1){
                ...
                $invoice = $this->Invoice->find('all', array('conditions' => array('Invoice.due_Date' => $today, 'Invoice.is_paid' => 0), 'fields' => array('Invoice.*', 'DATE(Invoice.due_date) AS due_date', 'Project.name', 'CONCAT(InvoiceCustomer.first_name, " ", InvoiceCustomer.last_name) AS customer'), 'order' => 'Invoice.due_date ASC'));
                $this->Invoice->unbindModel(array('belongsTo' => array('Project', 'InvoiceCustomer')));
                $invoice = Hash::combine($invoice, "{n}.0.due_date", "{n}");
                $this->loadModel('CompanyUser');
                $this->CompanyUser->bindModel(
                ...
                $this->Email->to = $emailUsers;
                $this->Email->from = FROM_EMAIL_NOTIFY;
                $this->Email->subject = "Orangescrum - Invoice Daily Alert for ".date('d M, Y', strtotime($today));
                $this->Email->sendAs = 'html';
                $this->Email->template = "invoice_daily_alert";
                $this->set('invoices', $invoice);
                $this->Sendgrid->sendgridsmtp($this->Email);
                echo "Success";
            }else{
    ...
    function autoGenerateInvoices(){
            ...
            array(
                'hasOne' => array(
                    'InvoiceCustomer' => array(
                        ...
                        'foreignKey' => FALSE,
                        'condition' => array(
                            'InvoiceCustomer.id' => 'Invoice.customer_id'
                        )
                    ),
        ...
        $invoices = $this->Invoice->find('all', array('conditions' => array('Invoice.is_recurring' => 1)));
        foreach($invoices as $k => $invoice){
            $data = array();
            $recurringDetals = array(
                'frequency' => $invoice['RecurringInvoice']['frequency'],
                'interval' => $invoice['RecurringInvoice']['interval'],
                'bymonthday' => $invoice['RecurringInvoice']['bymonthday'],
                'byday' => $invoice['RecurringInvoice']['byday'],
                'byweekno' => $invoice['RecurringInvoice']['byweekno'],
                'bymonth' => $invoice['RecurringInvoice']['bymonth'],
                'startDate' => $invoice['RecurringInvoice']['start_Date'],
                'occurrences' => $invoice['RecurringInvoice']['occurrences'],
                'endDate' => $invoice['RecurringInvoice']['end_Date'] !== '' ? $invoice['RecurringInvoice']['end_Date'] : gmdate('Y-m-d')
            );
            $data['data']['v']['data']['invoiceId'] = $invoice['Invoice']['id'];
            $data['data']['v']['data']['from'] = $this->Format->getOwnerEmailId($invoice['Invoice']['company_id']);
            $data['data']['v']['data']['to'] = $invoice['InvoiceCustomer']['email'];
            $companyName = $this->Format->getCompanyName($invoice['Invoice']['company_id']);
            $data['data']['v']['data']['subject'] = 'Invoice from '.$companyName;
            $data['data']['v']['message'] = "Hi ". $invoice['InvoiceCustomer']['title']. " ". $invoice['InvoiceCustomer']['first_name']." ". $invoice['InvoiceCustomer']['last_name']. ",\n Here's your invoice! We appreciate your prompt payment. \n Thanks for your business!\n". $companyName;
        }
        try{
            $this->requestAction('Invoice/invoices/sendInvoiceEmail',$data);
        } catch(Exception $e){
            echo $e->getMessage();
        }
        exit;
    ...
    function caseStartReminder(){
        $this->LoadModel('Easycase');
        $this->LoadModel('UserNotification');
        $this->LoadModel('CompanyUser');
        $user_ids = $this->CompanyUser->find('list', array('conditions' => array('user_type < ' => 3, 'is_active' => 1, 'user_id'), 'fields' => array('id', 'user_id')));
        $user_lists = $this->UserNotification->find('list', array('conditions' => array('user_id' => $user_ids, 'case_reminder' => 1), 'fields' => array('id', 'user_id')));

     
        $sql = "SELECT Easycase.*,Project.* from easycases as Easycase LEFT JOIN projects as Project On Project.id = Easycase.project_id WHERE Easycase.istype =1 AND DATE(Easycase.gantt_start_date)='".date('Y-m-d')."' AND Easycase.case_reminder = 1 ORDER BY Easycase.gantt_start_date ASC";
        
        $case_lists = $this->Easycase->query($sql);
       
       // echo "<pre>";print_r($case_lists);print_r($user_lists);
        if(count($case_lists) > 0){
        foreach($case_lists as $kcl => $vcl){
            $strt_time = strtotime($vcl['Easycase']['gantt_start_date']);
            $curnt_time = strtotime(date('Y-m-d H:i:s'));
           // echo $vcl['Easycase']['gantt_start_date']."<br/>";echo date('Y-m-d H:i:s')."<br/>";
            $date_dffernece = round(($strt_time - $curnt_time) / 60) ;//date('i',$strt_time - $curnt_time);
           // print_r($date_dffernece);exit;
            if(in_array($vcl['Easycase']['user_id'], $user_lists) || in_array($vcl['Easycase']['assign_to'], $user_lists)){
                if($date_dffernece == 15){
                    if($vcl['Easycase']['user_id'] == $vcl['Easycase']['assign_to']){
                    $user = $this->User->getUserFields(array('User.id' => $vcl['Easycase']['user_id']), array('id', 'name', 'last_name', 'email','timezone_id'));
                                $this->Postcase->caseDailyReminder($user,$vcl);
                    } elseif(!empty($vcl['Easycase']['user_id']) && !empty($vcl['Easycase']['assign_to'])) {
                        $user1 = $this->User->getUserFields(array('User.id' => $vcl['Easycase']['user_id']), array('id', 'name', 'last_name', 'email','timezone_id'));
                        $user2 = $this->User->getUserFields(array('User.id' => $vcl['Easycase']['assign_to']), array('id', 'name', 'last_name', 'email','timezone_id'));
                        $this->Postcase->caseDailyReminder($user1,$vcl);
                        $this->Postcase->caseDailyReminder($user2,$vcl);
                    } else {
                        if(!empty($vcl['Easycase']['user_id'])){
                            $user1 = $this->User->getUserFields(array('User.id' => $vcl['Easycase']['user_id']), array('id', 'name', 'last_name', 'email','timezone_id'));
                            $this->Postcase->caseDailyReminder($user1,$vcl);
        ...
        echo "No Task Present for today" ;
    }
       echo "Success" ; exit; 
    }

    function startDateTaskReminder(){
        $this->LoadModel('Easycase');
        $this->LoadModel('UserNotification');
        $this->LoadModel('CompanyUser');
        $user_ids = $this->CompanyUser->find('list', array('conditions' => array('is_active' => 1, 'user_id'), 'fields' => array('id', 'user_id')));
        $user_lists = $this->UserNotification->find('list', array('conditions' => array('user_id' => $user_ids, 'case_reminder' => 1), 'fields' => array('id', 'user_id')));
        //echo "<pre>";print_r($user_lists);exit;
        
        if(count($user_lists) > 0 ) {
        foreach($user_lists as $kul => $vul){
            $sql_today_strt_tsklst = "SELECT Easycase.*,Project.* from easycases as Easycase LEFT JOIN projects as Project On Project.id = Easycase.project_id WHERE Easycase.istype =1 AND DATE(Easycase.gantt_start_date)='".date('Y-m-d')."'  AND (Easycase.user_id = ".$vul." OR Easycase.assign_to = ".$vul.") ORDER BY Easycase.project_id ASC ,Easycase.case_no ASC, Easycase.gantt_start_date ASC";
            $todays_task_start_list = $this->Easycase->query($sql_today_strt_tsklst);
            $sql_today_due_tsklst = "SELECT Easycase.*,Project.* from easycases as Easycase LEFT JOIN projects as Project On Project.id = Easycase.project_id WHERE Easycase.istype =1 AND DATE(Easycase.due_date)='".date('Y-m-d')."'  AND (Easycase.user_id = ".$vul." OR Easycase.assign_to = ".$vul.") ORDER BY Easycase.project_id ASC,Easycase.case_no ASC,Easycase.gantt_start_date ASC";
            $todays_task_due_list = $this->Easycase->query($sql_today_due_tsklst);
            $sql_today_ovrdue_tsklst = "SELECT Easycase.*,Project.* from easycases as Easycase LEFT JOIN projects as Project On Project.id = Easycase.project_id WHERE Easycase.istype =1 AND DATE(Easycase.due_date) < '".date('Y-m-d')."'  AND (Easycase.user_id = ".$vul." OR Easycase.assign_to = ".$vul.") ORDER BY Easycase.project_id ASC,Easycase.case_no ASC,Easycase.gantt_start_date ASC";
            $todays_task_ovrdue_list = $this->Easycase->query($sql_today_ovrdue_tsklst);
            $user_list = $this->User->getUserFields(array('User.id' => $vul), array('id', 'name', 'last_name', 'email','timezone_id'));
            $this->Postcase->TotalCaseReminder($todays_task_start_list,$todays_task_due_list,$todays_task_ovrdue_list,$user_list);
        }
