...
class ArchivesController extends AppController {
	...
	function case_list($uniq_id = null)
	{
        ...
        if(defined('CR') && CR==1 && $this->Auth->user('is_client') == 1){
			...
			if($pjid=="all"){
				...
				$limit1 = $this->params['data']['limit1'];
				$limit2 = $this->params['data']['limit2'];
				$cse = $this->Easycase->query("SELECT Easycase.id,Easycase.title,Easycase.uniq_id,Easycase.format,Easycase.case_no,Easycase.type_id,Easycase.legend,Easycase.user_id,Easycase.dt_created,Easycase.istype,Easycase.project_id,Archive.dt_created, User.name, User.last_name, User.short_name FROM easycases as Easycase,archives as Archive, users as User WHERE Easycase.id=Archive.easycase_id AND Easycase.user_id=User.id AND Archive.type = '1' AND Archive.company_id ='".SES_COMP."' ".$qry." AND Easycase.project_id != '0' AND ".$clt_sql." ORDER BY Archive.dt_created DESC LIMIT ".$limit1.",".$limit2);
				...
				$this->loadModel('Easycase');	
				$caseCount1 = $this->Easycase->query("SELECT Easycase.id,Easycase.title,Easycase.uniq_id,Easycase.format,Easycase.case_no,Easycase.type_id,Easycase.legend,Easycase.user_id,Easycase.dt_created,Easycase.istype,Easycase.project_id,Archive.dt_created FROM easycases as Easycase,archives as Archive WHERE Easycase.id=Archive.easycase_id AND Archive.type = '1' AND Archive.company_id ='".SES_COMP."' AND ".$clt_sql." AND Easycase.project_id = '".$pjid."'");
				$caseCount = count($caseCount1);
				$this->set('caseCount',$caseCount);
				$page = $casePage;
				$limit1 = $page*$page_limit-$page_limit;
        ...
        $cse = $this->Easycase->query("SELECT Easycase.id,Easycase.title,Easycase.uniq_id,Easycase.format,Easycase.case_no,Easycase.type_id,Easycase.legend,Easycase.user_id,Easycase.dt_created,Easycase.istype,Easycase.project_id,Archive.dt_created FROM easycases as Easycase,archives as Archive WHERE Easycase.id=Archive.easycase_id AND Archive.type = '1' AND Archive.company_id ='".SES_COMP."' AND ".$clt_sql." AND Easycase.project_id = '".$pjid."' ORDER BY Archive.dt_created DESC LIMIT ".$limit1.",".$limit2);
				$this->set('page_limit',$page_limit);
				$this->set('casePage',$casePage);
				$this->set('list',$cse);
				$this->set('pjid',$pjid);
	...
	function move_list()
	{
		...
		foreach($val as $val){
			...
			$this->Easycase->query("UPDATE easycases SET isactive = 1 where case_no = ".$qrid['0']['easycases']['case_no']. " AND project_id = " .$qrid['0']['easycases']['project_id']. " AND case_count = 0");
