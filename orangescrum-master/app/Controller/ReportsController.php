...
class ReportsController extends AppController {
    ...
    function chart() {
        ...
        if (!empty($rptdata)) {
            ...
            $this->set('frm', $frmt->chngdate($rptdata[0]['SaveReport']['frm_dt']));
            $this->set('to', $frmt->chngdate($rptdata[0]['SaveReport']['to_dt']));
    ...
    function weeklyusage_report() {
        ...
        $view = new View($this);
        $frmt = $view->loadHelper('Format');
        $frmdt = $frmt->chngdate(date("M d, Y", (strtotime($dateCurnt) - (7 * 24 * 60 * 60))));
        $todt = $frmt->chngdate(date("M d, Y", strtotime($dateCurnt)));
    ...
    function hours_report() {
        ...
        if (!empty($rptdata)) {
            ...
            $this->set('frm', $frmt->chngdate($rptdata[0]['SaveReport']['frm_dt']));
            $this->set('to', $frmt->chngdate($rptdata[0]['SaveReport']['to_dt']));
            $before = $frmt->chgdate(date('M d, Y', strtotime($rptdata[0]['SaveReport']['frm_dt'])));
            $to = $frmt->chgdate(date('M d, Y', strtotime($rptdata[0]['SaveReport']['to_dt'])));
            $days = (strtotime($to) - strtotime($before)) / (60 * 60 * 24);
        } else {
            ...
            $frmt = $view->loadHelper('Format');
            $days = (strtotime(date("Y-m-d H:i:s")) - strtotime($before)) / (60 * 60 * 24) + 1;
            $this->set('frm', $frmt->chngdate(date('M d, Y', strtotime($cur_time . "-7 day"))));
            $this->set('to', $frmt->chngdate(date("M d, Y")));
        }
    }
    ...
    function hours_linechart() {
        ...
        for ($i = 0; $i <= $x; $i++) {
            ...
            $frmt = $view->loadHelper('Format');
            $dt = date('Y-m-d', strtotime(date("Y-m-d", strtotime($before)) . $m));
            $dts = $frmt->chngdate(date('M d, Y', strtotime(date("Y-m-d H:i:s", strtotime($before)) . $m)));
            $times = explode(" ", GMT_DATETIME);
            array_push($dt_arr, $dt);
            array_push($dts_arr, $dts);
        }
    ...
    function tasks_trend() {
        ...
        for ($i = 0; $i <= $x; $i++) {
            ...
            $view = new View($this);
            $frmt = $view->loadHelper('Format');
            $dts = $frmt->chngdate(date("Y-m-d H:i:s", strtotime($before)) . $m);
            $times = explode(" ", GMT_DATETIME);
            //$dt=$dt." ".$times['1'];
            array_push($dt_arr, $dt);
            array_push($dts_arr, $dts);
