...
class PostcaseComponent extends CookieComponent {
    ...
    function casePosting($formdata) { //echo "<pre>";print_r($formdata);exit;
        ...
        $postParam['Easycase']['previous_legend'] = $formdata['CS_legend'];
        $postParam['Easycase']['hours'] = $formdata['hours'];
        $postParam['Easycase']['gantt_start_date'] = $formdata['gantt_start_date'];
        $postParam['Easycase']['case_reminder'] = $formdata['CS_case_reminder'];

        if ($formdata['company_work_hour']) {
...
####### Due Date (can be NULL, change Date format)
            if ($postParam['Easycase']['due_date']) {
                ...
                $postParam['Easycase']['due_date'] = date("Y-m-d h:i a", strtotime($postParam['Easycase']['due_date']));

                $duetdt = date("Y-m-d H:i:s", strtotime($postParam['Easycase']['due_date']));
                $postParam['Easycase']['due_date'] = $this->Tmzone->convert_to_utc(SES_TIMEZONE, TZ_GMT, TZ_DST, TZ_CODE, $duetdt, "datetime");
            } else {
                $postParam['Easycase']['due_date'] = NULL;
            }
            if (!empty($postParam['Easycase']['gantt_start_date'])) {
				$postParam['Easycase']['gantt_start_date'] =date("Y-m-d h:i a", strtotime($postParam['Easycase']['gantt_start_date']));
                $strtdt = date("Y-m-d H:i:s", strtotime($postParam['Easycase']['gantt_start_date']));
                $postParam['Easycase']['gantt_start_date'] = $this->Tmzone->convert_to_utc(SES_TIMEZONE, TZ_GMT, TZ_DST, TZ_CODE, $strtdt, "datetime");
            } else {
                $postParam['Easycase']['gantt_start_date'] = NULL;
                $postParam['Easycase']['case_reminder'] = 0;
            }
...
#### Update the status and legend of original case
        ...
        $emailMsg = $this->Format->convert_ascii(trim($postParam['Easycase']['message']));
        ...
        $postParam['Easycase']['previous_legend'] =$postParam['Easycase']['legend'];
//return pr($postParam);
...
//}
        ...
        $Easycase->recursive = -1;
        $conds = "SELECT SUM(Easycase.estimated_hours) AS estimated_hrs FROM easycases AS Easycase WHERE Easycase.project_id=" . $formdata['pjrjct_id'] . " AND Easycase.istype = 1";
        $tot_etimate_hr = $Easycase->query($conds);
        $total_etimate_hr = ceil(($tot_etimate_hr[0][0]['estimated_hrs']) / 3600);
        $tsk_estimate_hr = ceil($postParam['Easycase']['estimated_hours'] / 3600);
        $new_project_estimate_hr = $formdata['prjct_estmt_hr'];
        if ($formdata['prjct_estmt_hr'] < $total_etimate_hr) {
            $dffrnce_hr = $total_etimate_hr - $formdata['prjct_estmt_hr'];
            $new_prjct_estmate_hr['Project']['estimated_hours'] = $dffrnce_hr + $formdata['prjct_estmt_hr'];
            //print_r($new_prjct_estmate_hr);echo $formdata['pjrjct_id'];exit;
            $Project->id = $formdata['pjrjct_id'];
            $Project->save($new_prjct_estmate_hr);
            $project_estimate_hr = $Project->find('first', array('conditions' => array('Project.company_id' => SES_COMP, 'Project.isactive' => 1, 'Project.id' => $formdata['pjrjct_id']), 'fields' => array('Project.estimated_hours')));
            $new_project_estimate_hr = (int) $project_estimate_hr['Project']['estimated_hours'];
        }
        if (!empty($formdata['prjct_estmt_hr']) && !empty($formdata['estimated_hours'])) {

            if ($new_project_estimate_hr < $total_etimate_hr) {
                $ret_res['estimate_cond'] = array('estimated_hr_status' => 'more', 'project_estimated_hr' => $formdata['prjct_estmt_hr'], 'total_estimated_hr' => $total_etimate_hr, 'tsk_estimate_hr' => $tsk_estimate_hr, 'tsk_id' => $caseid);
                //array_push($ret_res,$estimated_hr_data);             
            }
        }
        if (isset($postParam['Easycase']['assign_to']) && !empty($postParam['Easycase']['assign_to'])) {
...
/*
    ...
    function checkHoliday($date) {
        ...
        $new_data = array();
        if ($new_date['weekday'] == "Saturday" || $new_date['weekday'] == "Sunday") {
            $new_data['UserLeave']['company_id'] = SES_COMP;
            $new_data['UserLeave']['user_id'] = SES_ID;
            $new_data['UserLeave']['start_date'] = $date;
            $new_data['UserLeave']['end_date'] = $date;
            $new_data['UserLeave']['reason'] = $new_date['weekday'];
            return $new_data;
        } else {
            return $new_data;
        }
    ...
    function setBookedData($postParam, $estimated_hours, $easycase_id, $company_id, $assignId = '') {
        $isAssignedUserFree = 1;
        ...
        $LogTime = ClassRegistry::init('Timelog.LogTime');
        $log_cond = array('LogTime.task_date >=' => date('Y-m-d', strtotime($postParam['Easycase']['gantt_start_date'])), 'LogTime.task_date <=' => date('Y-m-d', strtotime($postParam['Easycase']['due_date'])), 'LogTime.task_id' => $easycase_id, 'NOT' => array('LogTime.user_id' => $postParam['Easycase']['assign_to']));
        $log_data = $LogTime->find('first', array('fields' => array('SUM(LogTime.total_hours) as total_hours'), 'conditions' => $log_cond));
        if (!empty($log_data['0']['total_hours'])) {
            $estimated_hours = $estimated_hours - $log_data['0']['total_hours'];
        }
        ...
        if ($isAssignedUserFree == 1) {
            foreach ($leaves as $k => $leave) {
                ...
                /* 14/09/17 */
                if(strtotime($postParam['Easycase']['gantt_start_date']) <= strtotime($leave['UserLeave']['end_date']) && strtotime($lastdate) >= strtotime($leave['UserLeave']['start_date'])){
                   $isAssignedUserFree = 3;
                   return  $isAssignedUserFree; 
                }
            }
        ...
        if (!empty($assignId)) {
            $startdate = $assigned_Resource_date = date('Y-m-d');
        } else {
        ...
        $do = $no_of_days;
        $i = $no_of_days;
        $assigned_Resource_date1 = $assigned_Resource_date;
        while ($i > 0) {
            $holiday = $this->checkHoliday($assigned_Resource_date1);
            if (!empty($holiday)) {
                $leaves[$i] = $holiday;
            }
            $assigned_Resource_date1 = date('Y-m-d', strtotime($assigned_Resource_date1 . " +" . 1 . " days"));
            $i--;
        }
//       print_r($leaves);exit;
        while ($do > 0) {
            ...
            if (!$inleave) {
                if ($startdate == date('Y-m-d', strtotime($postParam['Easycase']['due_date']))) {
                    ...
                    $new_date = getdate(strtotime($assigned_Resource_date));
                    if ($new_date['weekday'] == "Saturday" || $new_date['weekday'] == "Sunday" && $do == 1) {
                        $working_dates[] = date('Y-m-d', strtotime($assigned_Resource_date . " +" . 1 . " days"));
                    } else {
                        $working_dates[] = $assigned_Resource_date;
                    }
                    $do--;
        ...
        if (empty($partial_days)) {
            if ($estimated_hours > $perDayWorkSec) {
                ...
                if ($startdate == date('Y-m-d', strtotime($postParam['Easycase']['due_date']))) {
                    ...
                    $overload = array('date' => $startdate, 'easycase_id' => $easycase_id, 'project_id' => $postParam['Easycase']['project_id'], 'user_id' => $postParam['Easycase']['assign_to'], 'company_id' => $company_id, 'overload' => $overload_hr);
    ...
    // function to send the task reminder email //
    function caseDailyReminder($user_details, $case_details) {
        $user = ClassRegistry::init('User');
        $from = FROM_EMAIL_NOTIFY;
        $to = $user_details['User']['email'];
        App::import('helper', 'Format');
        // echo "<pre>";print_r($user_details);print_r($case_details);exit;
        $frmtHlpr = new FormatHelper(new View(null));
        if (isset($case_details['Easycase']['user_id']) && !empty($case_details['Easycase']['user_id'])) {
            $crtd_user = $user->getUserFields(array('User.id' => $case_details['Easycase']['user_id']), array('id', 'name', 'last_name', 'email'));
            $crtd_user_name = $frmtHlpr->formatText(trim($crtd_user['User']['name'] . " " . $crtd_user['User']['last_name']));
        }
        if (isset($case_details['Easycase']['assign_to']) && !empty($case_details['Easycase']['assign_to'])) {
            $assgn_user = $user->getUserFields(array('User.id' => $case_details['Easycase']['assign_to']), array('id', 'name', 'last_name', 'email'));
            $assgn_user_name = $frmtHlpr->formatText(trim($assgn_user['User']['name'] . " " . $assgn_user['User']['last_name']));
        }
        $Timezone = ClassRegistry::init('Timezone');
        $timezn = $Timezone->find('first', array('conditions' => array('Timezone.id' => $user_details['User']['timezone_id']), 'fields' => array('Timezone.gmt_offset', 'Timezone.dst_offset', 'Timezone.code')));
        App::import('Helper', 'Tmzone');
        $tmzone = new TmzoneHelper(new View(null));
        $start_date = $tmzone->GetDateTime($user_details['User']['timezone_id'], $timezn['Timezone']['gmt_offset'], $timezn['Timezone']['dst_offset'], $timezn['Timezone']['code'], $case_details['Easycase']['gantt_start_date'], "datetime");
        $start_dates = date('M d, D h:i a', strtotime($start_date));
        $end_date = date('M d, D h:i a', strtotime($case_details['Easycase']['due_date']));
        $fromname = $frmtHlpr->formatText(trim($user_details['User']['name'] . " " . $user_details['User']['last_name']));
        $subject = "Tasks Start Alert - " . ucfirst($case_details['Project']['name']) . ":" . ucfirst($case_details['Easycase']['title']);
        $message = "<table><tr><td><table cellpadding='0' cellspacing='0' align='left' border='0' style='border-collapse:collapse;border-spacing:0;text-align:left;width:600px;border:1px solid #5191BD'>
                    <tr style='background:#5191BD;height:50px;'>
                    <td style='font:bold 14px Arial;padding:10px;color:#FFFFFF;'>
                    <span style='font-size:18px;'>Orangescrum</span> - " . __("Task Reminder", true) . "
                    </td>
                    </tr>
                    <tr>
                    <td align='left' style='font:14px Arial;padding:10px;'>
                    Hi " . ucfirst(trim($user_details['User']['name'])) . ",
                    </td>
                    </tr>
                    <tr>
                    <td style='font:14px Arial;padding:10px;'>
                    " . __("This is a reminder that the following task going to start in next 15 minutes.", true) . "
                    <br/><br/><br/>
                    ...
                    <td>
                        ...
                        <tr>
                            <td>" . __($case_details['Project']['name'], true) . "</td>
                            <td>" . __($case_details['Easycase']['case_no'], true) . "</td>
                            <td>" . __($case_details['Easycase']['title'], true) . "</td>
                            <td>" . __($crtd_user_name, true) . "</td>
                            <td>" . __($assgn_user_name, true) . "</td>
                            <td>" . $start_dates . "</td>
                            <td>" . $end_date . "</td>
                        </tr>
                        </table>
    ...
    function TotalCaseReminder($today_strt_tsklst, $today_due_tsklst, $today_ovrdue_tsklst, $user_details) {
        $user = ClassRegistry::init('User');
        $from = FROM_EMAIL_NOTIFY;
        $to = $user_details['User']['email'];
        App::import('helper', 'Format');
        // echo "<pre>";print_r($today_strt_tsklst);print_r($today_due_tsklst);print_r($today_ovrdue_tsklst);exit;
        $frmtHlpr = new FormatHelper(new View(null));
        $Timezone = ClassRegistry::init('Timezone');
        $timezn = $Timezone->find('first', array('conditions' => array('Timezone.id' => $user_details['User']['timezone_id']), 'fields' => array('Timezone.gmt_offset', 'Timezone.dst_offset', 'Timezone.code')));
        App::import('Helper', 'Tmzone');
        $tmzone = new TmzoneHelper(new View(null));
        if (!empty($today_strt_tsklst)) {
            foreach ($today_strt_tsklst as $kt => $vt) {
                $today_strt_tsklst[$kt]['Easycase']['created_by'] = "--";
                $today_strt_tsklst[$kt]['Easycase']['assigned_to'] = "--";
                $today_strt_tsklst[$kt]['Easycase']['start_date'] = "--";
                $today_strt_tsklst[$kt]['Easycase']['end_date'] = "--";
                if (isset($vt['Easycase']['user_id']) && !empty($vt['Easycase']['user_id'])) {
                    $crtd_user = $user->getUserFields(array('User.id' => $vt['Easycase']['user_id']), array('id', 'name', 'last_name', 'email'));
                    $crtd_user_name = $frmtHlpr->formatText(trim($crtd_user['User']['name'] . " " . $crtd_user['User']['last_name']));

                    $today_strt_tsklst[$kt]['Easycase']['created_by'] = ucfirst($crtd_user_name);
                }
                if (isset($vt['Easycase']['assign_to']) && !empty($vt['Easycase']['assign_to'])) {
                    $assgn_user = $user->getUserFields(array('User.id' => $vt['Easycase']['assign_to']), array('id', 'name', 'last_name', 'email'));
                    $assgn_user_name = $frmtHlpr->formatText(trim($assgn_user['User']['name'] . " " . $assgn_user['User']['last_name']));
                    $today_strt_tsklst[$kt]['Easycase']['assigned_to'] = ucfirst($assgn_user_name);
                }
                if (isset($vt['Easycase']['gantt_start_date']) && !empty($vt['Easycase']['gantt_start_date'])) {
                    $start_date = $tmzone->GetDateTime($user_details['User']['timezone_id'], $timezn['Timezone']['gmt_offset'], $timezn['Timezone']['dst_offset'], $timezn['Timezone']['code'], $vt['Easycase']['gantt_start_date'], "datetime");
                    $start_dates = date('M d h:i a', strtotime($start_date));
                    $today_strt_tsklst[$kt]['Easycase']['start_date'] = $start_dates;
                }
                if (isset($vt['Easycase']['due_date']) && !empty($vt['Easycase']['due_date'])) {
                    $end_date = date('M d h:i a', strtotime($vt['Easycase']['due_date']));
                    $today_strt_tsklst[$kt]['Easycase']['end_date'] = $end_date;
                }
            }
        } //echo "<pre>";print_r($today_strt_tsklst);exit;
        if (!empty($today_due_tsklst)) {
            foreach ($today_due_tsklst as $kd => $vd) {
                $today_due_tsklst[$kd]['Easycase']['created_by'] = "--";
                $today_due_tsklst[$kd]['Easycase']['assigned_to'] = "--";
                $today_due_tsklst[$kd]['Easycase']['start_date'] = "--";
                $today_due_tsklst[$kd]['Easycase']['end_date'] = "--";
                if (isset($vd['Easycase']['user_id']) && !empty($vd['Easycase']['user_id'])) {
                    $crtd_user = $user->getUserFields(array('User.id' => $vd['Easycase']['user_id']), array('id', 'name', 'last_name', 'email'));
                    $crtd_user_name = $frmtHlpr->formatText(trim($crtd_user['User']['name'] . " " . $crtd_user['User']['last_name']));
                    $today_due_tsklst[$kd]['Easycase']['created_by'] = ucfirst($crtd_user_name);
                }
                if (isset($vd['Easycase']['assign_to']) && !empty($vd['Easycase']['assign_to'])) {
                    $assgn_user = $user->getUserFields(array('User.id' => $vd['Easycase']['assign_to']), array('id', 'name', 'last_name', 'email'));
                    $assgn_user_name = $frmtHlpr->formatText(trim($assgn_user['User']['name'] . " " . $assgn_user['User']['last_name']));
                    $today_due_tsklst[$kd]['Easycase']['assigned_to'] = ucfirst($assgn_user_name);
                }
                if (isset($vd['Easycase']['gantt_start_date']) && !empty($vd['Easycase']['gantt_start_date'])) {
                    $start_date = $tmzone->GetDateTime($user_details['User']['timezone_id'], $timezn['Timezone']['gmt_offset'], $timezn['Timezone']['dst_offset'], $timezn['Timezone']['code'], $vd['Easycase']['gantt_start_date'], "datetime");

                    $start_dates = date('M d h:i a', strtotime($start_date));
                    $today_due_tsklst[$kd]['Easycase']['start_date'] = $start_dates;
                }
                if (isset($vd['Easycase']['due_date']) && !empty($vd['Easycase']['due_date'])) {
                    $end_date = date('M d h:i a', strtotime($vd['Easycase']['due_date']));
                    $today_due_tsklst[$kd]['Easycase']['end_date'] = $end_date;
                }
            }
        }
        // echo "<pre>";print_r($today_ovrdue_tsklst);exit;
        if (!empty($today_ovrdue_tsklst)) {
            foreach ($today_ovrdue_tsklst as $kod => $vod) {
                $today_ovrdue_tsklst[$kod]['Easycase']['created_by'] = "--";
                $today_ovrdue_tsklst[$kod]['Easycase']['assigned_to'] = "--";
                $today_ovrdue_tsklst[$kod]['Easycase']['start_date'] = "--";
                $today_ovrdue_tsklst[$kod]['Easycase']['end_date'] = "--";
                if (isset($vod['Easycase']['user_id']) && !empty($vod['Easycase']['user_id'])) {
                    $crtd_user = $user->getUserFields(array('User.id' => $vod['Easycase']['user_id']), array('id', 'name', 'last_name', 'email'));
                    $crtd_user_name = $frmtHlpr->formatText(trim($crtd_user['User']['name'] . " " . $crtd_user['User']['last_name']));
                    $today_ovrdue_tsklst[$kod]['Easycase']['created_by'] = ucfirst($crtd_user_name);
                }
                if (isset($vod['Easycase']['assign_to']) && !empty($vod['Easycase']['assign_to'])) {
                    $assgn_user = $user->getUserFields(array('User.id' => $vod['Easycase']['assign_to']), array('id', 'name', 'last_name', 'email'));
                    $assgn_user_name = $frmtHlpr->formatText(trim($assgn_user['User']['name'] . " " . $assgn_user['User']['last_name']));
                    $today_ovrdue_tsklst[$kod]['Easycase']['assigned_to'] = $assgn_user_name;
                }
                if (isset($vod['Easycase']['gantt_start_date']) && !empty($vod['Easycase']['gantt_start_date'])) {
                    $start_date = $tmzone->GetDateTime($user_details['User']['timezone_id'], $timezn['Timezone']['gmt_offset'], $timezn['Timezone']['dst_offset'], $timezn['Timezone']['code'], $vod['Easycase']['gantt_start_date'], "datetime");
                    $start_dates = date('M d, D h:i a', strtotime($start_date));
                    $end_date = date('M d, D h:i a', strtotime($vod['Easycase']['due_date']));
                    $today_ovrdue_tsklst[$kod]['Easycase']['start_date'] = $start_dates;
                }
                if (isset($vod['Easycase']['due_date']) && !empty($vod['Easycase']['due_date'])) {
                    $end_date = date('M d, D h:i a', strtotime($vod['Easycase']['due_date']));
                    $today_ovrdue_tsklst[$kod]['Easycase']['end_date'] = $end_date;
                }
            }
        }
        $fromname = $frmtHlpr->formatText(trim($user_details['User']['name'] . " " . $user_details['User']['last_name']));
        $subject = "Daily Task(s) Alert - " . date('M d');
        $message = "<table><tr><td><table cellpadding='0' cellspacing='0' align='left' border='0' style='border-collapse:collapse;border-spacing:0;text-align:left;width:800px;border:1px solid #5191BD'>
                    <tr style='background:#5191BD;height:50px;'>
                    <td style='font:bold 14px Arial;padding:10px;color:#FFFFFF;'>
                    <span style='font-size:18px;'>Orangescrum</span> - " . __("Task Reminder", true) . "
                    </td>
                    ...
                    <tr>
                    <td align='left' style='font:14px Arial;padding:10px;'>
                    Good Morning " . ucfirst(trim($user_details['User']['name'])) . ",
                    </td>
                    </tr>
                    <tr>
                    <td style='font:14px Arial;padding:10px;'>
                    " . __("This is a gentle reminder of your Task(s) List today!", true) . "
                    <br/><br/><br/>
                    ...
                    <tr style='background:#5191BD;height:50px;'>
                    <td style='font:bold 14px Arial;padding:10px;color:#FFFFFF;'>
                    <span style='font-size:18px;'>" . __("Task(s) Starting Today ", true) . "</span>
                    </td>
                    </tr>";
        if (!empty($today_strt_tsklst)) {
            $message.= "<tr>
                    <td>
                       <table style='text-align:center;width:100%'>
                        <tr>
                            <th>" . __('Project', true) . "</th>
                            <th>" . __('Task Title', true) . "</th>
                            <th>" . __('Created By', true) . "</th>
                            <th>" . __('Assign To', true) . "</th>
                            <th>" . __('Start Date', true) . "</th>
                            <th>" . __('Start Date', true) . "</th>
                        </tr>";
            foreach ($today_strt_tsklst as $ktt => $vtt) {
                $message.="<tr>
                            <td>" . __($vtt['Project']['name'], true) . "</td>
                            <td>" . __($vtt['Easycase']['title'], true) . "</td>
                            <td>" . __($vtt['Easycase']['created_by'], true) . "</td>
                            <td>" . __($vtt['Easycase']['assigned_to'], true) . "</td>
                            <td>" . $vtt['Easycase']['start_date'] . "</td>
                            <td>" . $vtt['Easycase']['end_date'] . "</td>
                        </tr>";
            }
            $message.="</table>
                    </td>
            ...
            $message.="
                    <tr>
                    <td>" . __('No Task for today') . "</td>
                    </tr> ";
        }
        $message.="<tr style='background:#5191BD;height:50px;margin-top:50px'>
                    <td style='font:bold 14px Arial;padding:10px;color:#FFFFFF;'>
                    <span style='font-size:18px;'>" . __("Task(s) Due Today ", true) . "</span>
                    </td>
                    </tr>";
        if (!empty($today_due_tsklst)) {
            $message.= "<tr>
            ...
            foreach ($today_due_tsklst as $kdt => $vdt) {
                $message.="<tr>
                            <td>" . __($vdt['Project']['name'], true) . "</td>
                            
                            <td>" . __($vdt['Easycase']['title'], true) . "</td>
                            <td>" . __($vdt['Easycase']['created_by'], true) . "</td>
                            <td>" . __($vdt['Easycase']['assigned_to'], true) . "</td>
                            <td>" . $vdt['Easycase']['start_date'] . "</td>
                            <td>" . $vdt['Easycase']['end_date'] . "</td>
                        </tr>";
            }
            $message.="</table>
                    </td>
            ...
            $message.="
                     <table style='text-align:center;width:100%'>
                    <tr>
                    <td>" . __('No Due Task') . "</td>
                    </tr> </table>";
        }
        $message.="<tr style='background:#5191BD;height:50px;margin-top:50px'>
                    <td style='font:bold 14px Arial;padding:10px;color:#FFFFFF;'>
                    <span style='font-size:18px;'>" . __("Task(s) Overdue Today ", true) . "</span>
                    </td>
                    </tr>";
        if (!empty($today_ovrdue_tsklst)) {
            $message.= "<tr>
            ...
            foreach ($today_ovrdue_tsklst as $kdt => $vdt) {
                $message.="<tr>
                            <td>" . __($vdt['Project']['name'], true) . "</td>
                            <td>" . __($vdt['Easycase']['title'], true) . "</td>
                            <td>" . __($vdt['Easycase']['created_by'], true) . "</td>
