...
class FormatComponent extends Component {
    ...
    public $components = array('Session', 'Email', 'Cookie', 'Postcase');
    ...
    function getWikiCategoryName($WikiCatId) {
    ...
    function getWikiSubCategoryName($WikiSubcatId) {
    ...
    function getWikiProjectName($WikiProjectId) {
    ...
    function milestoneFilter($caseMilestone) {
        $qry = "";
        $qryAsn = "";
        if ($caseMilestone != "all") {
            if (strstr($caseMilestone, "-")) {
                ...
                $qry .= " WHERE EasycaseMilestone.milestone_id=" . $caseMilestone;
            }
        }
        return $qry;
    }

    function kanbanmilestoneFilter($caseMilestone) {
        $qry = "";
        $qryAsn = "";
        if ($caseMilestone != "all") {
            if (strstr($caseMilestone, "-")) {
                ...
                $qry .= " AND EasycaseMilestone.milestone_id=" . $caseMilestone;
            }
        }
        return $qry;
    }

    function projectFilter($caseProject) {
        $qry = "";
        $qryAsn = "";
        if ($caseProject != "all") {
            if (strstr($caseProject, "-")) {
                $asnArr = explode("-", $caseProject);
                foreach ($asnArr as $asnChk) {
                    $qryAsn .= $asnChk . ',';
                }
                $qry .= trim($qryAsn, ',');
    ...
    function downloadFile($filename, $chkfile = '') {
        set_time_limit(0);
        ob_clean();
        ...
        if (USE_S3 == 0) {
            ...
            if (file_exists(DIR_CASE_FILES . $fname)) {
                ...
                if ($chkfile != '' && file_exists(DIR_CASE_FILES . basename($chkfile))) {
                    $file_path = DIR_CASE_FILES . basename($chkfile);
                } else {
            ...
            if ($info) {
                ...
                if ($chkfile != '') {
                    $info = $s3->getObjectInfo(BUCKET_NAME, DIR_CASE_FILES_S3_FOLDER . $chkfile);
                    if ($info) {
                        $fileurl = $this->generateTemporaryURL(DIR_CASE_FILES_S3 . $chkfile);
                        $file_path = $fileurl;
                    } else {
                        $var = "<table align='center' width='100%'><tr><td style='font:bold 12px verdana;color:#FF0000;' align='center'>" . __('Oops! File not found.') . "<br/> " . __('File may be deleted or make sure you specified correct file name.') . "</td></tr></table>";
                        die($var);
    ...
    function caseMileList($mid) {
        $Milestone = ClassRegistry::init('Milestone');
        $Milestone->recursive = -1;
        $milestone = $Milestone->find('list', array('conditions' => array('Milestone.id' => $mid, 'Milestone.isactive' => 1), 'fields' => array('Milestone.title')));
        if (count($milestone) == 1) {
            $milelist = array_values($milestone);
            return $milelist[0];
        } else {
            return $milestone;
        }
...
//	}
    ...
    function new_chngdttime($lgdt, $lgtime) {
        $newdt = $lgdt . " " . $lgtime;
        if ($GLOBALS['TimeFormat'] == 2) {
            $time_format = 'H:i';
        } else {
    ...
    function getRate($user_id = NULL, $type_id = NULL, $project_id = NULL) {
        $finalrate = 0;
        $InvoiceSetting = ClassRegistry::init('InvoiceSetting');
        if ($user_id != '') {
            //find if default rate set to 1 in invoice settings
            $is_set = $InvoiceSetting->find('first', array('conditions' => array('InvoiceSetting.company_id' => SES_COMP), 'fields' => array('InvoiceSetting.is_default_rate', 'InvoiceSetting.rate')));
            if (empty($is_set['InvoiceSetting']['is_default_rate'])) {
                $User = ClassRegistry::init('User');
                // $roleId = $User->find('first', array('conditions' => array('User.id' => $user_id), 'fields' => array('User.role_id')));
                $findProjectDefaultRate = 0;
                $findInvoiceDefaultRate = 0;
                if ($user_id) {
                    $RoleRate = ClassRegistry::init('RoleRate');
                    $rate = $RoleRate->find('first', array('conditions' => array('RoleRate.user_id' => $user_id, 'RoleRate.project_id' => $project_id, 'RoleRate.type_id' => $type_id, 'RoleRate.company_id' => SES_COMP), 'fields' => array('RoleRate.rate', 'RoleRate.role_id')));

                    if (!empty($rate) && !empty($rate['RoleRate']['rate']) && $rate['RoleRate']['rate'] != 0.0) {
                        $finalrate = $rate['RoleRate']['rate'];
                    } else {
                        $findProjectDefaultRate = 1;
                    }
                } else {
                    $findProjectDefaultRate = 1;
                }
                if ($findProjectDefaultRate) {
                    $Project = ClassRegistry::init('Project');
                    $projectRate = $Project->find('first', array('conditions' => array('Project.id' => $project_id), 'fields' => array('Project.rate')));

                    if (!empty($projectRate) && !empty($projectRate['Project']['rate']) && $projectRate['Project']['rate'] != 0.0) {
                        $finalrate = $projectRate['Project']['rate'];
                    } else {
                        /*  $defaultRate = ClassRegistry::init('DefaultRate');
                          $dfltrate = $defaultRate->find('first', array('conditions' => array('DefaultRate.user_id' => $user_id,'DefaultRate.role_id' => $rate['RoleRate']['role_id'],'DefaultRate.task_type_id'=> $type_id), 'fields' => array('DefaultRate.rate')));

                          if(!empty($dfltrate) && $dfltrate['DefaultRate']['rate'] != 0.0){
                          $finalrate = $rate['DefaultRate']['rate'];
                          } else { */
                        $cmpny_dfltrate = $InvoiceSetting->find('first', array('conditions' => array('InvoiceSetting.company_id' => SES_COMP), 'fields' => array('InvoiceSetting.rate')));
                        $finalrate = $cmpny_dfltrate['InvoiceSetting']['rate'];
                        //  }
                    }
                }/*
                  if($findInvoiceDefaultRate){
                  $rate = $InvoiceSetting->find('first', array('conditions' => array('InvoiceSetting.company_id' => SES_COMP), 'fields' => array('InvoiceSetting.rate')));
                  $finalrate = $rate['InvoiceSetting']['rate'];
                  } */
            } else {
                $finalrate = $is_set['InvoiceSetting']['rate'];
            }
        }
        return $finalrate;
    }
    ...
    function getLeaveDates($start_date, $end_date, $id, $type = null) {
        ...
        for ($i = 0; $i <= $x; $i++) {
            ...
            $dt_list = array_merge($dt_list, array($dt));
        }
        if ($type == 'list') {
            return $dt_list;
        } else {
    ...
    function display_activity_log_wiki($WikiUserId, $WikiActivityLog) {
    ...
    /*
     * Author Satyajeet
     * To get the RAG Status of a project
     */

    function getRAGStatus($projects) {
        $data = $projects;

        foreach ($projects as $k => $project) {
            $project_no_of_days = $this->getProjectDuration($project['start_date'], $project['end_date']);
            if (strtotime($projects['current_date']) > strtotime($project['end_date'])) {
                $current_no_of_days = $project_no_of_days;
            } else {
                $current_no_of_days = $this->getProjectDuration($project['start_date'], $projects['current_date']);
            }
            if (isset($project['total_tasks']) && $project['spent_hours'] != "0.0" && !empty($project['spent_hours'])) {
                $prj_est_hr = (int) $project['estimated_hours'];
                if (strtotime($projects['current_date']) > strtotime($project['end_date'])) {
                    $present_tobe_hr = $prj_est_hr;
                } else {
                    if (isset($GLOBALS['company_work_hour']) && !empty($GLOBALS['company_work_hour'])) {
                        $hr_per_day = $GLOBALS['company_work_hour'];
                    } else {
                        $hr_per_day = round($prj_est_hr / $project_no_of_days, 2);
                    }
                    $present_tobe_hr = round($hr_per_day * $current_no_of_days, 2);
                }

                $min_tolerance = (isset($project['min_range_percent']) && !empty($project['min_range_percent'])) ? $project['min_range_percent'] : 0;
                $max_tolerance = (isset($project['max_range_percent']) && !empty($project['max_range_percent'])) ? $project['max_range_percent'] : 0;

                $min_deviation_estimate = round(( $min_tolerance / 100) * $present_tobe_hr, 2);
                $max_deviation_estimate = round(( $max_tolerance / 100) * $present_tobe_hr, 2);
                $min_deviation_estimate_hr = round($present_tobe_hr - $min_deviation_estimate, 2);
                $max_deviation_estimate_hr = round($present_tobe_hr - $max_deviation_estimate, 2);
                //  echo "spent_hour-->".$project['spent_hours']."<br/>Max_deviation-->".$max_deviation_estimate_hr."<br/>min_range-->".$min_deviation_estimate_hr."<br/>current-->".$present_tobe_hr."<br/>
                //             total hr-->".$prj_est_hr."<br/>rate per day -->".$hr_per_day."<br/>current_day-->".$current_no_of_days."<br/>projecy_no of days-->".$project_no_of_days."<br/>";
                if ($project['spent_hours'] <= $min_deviation_estimate_hr && $project['spent_hours'] >= $max_deviation_estimate_hr) {
                    $lag_time = round($project['spent_hours'] - $min_deviation_estimate_hr, 2);
                    $lag_times = $this->hours_day_format($lag_time);
                    $rag = 'amber';
                    $title = "Poor";
                    $title = $lag_times . " more then the normal Estimated Time";
                } else if ($project['spent_hours'] < $max_deviation_estimate_hr) {
                    //$progress_percentage = ceil(($min_deviation_estimate_hr - $project['spent_hours'])/);
                    $lag_time = round($max_deviation_estimate_hr - $project['spent_hours'], 2);
                    // echo "project_days:".$project_no_of_days."<br/>current_days:".$current_no_of_days."<br/>present_hr_to_be".$present_tobe_hr."<br/>max-deviation:".$max_deviation_estimate_hr."<br/>lag_time:".$lag_time."<br/>";
                    $lag_times = $this->hours_day_format($lag_time);
                    $rag = 'red';
                    $title = $lag_times . " less than Estimated Time";
                } else {
                    $lag_time = round($project['spent_hours'] - $min_deviation_estimate_hr, 2);
                    // echo "project_days:".$project_no_of_days."<br/>hrs_perday:".$hr_per_day."<br/>present_hr_to_be".$present_tobe_hr."<br/>max-deviation:".$max_deviation_estimate_hr."<br/>lag_time:".$lag_time."<br/>";
                    $lag_times = $this->hours_day_format($lag_time);
                    $rag = 'green';
                    $title = "Excellent: " . $lag_times . " more than Estimated Time";
                }

                if (($project['cost_approved'] != '0.00' && !empty($project['cost_approved'])) || ($project['budget'] != '0.00' && !empty($project['budget'])) && ($project['hourly_rate'] != '0' || !empty($project['hourly_rate']))) {
                    $total_cost = isset($project['cost_approved']) && !empty($project['cost_approved']) ? $project['cost_approved'] : $project['budget'];
                    $default_rate = isset($project['hourly_rate']) && !empty($project['hourly_rate']) ? $project['hourly_rate'] : '0';

                    $spent_hr = isset($project['spent_hours']) && !empty($project['spent_hours']) ? $project['spent_hours'] : 0;
                    $rate_per_day = round($total_cost / $project_no_of_days, 2);
                    $present_tobe_rate = round($rate_per_day * $current_no_of_days, 2);
                    $prj_est_hr = (int) $project['estimated_hours'];
                    if (isset($GLOBALS['company_work_hour']) && !empty($GLOBALS['company_work_hour'])) {
                        $hrs_per_day = $GLOBALS['company_work_hour'];
                    } else {
                        $hrs_per_day = round($prj_est_hr / $project_no_of_days, 2);
                    }
                    $rate_per_hr = round($rate_per_day / $hrs_per_day, 2);
                    $spent_rate = round($rate_per_hr * $spent_hr, 2);
                    $min_deviation_rate = round(( $min_tolerance / 100) * $present_tobe_rate, 2);
                    $max_deviation_rate = round(( $max_tolerance / 100) * $present_tobe_rate, 2);
                    $min_range_rate = round($present_tobe_rate + $min_deviation_estimate, 2);
                    $max_range_rate = round($present_tobe_rate + $max_deviation_rate, 2);
                    //   echo "spent_rate-->".$spent_rate."<br/>Max_deviation-->".$max_range_rate."<br/>min_range-->".$min_range_rate."<br/>present_rate-->".$present_tobe_rate."<br/>
                    //           total cost-->".$total_cost."<br/>rate per day -->".$rate_per_day."<br/>current_day-->".$current_no_of_days."<br/>projecy_no of days-->".$project_no_of_days."<br/>";
                    if ($spent_rate >= $min_range_rate && $spent_rate <= $max_range_rate) {
                        $rate_percentage = round((($spent_rate - $min_range_rate) / $min_range_rate) * 100, 2);
                        $rag2 = 'amber';
                        $title2 = $rate_percentage . " % more than minimum tolerance rate ";
                    } else if ($spent_rate > $max_range_rate) {
                        $rate_percentage = round((($spent_rate - $max_range_rate) / $max_range_rate) * 100, 2);
                        $rag2 = 'red';
                        $title2 = $rate_percentage . " % more than Budget ";
                    } else {
                        $rate_percentage = round((($min_range_rate - $spent_rate) / $spent_rate) * 100, 2);
                        $rag2 = 'green';
                        $title2 = "Excellent: " . $rate_percentage . " % less than Budget ";
                    }
                } else {
                    $rag2 = 'grey';
                    $title2 = "No budget for this project";
                }
            } else {
                if (!isset($project['spent_hours']) && empty($project['spent_hours'])) {
                    $rag = $rag2 = 'grey';
                    $title = $title2 = "No Time log is present in this project";
                } else {
                    $rag = $rag2 = 'grey';
                    $title = $title2 = "No Task present in this project";
                }
            }
            $data[$k]['rag'] = $rag;
            $data[$k]['rag_title'] = $title;
            $data[$k]['rag2'] = $rag2;
            $data[$k]['rag_title2'] = $title2;
        }
        unset($data['current_date']);
        #echo "<pre>";print_r($data);exit; 
        return $data;
        /* if(isset($project['end_date']) && (strtotime(date('Y-m-d')) - strtotime($project['end_date'])) > 0 && !isset($project['total_tasks'])){
          $rag = $rag2 = 'red';
          $title = $title2 = "Critical";

          }else{
          if(isset($project['total_tasks'])){
          if(isset($project['end_date']) && (strtotime(date('Y-m-d')) - strtotime($project['end_date'])) > 0 && $project['total_tasks'] && isset($project['closed_tasks']) && $project['total_tasks'] != $project['closed_tasks']){
          $rag = $rag2 = 'red';
          $title = $title2 = "Critical";
          }else{
          $projectduration = $this->getProjectDuration($project);
          $min_tolerance = (isset($project['min_range_percent']) && !empty($project['min_range_percent'])) ? $project['min_range_percent'] :  0 ;
          $max_tolerance = (isset($project['max_range_percent']) && !empty($project['max_range_percent'])) ? $project['max_range_percent'] :  0 ;
          $prj_est_hr = (int)$project['estimated_hours'] ;
          $min_deviation_estimate =  (int)(( $min_tolerance/100) * $prj_est_hr)  ;
          $max_deviation_estimate =  (int)(( $max_tolerance/100) * $prj_est_hr)  ;
          $min_deviation_estimate_hr = (int)($prj_est_hr - $min_deviation_estimate) ;
          $max_deviation_estimate_hr = (int)($max_deviation_estimate + $prj_est_hr) ;
          $spent_days = $this->days_differnce(isset($project['start_date']) ? $project['start_date'] : $project['dt_created']);
          $actual_hr = (int)($spent_days * 8);
          $min_deviation_actual_hr = (int)(($actual_hr * $min_tolerence)/100 ) != 0 ? (int)(($actual_hr * $min_tolerence)/100 ) : $actual_hr ;
          $max_deviation_actual_hr = (int)(($actual_hr * $max_tolerence)/100 ) != 0 ? (int)(($actual_hr * $max_tolerence)/100 ) : $actual_hr ;
          //echo "min-tolenrnce-->".$min_tolerance."<br/>max_tolernce-->".$max_tolerance."<br/>projct_esthr--->".$project['estimated_hours']."<br/> max_dev-->".$max_deviation_estimate_hr."<br/>min_deviation --->".$min_deviation_estimate_hr."<br/>actual_hr".$actual_hr."<br/>spent_hr--->".$project['spent_hours']."<br/>";
          $remaining_hr = (int)($prj_est_hr - $project['spent_hours']) ;
          if($project['spent_hours'] > $min_deviation_estimate_hr && $project['spent_hours'] < $max_deviation_estimate_hr){
          $progress_percentage = (int)((($project['spent_hours'] - $min_deviation_estimate_hr)/$project['spent_hours'])* 100);
          $rag = 'amber';
          $title = "Poor";
          $title = "Critical:".$progress_percentage." % more than Min Tolerance hour";
          //echo 'amber';
          }
          else if($project['spent_hours'] > $max_deviation_estimate_hr){
          $progress_percentage = (int)((($project['spent_hours'] - $max_deviation_estimate_hr)/$project['spent_hours'])* 100);
          $rag = 'red';
          $title = "Critical:".$progress_percentage." % more than Estimated hour";
          //echo "red";
          }else if($project['spent_hours'] < $actual_hr){
          $progress_percentage = (int)((($actual_hr - $project['spent_hours'])/$actual_hr)* 100);
          $rag = 'red';
          $title = "Critical:".$progress_percentage." % more than actual hour";
          // echo "red_more";
          } else{
          $rag = 'green';
          $title = "Excellent";
          ...
          if($project['cost_approved'] != '0.00'|| $project['budget'] != '0.00'){
          $total_cost = isset($project['cost_approved']) && !empty($project['cost_approved']) ? $project['cost_approved'] : $project['budget'] ;
          $default_rate = isset($project['hourly_rate']) && !empty($project['hourly_rate']) ? $project['hourly_rate'] : '0' ;

          $spent_hr = isset($project['spent_hours']) && !empty($project['spent_hours']) ? $project['spent_hours'] : 0;
          $rest_hour = $project['estimated_hours'] - $spent_hr ;
          $current_rate = (int)($default_rate * $spent_hr) ;
          $remaining_rate = (int) ($default_rate * $rest_hour) ;
          $spent_days = $this->days_differnce(isset($project['start_date']) ? $project['start_date'] : $project['dt_created']);
          $actual_rate = (int) ($default_rate* $spent_days * 8) ;
          $min_deviation_rate =  (int)(( $min_tolerance/100) * $total_cost)  ;
          $max_deviation_rate =  (int)(( $max_tolerance/100) * $total_cost)  ;
          $min_range_rate = (int)($total_cost - $min_deviation_estimate) ;
          $max_range_rate = (int)($max_deviation_rate + $total_cost) ;
          /* $combined_rate = $current_rate + $range_rate ;
          $differnce_rate = ($remaining_rate -$current_rate) > 0 ? $remaining_rate - $current_rate : $current_rate - $remaining_rate ;
          echo '$spent_hr==>'.$spent_hr.'--$rest_hour-->'.$rest_hour.'--$current_rate-->'.$current_rate.'--$remaining_rate-->'.$remaining_rate.'<br/>
          --$spent_days-->'.$spent_days.'---$actual_rate-->'.$actual_rate.'--$min_range_rate-->'.$min_range_rate.'--$max_range_rate-->'.$max_range_rate.'<br/>

          $combined_rate--> '.$combined_rate.'--$differnce_rate-->'.$differnce_rate; */

        /*   if($current_rate > $max_range_rate){ 
          $rate_percentage = (int)((($current_rate - $max_range_rate)/$current_rate)* 100);
          $rag2 = 'red';
          $title2 = "Critical:".$rate_percentage." % more than Budget ";
          } else if($remaining_rate > $current_rate){
          if($remaining_rate >= $min_range_rate && $remaining_rate <= $max_range_rate){
          $rate_percentage = (int)((($remaining_rate - $min_range_rate)/$remaining_rate)* 100);
          $rag2 = 'amber';
          $title2 = "Poor:".$rate_percentage." % more than Minimun Tolerannce Rate ";
          }
          else if($current_rate <= $actual_rate ){
          $rag2 = 'green';
          $title2 = "Excellent";
          } else{
          $rate_percentage = (int)((($remaining_rate - $current_rate)/$current_rate)* 100);
          $rag2 = 'red';
          $title2 = "Critical:".$rate_percentage." % less than Remaining Budget ";
          }
          } else{
          $rag2 = 'green';
          $title2 = "Excellent";
    ...
    /*
     * Author Satyajeet
     * To get Project duration
     */

    function getProjectDuration($strt_date, $end_date) {
        $start_date = date('Y-m-d', strtotime($strt_date));
        $ends_date = date('Y-m-d', strtotime($end_date));
        $start = new DateTime($start_date);
        $end = new DateTime($ends_date);
        ...
        /* $duration = 0;
          ...
          $duration = isset($project['start_date']) ? (int) ((strtotime($project['end_date']) - strtotime($project['start_date'])) / (60 * 60 * 24)) : (int) ((strtotime($project['end_date']) - strtotime($project['dt_created'])) / (60 * 60 * 24));
          }else if($project['estimated_hours']){
          $duration = (int) ($project['estimated_hours']/8); //taking 8 hours per day
          } */
        // return $duration;
    }
    ...
    function dateTimeFormat($lgdt) {
        ...
        $newdt = $date_format . " " . $time_format;
        return date($newdt, strtotime($lgdt));
    }
