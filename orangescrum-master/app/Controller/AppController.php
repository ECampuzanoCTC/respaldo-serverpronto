...
class AppController extends Controller {
    ...
    public $addons = array('TLG' => 'Time Log', 'INV' => 'Invoice', 'TSG' => 'Task Status Group', 'GNC' => 'GanttChart', 'RCT' => 'Recurring Task', 'API' => 'API', 'CHAT' => 'Chat', 'CR' => 'ClientManagement', 'PT' => "Project Template", 'MAPI' => 'Mobile API', 'TPAY' => 'Timelog with Payment', 'LANG' => 'Multilanguage Support', 'GTLG' => 'Timelog Gold', 'DBRD' => 'Executive Dashboard', 'GINV' => 'Invoice Pro', 'ROLE' => 'User Role Management', 'EXP' => 'Expense', 'WIKI' => 'Wiki');
    public $addonTables = array('TLG' => 'log_times', 'INV' => array('invoices', 'invoice_activities', 'invoice_customers', 'invoice_logs', 'currencies', 'invoice_activities'), 'TSG' => 'workflows', 'API' => 'company_apis', 'CHAT' => array('chat_groups', 'oschats', 'time_zone', 'time_zone_leap_second', 'time_zone_name', 'time_zone_transition', 'time_zone_transition_type'), 'TPAY' => array('log_times', 'payments', 'payment_activities', 'payment_logs', 'currencies'), 'LANG' => array('languages'), 'GTLG' => array('log_times', 'payments', 'payment_activities', 'payment_logs', 'currencies', 'project_booked_resources'), 'ROLE' => array('actions', 'modules', 'role_actions', 'role_groups', 'role_modules', 'roles'), 'EXP' => array('expenses', 'expense_details', 'expense_categories', 'expense_attachments', 'expense_approvers', 'expense_notes', 'expense_activities', 'approve_expenses', 'reject_expenses'), 'WIKI' => array('wikis', 'wiki_attachments', 'wiki_categories', 'wiki_subcategories', 'wiki_comments', 'wiki_approvers', 'wiki_activities', 'approve_wikis', 'reject_wikis')/* , 'DBRD' => array('technologies', 'project_types', 'project_statuses', 'business_units', 'user_roles', 'role_rates', 'currencies', 'invoice_customers', 'industries') */);
    ...
    public $addonFolders = array('TLG' => 'Timelog', 'INV' => 'Invoice', 'TSG' => 'TaskStatusGroup', 'GNC' => 'Ganttchart', 'API' => 'API', 'CHAT' => 'Chat', 'CR' => 'ClientRestriction', 'PT' => 'ProjectTemplate', 'MAPI' => 'MobileApi', 'TPAY' => 'Timelog', "LANG" => 'MultiLanguage', 'GTLG' => 'Timelog', 'DBRD' => 'Dashboard', 'GINV' => 'Invoice', 'ROLE' => 'RoleManagement', 'EXP' => 'Expense', 'WIKI' => 'Wiki');
    ...
    public function beforeFilter() {
        ...
        if ($this->Auth->User("id")) {
            $this->loadModel('User');
            $user_date_time = $this->User->find('first', array("fields" => array("User.id", 'User.time_format', 'User.date_format'), 'conditions' => array('User.id' => $this->Auth->User("id"))));
            if (!empty($user_date_time['User']['time_format']) && $user_date_time['User']['time_format'] == 2) {
                $GLOBALS['TimeFormat'] = 2;
            } else {
                $GLOBALS['TimeFormat'] = 1;
            }
            if (!empty($user_date_time['User']['date_format']) && $user_date_time['User']['date_format'] == 2) {
                $GLOBALS['DateFormat'] = 2;
            } else {
                $GLOBALS['DateFormat'] = 1;
            }
        } else {
        ...
        if ($this->Auth->User("id")) {
            ...
            if ($this->Auth->User('id')) {
                ...
                define('WIKI', $this->is_AddonInstalled('WIKI'));
                ...
                define('DBRD', $this->is_AddonInstalled('DBRD'));
                define('GINV', $this->is_AddonInstalled('GINV'));
               // define('GINV', 1);
                define('ROLE', $this->is_AddonInstalled('ROLE'));
                define('EXP', $this->is_AddonInstalled('EXP'));
            ...
            if (!defined('SES_TIMEZONE')) {
                $this->loadModel('User');
                $user_timezone = $this->User->find('first', array("fields" => array("User.id", 'User.timezone_id'), 'conditions' => array('User.id' => $this->Auth->User("id"))));
                define('SES_TIMEZONE', $user_timezone['User']['timezone_id']);
            } else {
                $this->loadModel('User');
                $user_timezone = $this->User->find('first', array("fields" => array("User.id", 'User.timezone_id'), 'conditions' => array('User.id' => $this->Auth->User("id"))));
                define('SES_TIMEZONE', $user_timezone['User']['timezone_id']);
            ...
            if ((!stristr(PAGE_NAME, "ajax_") && !in_array(PAGE_NAME, $ajaxPageArray)) || !@$_COOKIE['SES_COMP']) {
                ...
                if (defined('ROLE') && ROLE == 1 && !defined('SES_ROLE')) {
                ...
                if (defined('ROLE') && ROLE == 1 && !defined('SES_ROLE')) {
                    define('SES_ROLE', $_COOKIE['SES_ROLE']);
                }
                if (!defined('SES_CLIENT')) {
                    define('SES_CLIENT', $_COOKIE['SES_CLIENT']);
                }
                if (!defined('SES_COMP')) {
            ...
            if (defined('GTLG') && GTLG == 1 || (defined('DBRD') && DBRD == 1) || (defined('GINV') && GINV == 1)) {
                $companyWorkHours = $Company->find('first', array('conditions' => array('Company.id' => SES_COMP), 'fields' => array('Company.work_hour,Company.is_allowed')));
                $GLOBALS['company_work_hour'] = $companyWorkHours['Company']['work_hour'];
                $GLOBALS['is_admin_allowed'] = $companyWorkHours['Company']['is_allowed'];
            }
            ...
            // find out the value of task reminder of the user //
                $this->loadModel('UserNotification');
                $usernotification_list = $this->UserNotification->find('first', array('conditions' => array('UserNotification.user_id' => SES_ID)));
                if (count($usernotification_list) > 0) {
                    $GLOBALS['case_reminder'] = $usernotification_list['UserNotification']['case_reminder'];
                }
            }
            ...
            if (!stristr(PAGE_NAME, "ajax_") && !in_array(PAGE_NAME, $ajaxPageArray)) {
                ...
                $getlatestproj = $this->ProjectUser->query("SELECT DISTINCT Project.id,Project.uniq_id,Project.name,Project.default_assign FROM project_users AS ProjectUser,projects AS Project WHERE Project.id= ProjectUser.project_id AND ProjectUser.user_id=" . SES_ID . " AND Project.isactive='1' AND Project.company_id='" . SES_COMP . "' order by ProjectUser.dt_visited DESC limit 1");
                ...
                if (defined('DBRD') && DBRD == 1 || defined('GINV') && GINV == 1 || defined('EXP') && EXP == 1) {
                    $this->loadModel('InvoiceCustomer');
                    $customer_list = $this->InvoiceCustomer->getCustomerList();
                    $GLOBALS['customer_list'] = $customer_list;
                }
            }
            ...
            if (PAGE_NAME == "dashboard" || PAGE_NAME == "milestone" || PAGE_NAME == "milestonelist" || PAGE_NAME == "activity" || PAGE_NAME == "glide_chart" || PAGE_NAME == 'chart' || PAGE_NAME == 'hours_report' || PAGE_NAME == 'mydashboard'|| PAGE_NAME == 'invoice' || PAGE_NAME == 'time_log' || PAGE_NAME == 'ganttv2') {
                $caseUrl = "";
                $urllvalue = 0;
                $urllvalueCase = 0;
                $projUniq = "";
                ...
                if (count($getallproj) == 1) {
                    ...
                    } else if ($_COOKIE['Invoice_All_Project']) {
                        $allpj = $_COOKIE['Invoice_All_Project'];
                    } else {
                        $allpj = "";
                    }
                }
                ...
                } elseif (!isset($_GET['project']) && $projUniq == "") {
                ...
                #echo $projUniq."<br/>".$curProjId;exit ;
            if(defined('ROLE') && ROLE == 1){
                if(isset($curProjId) && $curProjId != 'all' && $curProjId != ''){
                    $this->loadModel('ProjectUser');
                    $projectRole = $this->ProjectUser->find('first', array('conditions' => array('ProjectUser.project_id' => $curProjId, 'ProjectUser.user_id' => SES_ID), 'fields' => 'ProjectUser.role_id'));
                    ...
                    if(empty($projectRole['ProjectUser']['role_id']) || $projectRole['ProjectUser']['role_id'] == 0){
                        ...
                        $this->RoleAction->bindModel(array('belongsTo' => array('Action')));
                        $roleAccess = $this->RoleAction->find('all', array('conditions' => array('RoleAction.role_id' => SES_ROLE, 'RoleAction.company_id' => SES_COMP), 'fields' => array('Action.action', 'RoleAction.is_allowed')));
                        $roleAccess = Hash::combine($roleAccess, '{n}.Action.action', '{n}.RoleAction.is_allowed');
                    
                    } else{
                    $roleAccess = $this->ProjectAction->find('all', array('conditions' => array('ProjectAction.role_id' => $projectRole['ProjectUser']['role_id'], 'ProjectAction.project_id' => $curProjId), 'fields' => array('Action.action', 'ProjectAction.is_allowed')));
                    $roleAccess = Hash::combine($roleAccess, '{n}.Action.action', '{n}.ProjectAction.is_allowed');
                    }
                   # echo 'projectId=> '.$curProjId."<br/>";echo "<pre>";print_r($roleAccess);exit;
                ...
                $this->loadModel('Role');
                    $rolelist = $this->Role->find('list',array('conditions'=>array('Role.company_id' => SES_COMP),'fields'=>array('Role.id','Role.role'),'order'=>'Role.role ASC'));
                  
                    $this->set('rolelist',$rolelist);
                if (PAGE_NAME == 'mydashboard' && CONTROLLER == 'easycases') {
                    if (array_key_exists('View Dashboard', $roleAccess) && $roleAccess['View Dashboard'] == 0) {
                        $this->redirect(HTTP_ROOT . "dashboard");
                    }
                }
            }
        ...
        $new_dashboard = array('1' => array('id' => 1, 'name' => 'Timelog_chart1'),
            '2' => array('id' => 2, 'name' => 'Timelog_chart2'),
            '3' => array('id' => 3, 'name' => 'Timelog_chart3'));
        $GLOBALS['NEW_DASHBOARD'] = $new_dashboard;
